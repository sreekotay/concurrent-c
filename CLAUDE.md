...
- The lowerer is `cc/lower/*.cch` / `*.ccs`. **Fixes land in those sources**; `cc/bootstrap/lowerer/<pin>` and `out/.cc-build/clean/cc/lower/*` are regenerate-only, never hand-edited to “fix” a bug. Iterate: `make -C cc lower-cc`, gate: `./scripts/lowerer_selfhost.sh`, ship a seed: `./scripts/ship_seed.sh --promote`. Details: `cc/bootstrap/lowerer/README.md`.
