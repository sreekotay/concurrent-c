...
| `@parallel(h) { … }` | Growing dest: admit onto `h`. Statement; no `!>`. Snapshot captures. Never deny. Live set; `h.wait()` joins who is still running. Admit is safe from any fiber, concurrent with `h.wait()`. |
