...
| `@parallel(h) { … }` | Growing dest: admit onto `h` (`h`, `zip.app`, `srv->app`). Statement; no `!>`. Snapshot captures. Never deny. Live set; `h.wait()` joins who is still running. Admit is safe from any fiber, concurrent with `h.wait()`. |
| dest body (`@parallel(h) { … }` body, arms of `CCParallel h = @parallel { … } !>;`) | A fiber of `h`; the frame has moved on. `!>` resolves inside the body (`@errhandler` at its top, or `!>(e) { … }`) or is ill-formed. `return;` ends the fiber. `h` is nameable: `h.cancel()`, `h.fail(e)`. |
| `h.fail(e)` | `bool`. Records the dest error; first wins. `h.wait()` returns it after the join. Does not cancel — `h.fail(e); h.cancel() !>;` for both. Batch dest: fail. Accept dest: handle in the body; one bad client does not fail the server. |
| captures in a dest body | Closure rules: pointer copies the pointer, value copies, owner (`.destroy` type: `CCSocket`, `CCListener`, `CCSignal`, `CCString`) **moves** in. Atomic and `CCParallel` names are the frame's cell. Assign, `&x`, or `x.m(` on a by-value dest-bound name is ill-formed — `T* p = &x` in the frame (shared) or stop naming `x` there (moved). Immediate-wait arms: everything by reference. |
| server stop (`CCSignal`) | `CCSignal stop = cc_signal_watch2(SIGINT, SIGTERM) !> @destroy;` one arm `stopp->wait() !>(e){return;}; lnp->close();`, accept arm breaks on `CLOSED` then `stopp->close();`, `h.wait()` drains. `ln.close()` is two-phase (stop admission; last accepter out frees) and never from a signal handler. Process-directed signals only: `kill(getpid(), sig)`, not `raise()`. |
...
| `CCParallel h = @parallel { … } !>;` | Starts arms; does not join. `h.live()` is planted and not joined or left. `cc_parallel_empty()` is idle. After `h.wait()`, `h.joined` and `!h.live()`. `h.close(tx)` arms EMPTY to close `tx` on wait and leave. `h.leave()` / `h.leave(ctx, finish)` consume without joining (leftover LEFT-only). Do not mix wait and leave. Next kick overwrites `h`. When there is a kick, the first arm has finished; siblings may still run. One assignment arm is ill-formed: this dest is never live on the caller. One expression arm is the worker (spawned); dest is live. Join with `!>.wait()!>`, or bind the dest. Dest-bound arms are closures (see dest-body captures): assignment-arm LHS (`a = expr`) and a serial arm's one outer write are the construct's out slot and stay the frame; `&x` or `x.m(` on any other by-value name is ill-formed. `@destroy` waits (no cancel). `h.invalidate()` cancels then waits. `h.leave()` does not join — dest-live assignment-arm names are a frame hold. |
