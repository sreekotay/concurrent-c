...
| captures in a dest body | Closure rules: pointer copies the pointer, value copies, owner (`.destroy` type: `CCSocket`, `CCListener`, `CCSignal`, `CCString`) **moves** in. Atomic and `CCParallel` names are the frame's cell. Two arms sharing a listener take `CCListener* lnp = &ln;` in the frame — `ln.close()` on a bare `ln` closes a copy. Immediate-wait arms: everything by reference. |
