...
## CLI Modes
- `ccc <src> -o <out>` — Phase 1 simple compilation, no build.cc.
- `ccc build <target>` — Phase 2+ integrated build: executes build.cc, then compiles targets with exported consts in scope.
...
## Near-Term Work
- Keep Phase 1 build flow unchanged (Makefile/`ccc` binary).
...
- CLI: `ccc build [options] <input.ccs> <output.c>`.
...
- If no build.cc is found, compilation proceeds with only CLI consts (or none), same as `ccc <input.ccs> <output.c>`.
