...
That builds the compiler (and its TinyCC build dependency), installs to
`$PREFIX/bin`, and verifies the install by compiling a program against it.
Add `$PREFIX/bin` to `PATH` and `ccc` is ready:
