...
- **[`@variant`](cheatsheet.md#variant--data-alternatives)** — Ordinary tagged data, not `T!>(E)`. One arm is always active; projection needs a `switch` / `kind ==` / `?>` / `!>`; a variant `switch` names every arm (`default:` forfeits the check). Recipe: [recipe_variant.ccs](../examples/recipe_variant.ccs).
