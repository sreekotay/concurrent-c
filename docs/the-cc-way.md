...
- **[Typeviews](typehooks-typeviews.md#2-typeview--faces-and-allow-lists)** — Same-object authority lenses; narrow what a caller may do without wrappers or new allocation. Peel a protocol-free dest / transport face from the app page when listen/poll/fill/write is a different fact from HTTP or policy (specimen: [`real_projects/staticd/CCServer.cch`](../real_projects/staticd/CCServer.cch)).
...
- **[Results / `!>` / `?>`](language-concepts.md#2-errors-map-to-a-value-or-to-control-flow)** — Make fallible transitions force acknowledgement without forcing local policy. Do not make fallible operations appear infallible; keep errors distinct from successful values. A give-up that looks like success is silent degradation — say so at the position that caused it, unless the caller can already tell “couldn't” from “no work needed.”
- **[`@variant`](cheatsheet.md#variant--data-alternatives)** — Ordinary tagged data, not `T!>(E)`. One arm is always active; `@switch` / `kind ==` / `?>` / `!>` protect projection; use `case .arm(bind):` and field-path subjects (`r->del`). Recipe: [recipe_variant.ccs](../examples/recipe_variant.ccs) · [spec](../spec/draft_variants.md).
