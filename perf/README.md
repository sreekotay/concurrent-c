...
| `arena_lifetime_bench.ccs` | Per-operation cost of the lifetime model: bump alloc, checkpoint child capture/restore (empty, `@scratch` shape, nested, grown), unsized vs sized release, size-class reuse, tip vs buried regrow, Vec push/get/churn through owner tokens, String append/promote/template, Map insert/destroy, owner header and token registry churn. Compare against a ref's headers via `compare_arena_lifetime.sh <ref>`; receipts in `baselines/arena_lifetime_*.txt`. |
