...
echo "in ~2s. A *non-promoting* M:N runtime capped at $WORKERS workers would top out"
echo "near (${WORKERS} workers * ${DURATION}s / 2s) = $((WORKERS * DURATION / 2)) kidnappers — but that is NOT CC."
echo "CC's V2 sysmon does syscall-age eviction + orphan-and-replace: a worker"
echo "pinned in a blocking syscall is detached off-pool and a fresh worker takes"
echo "its slot, so CC drains all $KIDNAPPERS like the 1:1 runtimes. The $((WORKERS * DURATION / 2)) figure is"
echo "the baseline CC's promotion beats; read the 'Kidnappers Completed' column."
echo "================================================================="
