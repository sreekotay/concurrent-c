#!/usr/bin/env bash
# compare_wstore5.sh — wstore3 vs wstore5 vs Go death-time twin (wstore3.go)
#
# Standing cell (quote this; do not invent another command):
#   ./perf/compare_wstore5.sh stand
# get N/N, 5s, θ=0.35, wave=512, repeats=3. Override REPEATS / seconds after stand.
#
# Default without stand is the tax smoke: write 1/1 × {cc3, cc5, go} × sweep {off, on}, 1s.
#
#   ./perf/compare_wstore5.sh stand
#   ./perf/compare_wstore5.sh stand 5
#   REPEATS=1 ./perf/compare_wstore5.sh stand
#   ./perf/compare_wstore5.sh [seconds]
set -euo pipefail
...
mkdir -p "$OUT"
JOBLIST="$OUT/wstore5_this_run.jobs"
ORDER="$OUT/wstore5_this_run.order"

echo "# compare_wstore5 seconds=$SEC ncpu=$NCPU mixes=$MIXES shapes=$SHAPES repeats=$REPEATS FREE=${FREE:-1} GOMAXPROCS=${GOMAXPROCS:-} CC_WORKERS=${CC_WORKERS:-}"
echo "# build"
...
run() {
    ...
    "$bin" "$SEC" "$sweep" 0.35 512 "$shards" "$clients" "$mix" \
        > "$OUT/$rec"
    grep -E '^# (ops=|expire:|index:|compact:|keys:|waves=|mem:|hitch |wstore3|wstore5|wstore3-go)' "$OUT/$rec"
}
...
pat = re.compile(r"^(cc3|cc5|go)_(write|get|drain)_s([01])_(\d+)x(\d+)(?:_r(\d+))?\.txt$")
...
print(f"# {'mix':<6} {'shape':<6} {'3 off':>7} {'3 on':>7} {'3 tax':>7} "
      f"{'5 off':>7} {'5 on':>7} {'5 tax':>7} {'on 5/3':>8} "
...
print(f"# {'mix':<6} {'shape':<6} {'sweep':<5} "
      ...
      f"{'5 idle':>8} {'5 end':>8} {'5 rss':>8} {'5 keys':>8}")
for mix, sh, cl in order:
    for sweep, label in (("0", "off"), ("1", "on")):
        a = rows.get((mix, sh, cl, "cc3", sweep))
        b = rows.get((mix, sh, cl, "cc5", sweep))
        if not a or not b:
...
print()
print(f"# {'mix':<6} {'shape':<6} {'5 off':>7} {'5 on':>7} {'5 tax':>7} "
      f"{'Go off':>7} {'Go on':>7} {'Go tax':>7} {'on 5/Go':>8} "
      f"{'hit%':>5} {'expire':>7} {'waves':>6}")
for mix, sh, cl in order:
    def getg(lang, sweep):
        return rows.get((mix, sh, cl, lang, sweep))
    b0, b1 = getg("cc5", "0"), getg("cc5", "1")
    g0, g1 = getg("go", "0"), getg("go", "1")
    if not all((b0, b1, g0, g1)):
        missing += 1
        print(f"# {mix:<6} {sh + '/' + cl:<6}  (incomplete)")
        continue
    btax = tax(b0, b1)
    gtax = tax(g0, g1)
    ratio = b1["mops"] / g1["mops"] if g1["mops"] else 0.0
    expire = fmt_drop(b1["dropped"])
    mark = "" if b1["ok"] and g1["ok"] else " !dropped!=drained"
    print(f"# {mix:<6} {sh + '/' + cl:<6} {b0['mops']:7.2f} {b1['mops']:7.2f} "
          f"{fmt_tax(btax):>7} {g0['mops']:7.2f} {g1['mops']:7.2f} "
          f"{fmt_tax(gtax):>7} {ratio:8.2f} {b1['hit']:5.0f} {expire:>7} "
          f"{b1['waves']:6.0f}{mark}")
    if nrep > 1:
        print(f"# {'':<6} {'':<6}  [{b0['mops_lo']:.2f}-{b0['mops_hi']:.2f}] "
              f"[{b1['mops_lo']:.2f}-{b1['mops_hi']:.2f}]         "
              f"[{g0['mops_lo']:.2f}-{g0['mops_hi']:.2f}] "
              f"[{g1['mops_lo']:.2f}-{g1['mops_hi']:.2f}]")

print()
print(f"# {'mix':<6} {'shape':<6} {'sweep':<5} "
      f"{'5 idle':>8} {'5 end':>8} {'5 rss':>8} {'5 keys':>8} "
      f"{'Go idle':>8} {'Go end':>8} {'Go rss':>8} {'Go keys':>8}")
for mix, sh, cl in order:
    for sweep, label in (("0", "off"), ("1", "on")):
        b = rows.get((mix, sh, cl, "cc5", sweep))
        ...
        if b["live"] is not None and b["mapn"] is not None and b["live"] != b["mapn"]:
            mark += " !5 live!=map"
        if g["live"] is not None and g["mapn"] is not None and g["live"] != g["mapn"]:
            mark += " !go live!=map"
        print(f"# {mix:<6} {sh + '/' + cl:<6} {label:<5} "
              f"{fmt_kib(b['idle_c']):>8} {fmt_kib(b['end_c']):>8} {fmt_kib(b['end_r']):>8} "
              f"{fmt_keys(b['live']):>8} "
              f"{fmt_kib(g['idle_c']):>8} {fmt_kib(g['end_c']):>8} {fmt_kib(g['end_r']):>8} "
              f"{fmt_keys(g['live']):>8}{mark}")

print()
print(f"# {'mix':<6} {'shape':<6} {'sweep':<5} "
      f"{'5 trim':>8} {'5 t-rss':>8} {'5 B/k':>7} "
      f"{'Go trim':>8} {'Go t-rss':>8} {'Go B/k':>7}")
for mix, sh, cl in order:
    for sweep, label in (("0", "off"), ("1", "on")):
        b = rows.get((mix, sh, cl, "cc5", sweep))
        g = rows.get((mix, sh, cl, "go", sweep))
        ...
        if b["trim_live"] is not None and b["trim_map"] is not None and b["trim_live"] != b["trim_map"]:
            mark += " !5 live!=map"
        if g["trim_live"] is not None and g["trim_map"] is not None and g["trim_live"] != g["trim_map"]:
            mark += " !go live!=map"
        print(f"# {mix:<6} {sh + '/' + cl:<6} {label:<5} "
              f"{fmt_keys(b['trim_live']):>8} {fmt_kib(b['trim_r']):>8} "
              f"{fmt_bkey(b_per_key(b['trim_r'], b['trim_live'])):>7} "
              f"{fmt_keys(g['trim_live']):>8} {fmt_kib(g['trim_r']):>8} "
              f"{fmt_bkey(b_per_key(g['trim_r'], g['trim_live'])):>7}{mark}")

print("# ops table and mem table are one result. Quote both.")
print("# 3 = wstore3 (in-place same-gen). 5 = wstore5 (always-new write).")
print("# Go = wstore3.go death-time twin (not a 5 twin).")
print("# tax = (sweep-on − sweep-off) / sweep-off. on 5/3 and on 5/Go = sweep-on ratio.")
print("# expire = wstore5 sweep-on dropped (must equal drained_recs).")
print("# mem idle = post-init, before preload/clients. CC commit = arena; "
      "Go commit = HeapSys. keys = mix-end live_n. trim = expire to 1M after mix.")
if nrep > 1:
