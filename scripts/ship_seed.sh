...
# The module product is what a link would lower: the face with its members
# spliced in, as one source unit. An emit-only run refreshes the faces
# under $HDR_SRC but not that product, so it is asked for by name.
ts "emit the module product"
"$CCC" --no-cache --emit-module-c "$ROOT/cc/lower/lower.cch" -o "$LATEST/lower_cch.c"
rm -f "$LATEST/lower_cch.c.key" "$LATEST/lower_cch.c.lock"
[[ -s "$LATEST/lower_cch.c" ]] || { echo "error: empty module product for lower.cch" >&2; exit 1; }
