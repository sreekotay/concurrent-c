#!/usr/bin/env bash
# shadow_lower architectural gate (cc/shadow shadow lowerer).
#
...
if ! ./bin/c_pp_stage_spike_smoke; then
  echo "[test_shadow] FAIL: c_pp_stage_spike_smoke"
  exit 1
fi
...
if ! ./bin/c_pp_shadow_emit_smoke; then
  echo "[test_shadow] FAIL: c_pp_shadow_emit_smoke"
  exit 1
fi

bash scripts/test_shadow_recipes.sh

# Product build + cache smoke (succession path).
SHADOW_BIN="$ROOT/out/cc/bin/shadow_lower"
if [[ ! -x "$SHADOW_BIN" ]]; then
  echo "[test_shadow] FAIL: missing native shadow_lower at $SHADOW_BIN"
  exit 1
fi
SMOKE_BIN="${TMPDIR:-/tmp}/cc_shadow_build_smoke_$$"
CACHE_WORK="${TMPDIR:-/tmp}/cc_shadow_cache_deps_$$"
PROD_C="${TMPDIR:-/tmp}/cc_shadow_prod_generics_$$.c"
trap 'rm -rf "$SMOKE_BIN" "$CACHE_WORK" "$PROD_C"' EXIT
if ! "$SHADOW_BIN" examples/hello.ccs -o "$SMOKE_BIN"; then
  echo "[test_shadow] FAIL: shadow_lower host build"
  exit 1
fi
if ! "$SHADOW_BIN" examples/hello.ccs -o "$SMOKE_BIN"; then
  echo "[test_shadow] FAIL: shadow_lower cached rebuild"
  exit 1
fi
out="$("$SMOKE_BIN")"
if ! grep -Fq "All tasks completed." <<<"$out"; then
  echo "[test_shadow] FAIL: shadow-built hello missing expected output"
  exit 1
fi
echo "[test_shadow] shadow_lower host build + cache OK"

# Warm emit must miss when a quoted transitive include changes.
mkdir -p "$CACHE_WORK"
cp cc/shadow/shadow/cache_warm_main.ccs "$CACHE_WORK/main.ccs"
...
if ! "$SHADOW_BIN" --no-cache "$CACHE_WORK/main.ccs" -o "$CACHE_BIN"; then
  echo "[test_shadow] FAIL: cache_warm cold build"
  exit 1
fi
out1="$("$CACHE_BIN")"
if [[ "$out1" != "marker=1" ]]; then
  echo "[test_shadow] FAIL: cache_warm unexpected out1: $out1"
  exit 1
fi
# Warm rebuild with unchanged deps (must stay green).
if ! "$SHADOW_BIN" "$CACHE_WORK/main.ccs" -o "$CACHE_BIN"; then
  echo "[test_shadow] FAIL: cache_warm warm rebuild (unchanged)"
  exit 1
fi
printf '%s\n' 'static int cache_warm_marker(void) { return 42; }' \
  >"$CACHE_WORK/cache_warm_dep.inc"
if ! "$SHADOW_BIN" "$CACHE_WORK/main.ccs" -o "$CACHE_BIN"; then
  echo "[test_shadow] FAIL: cache_warm rebuild after include edit"
  exit 1
fi
out2="$("$CACHE_BIN")"
if [[ "$out2" != "marker=42" ]]; then
  echo "[test_shadow] FAIL: warm cache stale after include edit (got: $out2)"
  exit 1
fi
echo "[test_shadow] shadow_lower transitive-dep invalidation OK"
...
GOLDEN_PROD="$ROOT/cc/shadow/shadow/recipe_user_generics.product.c.golden"
if ! "$SHADOW_BIN" examples/recipe_user_generics.ccs -o "$PROD_C"; then
  echo "[test_shadow] FAIL: product lower recipe_user_generics"
  exit 1
fi
...
PY
then
  echo "[test_shadow] FAIL: recipe_user_generics product golden mismatch"
  echo "  regenerate: $SHADOW_BIN examples/recipe_user_generics.ccs -o $GOLDEN_PROD"
  exit 1
fi
if ! grep -Fq '/* generic Pair_int_double */' "$PROD_C"; then
  echo "[test_shadow] FAIL: product emit missing factory splice marker"
  exit 1
fi
echo "[test_shadow] product comptime golden OK"

# ccc --frontend=native delegates to native shadow_lower.
FRONT_BIN="${TMPDIR:-/tmp}/cc_shadow_frontend_smoke_$$"
if ! "$ROOT/out/cc/bin/ccc" --frontend=native examples/hello.ccs -o "$FRONT_BIN"; then
  echo "[test_shadow] FAIL: ccc --frontend=native"
  rm -f "$FRONT_BIN"
  exit 1
fi
fout="$("$FRONT_BIN")"
...
if ! grep -Fq "All tasks completed." <<<"$fout"; then
  echo "[test_shadow] FAIL: --frontend=native hello missing expected output"
  exit 1
fi
echo "[test_shadow] ccc --frontend=native OK"
