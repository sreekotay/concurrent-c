...
for a in "$@"; do
  case "$a" in
    ...
    --legacy|--compare-front)
      echo "[test] $a removed: ccc lowers with the clean lowerer" >&2
      exit 2
      ;;
    *) args+=("$a") ;;
  esac
...
if [ "$has_filter" = 1 ]; then
  ...
  # Host-cc command lines past any fixed buffer still name the right output.
  if ! sh scripts/test_long_link_line.sh; then
