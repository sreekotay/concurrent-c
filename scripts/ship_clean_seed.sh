#!/usr/bin/env bash
# Freeze the clean lowerer's tools as a bootstrap seed.
#
#   ./scripts/ship_clean_seed.sh              # snapshot → cc/bootstrap/clean/latest/
#   ./scripts/ship_clean_seed.sh --promote    # …then promote to <base>-N, flip last-good
#
...
# zero of `make -C cc` host-compiles it with no lowerer in hand, the way
# the shadow seed is built; `make -C cc lower-cc` then rebuilds the tools
# from their sources with the seeded ones, and scripts/lowerer_selfhost.sh
# says whether the two agree. Source of truth is cc/lower/*.cch and *.ccs;
# a pin is regenerate-only.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BOOT="$ROOT/cc/bootstrap/clean"
LATEST="$BOOT/latest"
CCC="$ROOT/cc/bin/ccc"
# The clean lowerer's own root: the module product and faces it lowered,
# which the driver compiles a clean unit against (out/include holds the
# other lowerer's).
...
ts() { echo "[clean-seed] $(date '+%H:%M:%S') $*"; }
rm -rf "$LATEST"
mkdir -p "$LATEST"
t0=$(date +%s)
for t in $TOOLS; do
  ts "emit $t"
  CC_LOWERER=clean "$CCC" --lowerer=clean --no-cache --emit-c-only "$ROOT/cc/lower/$t.ccs" -o "$LATEST/$t.c"
...
for p in sorted(latest.glob("*.c")) + sorted(latest.glob("*.h")):
    ...
    if new != text:
        p.write_text(new)
        n += 1
print(f"[clean-seed] rewrote {n} files")
left = []
for p in sorted(latest.glob("*.c")) + sorted(latest.glob("*.h")):
...
echo "  git add cc/bootstrap/clean/last-good cc/bootstrap/clean/${VERSION_BASE}-$N"
