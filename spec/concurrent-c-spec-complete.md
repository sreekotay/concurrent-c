...
Inside a dest-bound body, assigning a by-value name, taking its address, or using it as the receiver of a UFCS call that lowers to `&name` is ill-formed: the copy is not the frame's object. The diagnostic names the two intents — *shared*: take `T* p = &x;` in the frame and pass `p`; *moved*: stop naming `x` in the frame and drop its frame-side destroy. *Status: draft — the diagnostic is not implemented; the lowerer currently accepts such names (frame reference for an assigned or address-taken name, snapshot for a UFCS receiver).*
