...
| `CCParallel h = @parallel { … } !>;` | Dest bind: starts the arms, does not join. Arms are fibers of `h`; `h.wait()` joins and returns the dest error. | `CCParallel h = @parallel spawn { produce(tx); } !>; … h.wait() !>;` |
| `@parallel(h) { … }` | Dest-attach: admit one more fiber onto live `h`. Statement, no `!>`. Body errors resolve inside the body or are recorded with `h.fail(e)`. | `@parallel(g_clients) { serve(fd) !>(e) { g_clients.fail(e); return; }; }` |
...
ok_yield      ::= '@ok' '(' expr ')' ';'                   // only inside a bang_expr body: the value the unwrap yields
...
- `CALL !> DIVERGENT_STMT;` and `CALL !> { …; DIVERGENT_STMT }` *(expression)* — Evaluate `CALL` exactly once. On success, the surrounding expression's value is the unwrapped `T`. On error, `DIVERGENT_STMT` (or the block) runs; because it cannot fall through, the surrounding expression has no observable value on that path. `!>(e) …` binds the error to `e` across the body. `@ok(V);` counts as divergent: it ends the body, and the surrounding expression's value on that path is `V`, converted to `T` as an assignment would. `T x = CALL !> { …; @ok(V); }` is `T x = CALL ?> V;` with the statements before `@ok` run first on the error path. A body that reaches its end on some path without diverging is ill-formed, `@ok` on the other paths notwithstanding.
- `@ok(V);` — Inside a `!> BODY` or `!>(e) BODY` at expression position, including the assignment and declaration forms. Ends the body with `V` as the unwrap's value; control continues after the unwrap. `V` is a pure C expression. Outside such a body — at statement position `CALL !>;`, `CALL !> BODY` with no value to receive, in an `@errhandler` body, or anywhere else — `@ok` is ill-formed.
...
- `@errhandler(E e) STMT;` / `@errhandler(E e) { BODY }` — Registers a block-local handler for Result error type `E`. `CALL !>;` at statement position, `@err(e);` forwards, and `CALL !>;` at expression position dispatch with the two-pass rule of invariant 4. Once `F` is chosen, the binder is projected along that unique path (member selection; `@typeview` `as:` declaration order applies only here, not to handler selection). A macro callee's `E` is the error the functions its expansion can call answer with; when those answer with several, the unwrap dispatches on the error that arrives, and every such type must have a handler in scope. When the unwrap's error type cannot be resolved as a Result `E` at all (pointer-returning calls and other untyped LHS forms), dispatch matches `@errhandler(CCError …)` — the same ambient error type those binders use. Subject to the divergence rule of invariant 6. Stdlib helpers: `cc_error_log(e)` (report, returns) and `cc_error_exit(e)` (report then `exit(1)`).
...
// Checkpoints (a mark on the host; a child arena only once scratch outgrows the slab)
typedef struct CCArenaCheckpoint CCArenaCheckpoint;   // { child, parent, offset }
CCArenaCheckpoint cc_arena_checkpoint(CCArena a);           // C twin (@scratch); .arena == NULL: dead host
...
A checkpoint is a **mark** on the innermost active host: the slab word at
capture, a fresh epoch, and the head of the host's record list. Allocation
keeps bumping the host's own slab; every allocation above the mark carries
the mark's epoch. `cc_arena_restore` runs the records attached since the
mark, then returns the slab word to the mark in one CAS: scratch above the
...
(a regrow that would cross the mark), when parent-side storage is needed
(a record for a durable object, the reuse class table), or when marks nest
deeper than the stack. The range from the outermost mark to the tip becomes
the child's L1 in place, the host's tip parks at the slab end with the child
counted once, the inner marks and the records attached since the outermost
mark move to the child, and the child becomes active:

- Fresh allocation through `a` (and through anything that allocates from
  it: containers, owners, the local paths) lands in the innermost active
  child and carries the child's epoch. Views minted there go stale at
  restore.
- Realloc and release act on the host that owns the pointer: a pre-mark
  object regrows in host storage (a fresh host slab when the tip is
  parked), never into scratch.
- `cc_arena_release` through `a` reaches a pointer owned by `a` or by any
  active child below it.
- A checkpoint taken through `a` nests inside the innermost active child.
- `cc_arena_detach` and `cc_arena_adopt` refuse a host with an armed mark or
  an active child, and refuse an active child itself.
- `cc_arena_reset` and `cc_arena_free` tear down marks and active children
  with everything else.

Restore of a promoted mark frees the child (its extents, overflow, and
epoch) and pops the host's tip back to the mark. Restore is LIFO for
*armed* handles: it refuses while an inner armed mark is live. Abandon
disarms the mark and keeps its scratch, which dies with the enclosing
restore or the host. A checkpoint always arms on a live host; a hard cap
(`CC_ARENA_FIXED` without overflow) fails scratch closed once its tail is
gone. Attach records, holes, and overflow never refuse a capture or a
restore.

`cc_arena_restore` returns false and does not mutate on a dead, consumed, or abandoned
...
The form is selected by the tokens after `@parallel`: `{` (always try to spawn; spawned arms may be denied), `spawn` then `{` (meeting admit: spawned arms are not denied, §8.11.7), `(dest) { … }` with no `!>` (admit onto dest `dest`; a statement, not a Result; `dest` is a `CCParallel` name or one field hop — `h`, `zip.app`, `srv->app`), `(` or `seq (` (spawn if the predicate, §8.11.3, §8.11.5), `wait (` (ordered spawn loop over a turnstile; an expression of type `bool !>(CCError)`, §8.11.6), or `@for` (bisected range, §8.11.4). `spawn` is a brace join; combining it with `for` or `wait` is ill-formed. Assignment join and `@parallel for` are `CCParallel !>(CCError)`. A statement consumes the Result and waits (`!>.wait()!>;`) or binds the handle (`CCParallel h = … !>;`). A bare construct is an unconsumed Result. `!>;` with neither a dest bind nor `.wait()` unwraps the Result and discards the handle. The wait-for form is `bool !>(CCError)`.

`@parallel(h) { stmts }` admits a fiber onto live dest `h`. `h` is a `CCParallel` name or one field hop (`zip.app`, `srv->app`). It is the growing form of dest: the cell arrives after the brace. It is a statement: no `!>`, no dest bind. Spawned work is not denied. A later write of a snapshot name in the caller is not this fiber's object (accept `sock`, loop `x`). Pointer and array names copy the pointer. An atomic name is the caller's cell. Occupancy is who is still running. A finished admit is dropped when the fiber is done; the live index grows if it must (OOM is a programming error). `CC_PARALLEL_TASK_MAX` is brace width and the dest's inline pad, not a cap on this form. `h.wait()` joins the dest's arms and whoever is still admitted. Admit is safe from any fiber, concurrently with other admits and with `h.wait()`: the runtime serializes the dest's live index. Admit after join or leave is a programming error. Admit after cancel fails (`CC_ERR_CANCELLED`); dest-attach unwraps into the enclosing `@errhandler`. The same tokens with `!>` are the predicate join (§8.11.3).

**Dest bodies.** The body of a dest-attach, and every arm of a dest-bound construct (`CCParallel h = @parallel { … } !>;`), is a fiber of the dest. It runs after the statement that started it; the frame that wrote it has moved on. Control therefore does not leave the body: `!>` in a dest body resolves to a handler inside the body — an `@errhandler` at the top of the body, or an attached `!>(e) { … }` — and a `!>` with no such handler is ill-formed. `return;` ends the fiber; a valued `return` is ill-formed. `break`, `continue`, and `goto` that would leave the body are ill-formed. The dest's own name is nameable in the body and is the caller's dest: `h.cancel()`, `h.pause()`, `h.resume()`, `h.fail(e)`. The first arm of a dest-bound construct runs on the caller (§8.11.1); the same rules apply to it, and its `return;` ends that arm and continues with the plant.

`h.fail(e)` is UFCS for `cc_parallel_fail(h, e)`: it records `e` as the dest's error and is `bool` — `true` when this call recorded it, `false` when the dest already holds an error, is idle, or is joined. First error wins; the payload is the first `e`. `fail` does not cancel and does not stop admits: a body that wants both writes both (`h.fail(e); h.cancel() !>;`). `h.wait()` joins everything, then returns `err(e)` when the dest holds an error and `ok` otherwise. A dest that is never failed waits `ok` regardless of what its bodies logged. A body that neither handles its error nor records it is the silent path; the language does not supply it. Whether `fail` is the right spelling depends on what the dest is: a batch dest ("run this set, join, take the first error") fails; an accept dest whose bodies are independent clients handles inside the body and does not fail the server on one bad client.

**Captures.** A body or arm may use locals of the enclosing frame. How a name arrives follows from whether the body can outlive the frame that spelled it.

*Frame-bound bodies* — the arms of an immediate-wait join (`@parallel { … }`, `!>.wait()!>`) and the body of `@parallel wait for` — end before the statement does, so every name is the frame object, by reference. A write is visible to the frame after the join and to concurrently running arms or iterations, subject to the construct's independence and ordering rules. The loop index of a `for` form is a per-iteration value, not a capture.

*Dest-bound bodies* — the body of a dest-attach and every arm of a dest-bound construct (`CCParallel h = @parallel { … } !>;`) — may run after the admitting frame has returned, so they are closures and follow the closure capture model (§2.2): a pointer-typed name copies the pointer and uses through it hit the pointee; an array name copies the decayed element pointer; a plain value copies; an owner — a type with a `.destroy` hook (`CCSocket`, `CCListener`, `CCSignal`, `CCString`) — moves into the body, which destroys it, and the frame does not name it again. Two names are identity, never copies: an atomic name is the caller's cell, and a `CCParallel` name is the caller's dest. A pointer capture does not require the pointer slot to outlive the body, only the pointee; a dest-live handle does not extend any frame object's storage.

Inside a dest-bound body, assigning a by-value name, taking its address, or using it as the receiver of a UFCS call that lowers to `&name` is ill-formed: the copy is not the frame's object. The diagnostic names the two intents — *shared*: take `T* p = &x;` in the frame and pass `p`; *moved*: stop naming `x` in the frame and drop its frame-side destroy. The assignment-arm LHS of a dest-bound construct (`a = expr`) and a `@serial` arm's one outer write are the construct's out slot, not this rule.
...
In an immediate-wait join (`!>.wait()!>`), a `return` in any arm joins every spawned sibling, then returns from the function. The construct does not wait for a later or earlier arm that has not returned — including an arm that never will (an external hang). If two arms both `return`, which value is taken is not specified. On the sequential denial (`seq` false, or `#pragma(@parallel) off`) it is a normal C `return`: later arms do not run. Likewise an unhandled `!>` in an arm of an immediate-wait join raises into the frame's handler after the join. That raise is a `CCError`, whatever the arm unwrapped: the join is a rendezvous, not an operation of its own, and what it reports is that an arm failed. A `CCError` the arm raises goes across as it stands and a `CCIoError` through its `base`, so the frame needs an `@errhandler(CCError)` for the record to reach, whatever else it declares. A Result of any other error type has no handler in the arm and is handled there (`!>(e) { … }`). In a dest-bound construct the arms are fibers of the dest and neither of these leaves the arm (dest bodies, §8.11).
...
An assignment or `@serial` arm after the first is spawned as a fiber and attached to the dest; `.wait()` joins. `@parallel for` spawns one half of a span at each bisection; a span of length 0 or 1 is a sequential `for`. If spawn fails or the adaptive gate refuses the spawn (task kind INVALID), that arm or leftover span runs on the caller. Denied work still runs. An assignment-only immediate-wait join (no `return` or `goto` in an arm, no dest) does not emit an exit cell: a denied spawn is those assignments on the caller. The construct does not estimate how much work an arm contains. A caller who knows a cutoff writes it on the join (`@parallel (d < k) { … }`) so the same arms run in parallel above the cut and in order below it.
...
4. Inject default handlers **inside** synthetic `main` and each `@task`
   body, one per error type a statement-level `!>` can raise there:
   `@errhandler(CCError)`, `@errhandler(CCIoError)` and
   `@errhandler(CCPrintError)`, each printing the error's text to stderr and
   returning `1`, so statement-level `!>` works without a local handler.
   Dispatch is type-matched (§3.1): a user `@errhandler` for one of these
   types in the same scope overrides the default for that type and leaves
   the others. The injection is a declaration at the top of that body, so
   invariant 4 reads it as it reads a written one: it is in scope for the
   unwraps that follow it in the same body, and a `.ccs` unit, a file
   scope, and a function the injection does not name have no handler from
   it.
...
struct CCArenaSlab {
    ...
    size_t    tail_carved; // where a promoted child's region begins (== capacity when none)
...
**Checkpoint/Restore:** A checkpoint is a mark on the innermost active host (slab word, epoch, record-list head). Restore runs the records attached since the mark and returns the slab word to the mark in one CAS; the tip never pops below a mark. Promotion turns the marks into a child host only when scratch outgrows the slab, spills, needs parent-side storage, moves a pre-mark object, or nests deeper than the mark stack: the range above the outermost mark becomes the child's L1 in place, the host tip parks at the slab end, and the child becomes active. Restore of a promoted mark frees the child and pops the host tip. LIFO holds for armed handles; abandon disarms and keeps the scratch.
