...
#
# The header is tracked and regenerated by make whenever this script or the
# core is newer, so an edit to the header alone does not survive a fresh
# checkout: edit the core or this script, then regenerate. An optional
# argument names another output path (the suite generates to a temp file
# and compares).
set -e
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CORE="$ROOT/cc/include/ccc/cc_emit_tpl_core.inc.cch"
OUT="${1:-$ROOT/cc/src/comptime/emit_tpl_prelude.inc.h}"
