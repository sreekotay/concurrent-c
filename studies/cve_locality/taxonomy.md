...
| **T9 Wire length vs buffer** | Attacker length > buffer; over-read (Heartbleed-shaped); size wrap → tiny alloc; OOB index write | Schema `bytes len`; `cc_*_i64_checked`; Result `at`/`set` (all builds) | Hand `memcpy(dst, p, wire_len)`; bare `size_t a+b`; raw `ptr[i]=` past `len`. Cross-parser HTTP policy is out of corpus |
