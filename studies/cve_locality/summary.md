...
**Reading:** Both CC and Rust win by staying on their surface. **Prevented** = surface plus an enforceable rule (checker / schema). **Mitigated** = idiomatic structure dissolves the bug class without yet making the C-shaped rewrite ill-formed. The only `still_expressible` entry is FFI wrong deleter — Rust parity.
...
| Nursery-only task stop/join | CVE-2024-38561 | would flip T6 toward prevented |
...
|-----------|-------------|
| T3 residual (non-arena send) | fuller Send lattice (honest miss candidate) |
| Default-on strict Result unwrap | would harden SHAPE-T5 Gap |
| Nursery-only task stop | would flip CVE-2024-38561 toward prevented |
