...
/* Type hooks for declaration lifecycle and declarative UFCS lowering.
 ...
 int cc_symbols_set_type_cast_call(CCSymbolTable* t, const char* type_name, const char* callee);
 int cc_symbols_lookup_type_cast_call(CCSymbolTable* t, const char* type_name, const char** out_callee);
/* Extent hooks: kind is "field"|"call" for .len, "load"|"call" for .access. */
