 ...
 * all `@comptime` constructs (blocks / for / if) so whitelist parse + field
 * harvest can populate `cc_ct_field_reg_*` before comptime exec. Returns NULL
 * (skip type pass) when the harvested TU has no `@comptime`, or has
 * `@comptime` but no field-reflection markers (`type_of` /
...
/* Field registry (libshadow_comptime) — filled during type-pass harvest. */
...
/* Type-pass src was produced but parse/harvest failed — distinct from skipped. */
