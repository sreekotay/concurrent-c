...
static const char* cc__ufcs_canonicalize_type_alias(const char* type_name) {
...
static void cc__ufcs_normalize_decl_type(char* out, size_t out_sz, const char* type_name) {
    ...
    type_name = cc__ufcs_canonicalize_type_alias(type_name);
...
static const char* cc__ufcs_lookup_scoped_local_var_type(const char* src,
    ...
    {
        const char* canon = cc__ufcs_canonicalize_type_alias(out_type);
        if (canon != out_type) {
...
static int cc__emit_registered_callable(char* out,
    ...
    /* CCResult_<T>_<E> methods (value/error/is_ok/is_err/unwrap/unwrap_or/
     * unwrap_err) are generated by the `CC_DECL_RESULT_SPEC` macro and
     * never appear literally in the pre-expansion source, so the generic
     * "PascalCase in source" fallback below can't detect them.  Defer to
     * the family dispatcher unconditionally for these. */
    if (lowered_len > 0 && strncmp(recv_type_name, "CCResult_", 9) == 0 &&
        (strcmp(method, "value") == 0 ||
         strcmp(method, "error") == 0 ||
         strcmp(method, "is_ok") == 0 ||
         strcmp(method, "is_err") == 0 ||
         strcmp(method, "unwrap") == 0 ||
         strcmp(method, "unwrap_err") == 0 ||
    ...
    if (g_ufcs_source_text && lowered_len > 0) {
        ...
        if (tlen > 0 && tlen + 1 + strlen(method) + 1 < sizeof(type_method_candidate)) {
            ...
            if (getenv("CC_DEBUG_COMPTIME_UFCS")) {
                fprintf(stderr, "CC_DEBUG_COMPTIME_UFCS: fallback-check lowered=%s candidate=%s snake_in_src=%d pascal_in_src=%d\n",
                        lowered_name, type_method_candidate,
...
static void cc__resolve_dispatch_ctx(CCUFCSDispatchCtx* ctx, const char* recv) {
    ...
    if (reg) {
        ...
        if (ctx->recv_type_name) {
            ctx->recv_type_name = cc__ufcs_canonicalize_type_alias(ctx->recv_type_name);
        }
...
static int emit_desugared_call(char* out,
    ...
    if ((cc__is_string_recv_type(ctx.recv_type_name) || cc__is_string_recv_type(ctx.recv_family_type)) &&
        ...
        if (!has_args || !args_rewritten) {
            return recv_is_ptr ? snprintf(out, cap, "cc_string_push(%s, cc_slice_empty(), NULL)", recv)
                               : snprintf(out, cap, "cc_string_push(&%s, cc_slice_empty(), NULL)", recv);
        }
    ...
    if ((cc__is_string_recv_type(ctx.recv_type_name) || cc__is_string_recv_type(ctx.recv_family_type)) &&
        ...
        if (!has_args || !args_rewritten) {
            return recv_is_ptr ? snprintf(out, cap, "cc_string_cstr(%s, NULL)", recv)
                               : snprintf(out, cap, "cc_string_cstr(&%s, NULL)", recv);
        }
...
static char* cc__rewrite_nested_ufcs_args(const char* args_src) {
    ...
    if (!out) return NULL;
    /* The outer call's receiver-type hints (`g_ufcs_recv_type`,
     * `g_ufcs_recv_type_is_ptr`) apply only to the *outer* receiver, not to
     * any UFCS calls nested inside the argument list.  When
     * `cc__resolve_dispatch_ctx` processes a nested call it would otherwise
     * see the outer type as the default and skip the local-scope / type-
     * registry resolution that turns `key_res.value()` (receiver of type
     * `CCResult_CCSlice_CCError`) into the PascalCase `CCResult_T_E_value`
     * fallback.  Save / clear / restore around the recursion so the nested
     * rewrite rediscovers each receiver's type from scope, identical to
     * what a top-level `cc_ufcs_rewrite_line` call would see. */
    const char* saved_recv_type = g_ufcs_recv_type;
    int saved_recv_type_is_ptr = g_ufcs_recv_type_is_ptr;
    g_ufcs_recv_type = NULL;
    g_ufcs_recv_type_is_ptr = 0;
    int rc = cc_ufcs_rewrite_line(args_src, out, out_cap);
    g_ufcs_recv_type = saved_recv_type;
    g_ufcs_recv_type_is_ptr = saved_recv_type_is_ptr;
    if (rc != 0) {
