...
/* Arena-specific helper: find substring in range */
...
/* Arena-specific helper: find matching closing brace */
    ...
    *out_len = 0;
    ...
    for (int ei = 0; ei < edit_n; ei++) {
        ...
        for (int j = 0; j < root->node_count; j++) {
            ...
            if (!cc_pass_node_in_tu(root, ctx, n[j].file)) continue;
...
int cc__collect_arena_edits(const CCASTRoot* root,
    ...
    /* Check for @arena_init with await (hard error) */
    for (int ei = 0; ei < edit_n; ei++) {
        const ArenaEdit* e = &edits[ei];
        if (!(e->size_expr && strncmp(e->size_expr, "@buf:", 5) == 0)) continue;
        ...
        for (int j = 0; j < root->node_count; j++) {
            ...
            if (aw_off > e->brace_off && aw_off < e->close_off) {
                ...
                int aw_col = n[j].col_start > 0 ? n[j].col_start : 1;
                fprintf(stderr,
                        "%s:%d:%d: error: CC: @arena_init(buf, size) block cannot contain 'await'\n"
                        "%s:%d:%d: note: 'await' occurs here\n",
                        ctx && ctx->input_path ? ctx->input_path : "<input>",
                        n[e->node_idx].line_start > 0 ? n[e->node_idx].line_start : 1,
                        col1,
    ...
    /* Add edits to EditBuffer */
    int edits_added = 0;
    for (int ei = 0; ei < edit_n; ei++) {
        ArenaEdit e = edits[ei];
        if (e.close_off >= in_len || e.brace_off >= in_len || e.start_off >= in_len) continue;
        if (!(e.start_off < e.brace_off && e.brace_off < e.close_off)) continue;

        const char* indent = in_src + e.indent_off;
        size_t indent_len = e.indent_len;
        if (e.indent_off + indent_len > in_len) { indent = ""; indent_len = 0; }        char pro[512];
        char epi[256];
        int pn = 0;
        int en = 0;
        ...
        if (e.size_expr && strncmp(e.size_expr, "@buf:", 5) == 0) {
            ...
            pn = snprintf(pro, sizeof(pro),
                          ...
                          (int)indent_len, indent, e.id,
                          (int)indent_len, indent, e.id, buf_expr, size_expr,
                          (int)indent_len, indent, e.name, e.id);            en = snprintf(epi, sizeof(epi),
                          "%.*s  /* arena %s uses user buffer - no cleanup */\n",
                          (int)indent_len, indent, e.name);
        } else {
            pn = snprintf(pro, sizeof(pro),
        ...
        if (pn <= 0 || (size_t)pn >= sizeof(pro)) continue;
        if (en <= 0 || (size_t)en >= sizeof(epi)) continue;        /* Add prologue: replace [start_off, brace_off+1) */
        if (cc_edit_buffer_add(eb, e.start_off, e.brace_off + 1, pro, 10, "arena") == 0) {
            edits_added++;
        }

        /* Add epilogue: insert before close_off */
        if (cc_edit_buffer_add(eb, e.close_off, e.close_off, epi, 10, "arena") == 0) {
            edits_added++;
        }
    }
