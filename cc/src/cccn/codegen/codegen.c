...
static void emit_closure_defs(const CCNFile* file, FILE* out) {
    ...
    for (int i = 0; i < file->closure_count; i++) {
        ...
        if (has_captures) {
            ...
            /* Emit local variables for captured values - cast from intptr_t to int */
            for (int j = 0; j < def->captures.len; j++) {
                CCNNode* cap = def->captures.items[j];
                if (cap && cap->kind == CCN_EXPR_IDENT) {
                    fprintf(out, "  int %s = (int)__env->%s;\n", cap->as.expr_ident.name, cap->as.expr_ident.name);
                }
            }
        } else {
...
static void emit_node_ctx(const CCNNode* node, FILE* out, int indent, ClosureEmitCtx* ctx) {
    ...
    switch (node->kind) {
        ...
        case CCN_EXPR_IDENT: {
            ...
            if (param_idx >= 0) {
                fprintf(out, "(int)__arg%d", param_idx);
            } else {
        ...
        case CCN_INCLUDE: {
            /* Keep includes as-is (including .cch files) */
...
int cc_emit_c(const CCNFile* file, FILE* out) {
    ...
    /* Include stdlib for malloc/free (used by closures) */
    if (file->closure_count > 0) {
        fprintf(out, "#include <stdlib.h>\n");
        fprintf(out, "#include <stdint.h>\n\n");
        /* Generate inline closure infrastructure */
        fprintf(out, "/* Closure infrastructure */\n");
        fprintf(out, "typedef struct { void* (*fn)(void*); void* env; void (*drop)(void*); } CCClosure0;\n");
        fprintf(out, "typedef struct { void* (*fn)(void*, intptr_t); void* env; void (*drop)(void*); } CCClosure1;\n");
        fprintf(out, "typedef struct { void* (*fn)(void*, intptr_t, intptr_t); void* env; void (*drop)(void*); } CCClosure2;\n");
        fprintf(out, "static inline CCClosure0 cc_closure0_make(void* (*fn)(void*), void* env, void (*drop)(void*)) { CCClosure0 c = {fn, env, drop}; return c; }\n");
        fprintf(out, "static inline CCClosure1 cc_closure1_make(void* (*fn)(void*, intptr_t), void* env, void (*drop)(void*)) { CCClosure1 c = {fn, env, drop}; return c; }\n");
        fprintf(out, "static inline CCClosure2 cc_closure2_make(void* (*fn)(void*, intptr_t, intptr_t), void* env, void (*drop)(void*)) { CCClosure2 c = {fn, env, drop}; return c; }\n");
        fprintf(out, "static inline intptr_t cc_closure0_call(CCClosure0 c) { return (intptr_t)c.fn(c.env); }\n");
        fprintf(out, "static inline intptr_t cc_closure1_call(CCClosure1 c, intptr_t a0) { return (intptr_t)c.fn(c.env, a0); }\n");
        fprintf(out, "static inline intptr_t cc_closure2_call(CCClosure2 c, intptr_t a0, intptr_t a1) { return (intptr_t)c.fn(c.env, a0, a1); }\n\n");
    }
