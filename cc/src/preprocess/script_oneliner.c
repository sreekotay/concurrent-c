 ...
 * have to agree.
 *
 * Token-gated names: `arena`, `stdin`, `args` — no implication chain. */
#define CC_OL_PREDECL_ARENA "CCArena arena@(megabytes(1)) @destroy;\n"
#define CC_OL_PREDECL_STDIN                                                \
    "CCStdin __cc_stdin_src;\n"                                            \
    "CCArena __cc_stdin_arena@(megabytes(1)) @destroy;\n"                  \
    "BufReader::[CCStdin] stdin;\n"                                        \
    "cc_stdin_init(&__cc_stdin_src, __cc_stdin_arena);\n"                  \
    "stdin.init_arena(&__cc_stdin_src, __cc_stdin_arena, 4096);\n"
...
typedef struct {
    ...
    void* ctx;
    int recurse_holes; /* 1: predecls (${stdin} counts); 0: task_exists skip tpl */
} CCOlCodeWalk;
...
static void cc__ol_predecl_on_ident(const char* src, size_t len, size_t i,
    ...
    if (cc_match_ident_kw(src, len, i, "stdin")) {
        out->want_stdin = 1;
        return;
    }
    if (cc_match_ident_kw(src, len, i, "arena")) {
        out->want_arena = 1;
    }
}
...
static void cc__ol_decl_on_ident(const char* src, size_t len, size_t i,
    ...
    if (cc_match_ident_kw(src, len, i, "CCStdin")) {
        d->saw_CCStdin = 1;
        d->saw_CCArena = 0;
        d->saw_CCSlice = 0;
        d->saw_BufReader = 0;
    ...
    if (cc_match_ident_kw(src, len, i, "BufReader")) {
        d->saw_BufReader = 1;
        d->saw_CCArena = 0;
        d->saw_CCSlice = 0;
        d->saw_CCStdin = 0;
    ...
    if (d->saw_CCArena && cc_match_ident_kw(src, len, i, "arena")) {
        d->decl_arena = 1;
        d->saw_CCArena = 0;
        return;
    }
    if (cc_match_ident_kw(src, len, i, "stdin")) {
        ...
        if (j > 0 && (src[j - 1] == ']' || src[j - 1] == '>'))
            d->decl_stdin = 1;
        if (d->saw_BufReader || d->saw_CCStdin) d->decl_stdin = 1;
        d->saw_CCArena = 0;
        d->saw_CCSlice = 0;
        d->saw_CCStdin = 0;
        d->saw_BufReader = 0;
...
void cc_script_oneliner_suppress_existing_decls(const char* src, size_t len,
    ...
    walk.ctx = &d;
    cc__ol_walk_code(src, 0, len, &walk);
    if (d.decl_arena) p->want_arena = 0;
    if (d.decl_stdin) p->want_stdin = 0;
...
char* cc_script_oneliner_predecls_for(const char* src, size_t len,
    ...
    p.want_line = 0;
    p.want_nr = 0;
    if (p.want_arena) lines++;
    if (p.want_stdin) lines += 5;
    ...
    if (!out) return NULL;
    if (p.want_arena) {
        static const char s[] = CC_OL_PREDECL_ARENA;
        if (ind_len) {
    ...
    if (p.want_stdin) {
        static const char s[] = CC_OL_PREDECL_STDIN;
        if (ind_len) {
            memcpy(out + o, indent, ind_len);
            o += ind_len;
        }
...
static char* cc__ol_wrap_line_loop(const char* body, size_t body_len, int line_print,
    ...
    static const char head[] =
        ...
        "char[:] line;\n"
        "for (;;) {\n"
        "    bool __cc_more = stdin.read_line(&line) !>;\n"
        "    if (!__cc_more) break;\n"
        "    nr += 1;\n";
...
static size_t cc__ol_predecl_bytes(const CCScriptOnelinerPredecls* p) {
    ...
    if (!p) return 0;
    if (p->want_arena)
        n += sizeof(CC_OL_PREDECL_ARENA) - 1;
    if (p->want_stdin)
        n += sizeof(CC_OL_PREDECL_STDIN) - 1;
    if (p->want_args)
...
static size_t cc__ol_append_predecls(char* out, size_t o, size_t cap,
    ...
    if (p->want_arena || p->want_stdin || p->want_args) {
        if (o + 1 < cap) out[o] = '\n';
        o += 1;
    }
    return o;
