...
/* Deadline without a second mco_running(): use the fiber we already have. */
static inline CCDeadline* cc__deadline_hot(cc__fiber* fiber) {
    if (fiber && ((uintptr_t)fiber & 1)) {
        void* d = sched_v2_fiber_deadline_scope(
            (fiber_v2*)((uintptr_t)fiber & ~(uintptr_t)1));
        if (d) return (CCDeadline*)d;
    }
...
static inline void wake_batch_flush(void) {
    ...
    for (size_t i = 0; i < b->count; i++) {
        if (b->fibers[i]) {
            ...
            if (b->attribs[i] & CC_FIBER_UNPARK_ATTR_PREFER_LOCAL)
                cc__fiber_unpark_prefer_local(b->fibers[i]);
            else
...
typedef struct {
    ...
    int trace_recv_empty;
} cc__chan_wait_flag_ctx;

/* Park-hook flags. `reason` stays a dump label; do not strcmp it for control
 * flow. TRACE_EMPTY fires the buffered-empty recv trace. SPIN polls
 * `notified` a few times before committing to park (rendezvous). */
enum {
    CC__CHAN_WAIT_NONE = 0,
    CC__CHAN_WAIT_TRACE_EMPTY = 1 << 0,
    CC__CHAN_WAIT_SPIN = 1 << 1,
};

static inline void cc__chan_spin_hint(void);
static inline int cc__chan_rv_spin_count(void);
...
static void cc__chan_wait_flag_park(void* waitable, CCSchedFiber* fiber, void* io) {
    ...
    (void)io;
    cc__chan_wait_flag_ctx* ctx = (cc__chan_wait_flag_ctx*)waitable;
    if (ctx->trace_recv_empty && ctx->obj) {
...
static inline cc_sched_wait_result cc__chan_wait_notified_deadline(cc__fiber_wait_node* node,
    ...
    if (wait_flags & CC__CHAN_WAIT_SPIN) {
        int spin = cc__chan_rv_spin_count();
        while (spin-- > 0) {
            cc__chan_spin_hint();
            if (atomic_load_explicit(&node->notified, memory_order_acquire) != 0) {
                return CC_SCHED_WAIT_OK;
    ...
    cc__chan_wait_flag_ctx ctx = {
        ...
        .obj = obj,
        .node = node,
        .trace_recv_empty = (wait_flags & CC__CHAN_WAIT_TRACE_EMPTY) ? 1 : 0,
    };
...
static inline cc_sched_wait_result cc__chan_wait_notified_deadline_on(cc__fiber_wait_node* node,
                                                                      const struct timespec* abs_deadline,
                                                                      const char* reason,
                                                                      void* obj,
                                                                      int wait_flags,
    ...
    if (abs_deadline)
        return cc_sched_fiber_wait_until(&ctx, NULL, &ops, abs_deadline);
    if (fiber)
        return cc_sched_fiber_wait_on(fiber, &ctx, NULL, &ops);
...
static inline cc_sched_wait_result cc__chan_wait_notified(cc__fiber_wait_node* node, const char* reason, void* obj) {
    return cc__chan_wait_notified_deadline(node, NULL, reason, obj, CC__CHAN_WAIT_NONE);
}

static inline cc_sched_wait_result cc__chan_wait_notified_mark_close_on(cc__fiber_wait_node* node,
                                                                        const struct timespec* abs_deadline,
                                                                        const char* reason,
                                                                        void* obj,
                                                                        int wait_flags,
                                                                        cc__fiber* fiber) {
    cc_parallel_abort_if_denied_chan(reason);
    cc_sched_wait_result wait_rc = cc__chan_wait_notified_deadline_on(
        node, abs_deadline, reason, obj, wait_flags, (CCSchedFiber*)fiber);
...
static inline cc_sched_wait_result cc__chan_wait_notified_mark_close(cc__fiber_wait_node* node,
                                                                     ...
                                                                     void* obj,
                                                                     int wait_flags) {
    return cc__chan_wait_notified_mark_close_on(node, abs_deadline, reason, obj, wait_flags, NULL);
}
...
struct CCChan {
    ...
    void *buf;                 /* Published before the release of elem_size */
    size_t _Atomic elem_size;  /* 0 until buf is visible */
...
/* ensure_buf writes buf, then release-stores elem_size. The lock-free
 * path acquire-loads that size and must not read buf while it is still
 * zero — zero falls through to the mutex path, which calls ensure_buf.
 * The mutex does not cover the lock-free read. */
static inline size_t cc__chan_published_elem(const CCChan* ch) {
    return atomic_load_explicit(&ch->elem_size, memory_order_acquire);
}

static inline int cc__chan_lf_ready(CCChan* ch, size_t value_size) {
    size_t es = cc__chan_published_elem(ch);
    if (es == 0 || es != value_size) return 0;
    if (!ch->use_lockfree || ch->cap == 0 || !ch->buf) return 0;
    return ch->use_ring_queue || es <= sizeof(void*);
}


static inline uint32_t cc__chan_wake_sched_attrib(CCChan* ch) {
    uint32_t a = CC_FIBER_UNPARK_ATTR_NONE;
    if (ch && ch->cap == 0)
        a |= CC_FIBER_UNPARK_ATTR_PREFER_LOCAL;
    if (ch && cc__chan_fast_path_ok(ch))
        a |= CC_FIBER_UNPARK_ATTR_CONTENTION_LOCAL;
    return a;
}
...
#endif
/* Rendezvous spin-before-park. Default 0: SPSC measurements showed the
 * waiter arrived first and 32 yields never saw notified flip. Set
 * CC_CHAN_RV_SPIN to try a budget (arm64 yield ~1-2 cycles). */
#ifndef CC_CHAN_RV_SPIN_DEFAULT
#define CC_CHAN_RV_SPIN_DEFAULT 0
#endif

static _Atomic int g_cc_chan_lock_spin = -1;
static _Atomic int g_cc_chan_rv_spin = -1;
...
static inline int cc__chan_rv_spin_count(void) {
    int cached = atomic_load_explicit(&g_cc_chan_rv_spin, memory_order_relaxed);
    if (cached < 0) {
        const char* env = getenv("CC_CHAN_RV_SPIN");
        int v = CC_CHAN_RV_SPIN_DEFAULT;
        if (env && env[0]) {
            char* end = NULL;
            long parsed = strtol(env, &end, 10);
            if (end != env && parsed >= 0 && parsed <= 1024) v = (int)parsed;
        ...
        int expected = -1;
        (void)atomic_compare_exchange_strong_explicit(&g_cc_chan_rv_spin,
                                                      &expected, v,
                                                      memory_order_relaxed,
                                                      memory_order_relaxed);
        cached = atomic_load_explicit(&g_cc_chan_rv_spin, memory_order_relaxed);
    }
    return cached;
}
...
// Caller holds ch->mu. Publish buf, then release-store elem_size so a
// lock-free reader that acquire-loads a non-zero size sees the buffer.
static int cc_chan_ensure_buf(CCChan* ch, size_t elem_size) {
    size_t cur = atomic_load_explicit(&ch->elem_size, memory_order_relaxed);
    if (cur == 0) {
        void* buf;
        if (ch->use_lockfree && ch->cap > 0)
            buf = malloc(ch->lfqueue_cap * elem_size);
        else {
            size_t slots = (ch->cap == 0) ? 1 : ch->cap;
            buf = malloc(slots * elem_size);
        }
        if (!buf) return ENOMEM;
        ch->buf = buf;
        /* Brand before the release so an acquire of elem_size sees it. */
        cc__chan_set_fast_path_ok(ch,
                            cc__chan_minimal_path_enabled() &&
                            ch->use_lockfree && ch->cap > 0 &&
                            (elem_size <= sizeof(void*) || ch->use_ring_queue) &&
                            !ch->is_owned && !ch->is_ordered && !ch->is_sync);
        atomic_store_explicit(&ch->elem_size, elem_size, memory_order_release);
        return 0;
    }
    if (cur != elem_size) return EINVAL;
    return 0;
}
...
size_t cc_chan_elem_size(const CCChan* ch) {
    return ch ? cc__chan_published_elem(ch) : 0;
}
...
static int cc_chan_wait_full(CCChan* ch, const struct timespec* deadline) {
    ...
    if (ch->cap == 0) {
        if (fiber) {
            ...
            while (!cc__chan_closed(ch) && !ch->rx_error_closed && (ch->rv_has_value || (ch->rv_recv_waiters == 0 && !ch->recv_waiters_head))) {
                ...
                pthread_mutex_unlock(&ch->mu);
                cc_sched_wait_result wait_rc = cc__chan_wait_notified_mark_close(&node, deadline, "chan_send_wait_rendezvous", ch, CC__CHAN_WAIT_SPIN);
                cc_chan_lock(ch);
    ...
    if (fiber) {
        ...
        while (!cc__chan_closed(ch) && !ch->rx_error_closed && ch->count == ch->cap) {
            ...
            cc_sched_wait_result wait_rc = cc__chan_wait_notified_mark_close(&node, deadline, "chan_send_wait_full", ch, CC__CHAN_WAIT_NONE);
            cc_chan_lock(ch);
            if (wait_rc == CC_SCHED_WAIT_TIMEOUT) {
                cc__chan_remove_send_waiter(ch, &node);
                return ETIMEDOUT;
...
static int cc_chan_wait_empty(CCChan* ch, const struct timespec* deadline) {
    ...
    if (ch->cap == 0) {
        ...
        if (fiber) {
            ...
            while (!cc__chan_closed(ch) && !ch->rv_has_value) {
                ...
                cc_sched_wait_result wait_rc = cc__chan_wait_notified_mark_close(&node, deadline, "chan_recv_wait_rendezvous", ch, CC__CHAN_WAIT_SPIN);
    ...
    if (fiber) {
        ...
        while (!cc__chan_closed(ch) && ch->count == 0) {
            ...
            cc_sched_wait_result wait_rc = cc__chan_wait_notified_mark_close(&node, deadline, "chan_recv_wait_empty", ch, CC__CHAN_WAIT_TRACE_EMPTY);
            cc_chan_lock(ch);
...
static int cc__chan_try_enqueue_lockfree_impl(CCChan* ch, const void* value) {
    size_t es = cc__chan_published_elem(ch);
    if (!ch->use_lockfree || ch->cap == 0 || es == 0 || !ch->buf) return EAGAIN;
    if (!ch->use_ring_queue && es > sizeof(void*)) {
...
static int cc__chan_try_enqueue_into_lockfree_impl(CCChan* ch, CCClosure2 builder, CCArena arena, int* built) {
    size_t es = cc__chan_published_elem(ch);
    if (!ch->use_lockfree || ch->cap == 0 || es == 0 || !ch->buf) return EAGAIN;
    if (!ch->use_ring_queue && es > sizeof(void*)) return EAGAIN;
...
static int cc_chan_send_unbuffered(CCChan* ch, const void* value, const struct timespec* deadline,
    ...
    while (!cc__chan_closed(ch) && !ch->rx_error_closed) {
        ...
        /* No receiver; wait */
        cc_parallel_abort_if_denied_chan("chan_send_wait_rendezvous");
        cc__fiber_wait_node node = {0};
        ...
        while (!cc__chan_closed(ch) && !ch->rx_error_closed && !atomic_load_explicit(&node.notified, memory_order_acquire) && err == 0) {
            ...
            if (fiber && !deadline) {
                ...
                pthread_mutex_unlock(&ch->mu);
                if (!atomic_load_explicit(&node.notified, memory_order_acquire)) {
                    (void)cc__chan_wait_notified_mark_close_on(&node, NULL, "chan_send_wait_rendezvous", ch, CC__CHAN_WAIT_SPIN, fiber);
                }
...
static int cc_chan_recv_unbuffered(CCChan* ch, void* out_value, const struct timespec* deadline,
    ...
    while (!cc__chan_closed(ch)) {
        ...
        /* No sender; wait */
        cc_parallel_abort_if_denied_chan("chan_recv_wait_rendezvous");
        ch->rv_recv_waiters++;
        cc__fiber_wait_node node = {0};
        node.fiber = (fiber && !deadline) ? fiber : NULL;
        node.data = out_value;
        ...
        while (!cc__chan_closed(ch) && !atomic_load_explicit(&node.notified, memory_order_acquire) && err == 0) {
            ...
            if (fiber && !deadline) {
                pthread_mutex_unlock(&ch->mu);
                if (!atomic_load_explicit(&node.notified, memory_order_acquire)) {
                    (void)cc__chan_wait_notified_mark_close_on(&node, NULL, "chan_recv_wait_rendezvous", ch, CC__CHAN_WAIT_SPIN, fiber);
                }
...
/* Unbuffered send: skip branded-minimal, owned, lockfree, ensure_buf once
 * elem_size is set. One fiber lookup covers deadline and the wait path. */
static int cc_chan_send_cap0(CCChan* ch, const void* value, size_t value_size) {
    if (!ch || !value || value_size == 0) return EINVAL;
    if (ch->elem_size != 0 && ch->elem_size != value_size) return EINVAL;
    if (cc__chan_closed(ch)) return cc__chan_send_close_errno(ch);
    if (ch->rx_error_closed) return ch->rx_error_code;

    cc__fiber* fiber = cc__fiber_current();
    {
        CCDeadline* d = cc__deadline_hot(fiber);
        if (d)
    ...
    int err = cc_chan_send_unbuffered(ch, value, NULL, fiber);
    wake_batch_flush();
...
int cc_chan_send(CCChan* ch, const void* value, size_t value_size) {
    if (ch && ch->cap == 0 && !ch->is_owned)
        return cc_chan_send_cap0(ch, value, value_size);
    ...
    size_t pub = cc__chan_published_elem(ch);
    if (pub != 0 && cc__chan_fast_path_ok(ch) && value_size == pub) {
    ...
    if (cc__chan_lf_ready(ch, value_size)) {
    ...
    if (ch->cap == 0) {
        err = cc_chan_send_unbuffered(ch, value, NULL, NULL);
        wake_batch_flush();
        if (err == 0) cc__chan_signal_recv_ready(ch);
        return err;
    }
    ...
    if (ch->use_lockfree && (ch->use_ring_queue || ch->elem_size <= sizeof(void*))) {
        ...
        if (fiber) {
            ...
            chan_inflight_dec(ch);
                ...
                (void)cc__chan_wait_notified_mark_close(&node, NULL, "chan_send_wait_full", ch, CC__CHAN_WAIT_NONE);
                cc_chan_lock(ch);
                int notified = atomic_load_explicit(&node.notified, memory_order_acquire);
                if (notified == CC_CHAN_NOTIFY_SIGNAL) {
                    atomic_store_explicit(&node.notified, CC_CHAN_NOTIFY_NONE, memory_order_release);
...
static int cc_chan_recv_cap0(CCChan* ch, void* out_value, size_t value_size) {
    if (!ch || !out_value || value_size == 0) return EINVAL;
    if (ch->elem_size != 0 && ch->elem_size != value_size) return EINVAL;
    ...
    int err = cc_chan_recv_unbuffered(ch, out_value, NULL, fiber);
    wake_batch_flush();
    if (err == 0) cc__chan_signal_recv_ready(ch);
    return err;
}
...
int cc_chan_recv(CCChan* ch, void* out_value, size_t value_size) {
    if (ch && ch->cap == 0 && !ch->is_owned)
        return cc_chan_recv_cap0(ch, out_value, value_size);
    ...
    if (ch->cap == 0) {
        err = cc_chan_recv_unbuffered(ch, out_value, NULL, NULL);
        wake_batch_flush();
        if (err == 0) cc__chan_signal_recv_ready(ch);
        return err;
    }
...
int cc_chan_timed_send(CCChan* ch, const void* value, size_t value_size, const struct timespec* abs_deadline) {
    ...
    if (ch->cap == 0) {
        /* cc_chan_send_unbuffered owns the unlock on every return path. */
        err = cc_chan_send_unbuffered(ch, value, abs_deadline, NULL);
        wake_batch_flush();
        if (err == 0) cc__chan_signal_recv_ready(ch);
        return err;
    }
...
int cc_chan_timed_recv(CCChan* ch, void* out_value, size_t value_size, const struct timespec* abs_deadline) {
    ...
    if (ch->cap == 0) {
        /* cc_chan_recv_unbuffered owns the unlock on every return path. */
        err = cc_chan_recv_unbuffered(ch, out_value, abs_deadline, NULL);
        wake_batch_flush();
        return err;
    }
    ...
    if (rc == EAGAIN) {
        ...
        if (cc__fiber_in_context()) {
            ...
            if (ch->cap == 0) {
                ...
                cc_chan_lock(ch);
                err = f->is_send
                    ? cc_chan_send_unbuffered(ch, f->buf, p, NULL)
                    : cc_chan_recv_unbuffered(ch, f->buf, p, NULL);
            } else {
                err = f->is_send
