...
> `cc_preprocess_emit_splice`) and still never pre-expands — that desync is the
...
|-----------|-------------|
| **M0** | `PIPELINE.md`, `DIAG_AUDIT.md`, `PASS_INVENTORY` audit, `perf/compiler_baseline.txt`, `scripts/capture_baseline.sh`, orphan/unwired notes |
...
| **M3** | `cc_preprocess_canonicalize` + `cc_preprocess_emit_splice` split; deleted dead `cc_preprocess_for_initial_parse` / `cc_preprocess_for_reparse` wrappers and the phantom `cc_preprocess_for_light_reparse` entry |
...
|-------|--------|-------------|
| **M7.A** (pre-expand initial parse) | **Shipped** | `cc_cpp_expand()` runs TCC's CPP during initial parse so the prepended container/result-type `#include` lines resolve. GCC-style `# N "file" flags` markers normalized to bare C99 `#line` to prevent TCC's parser from re-triggering system-header inclusion. Originally opt-in via `CC_PRE_EXPAND=1`; collapsed to the default initial-parse path on 2026-05-29. |
...
| **M7.C2** caveat | Removed opt-in | `CC_PRE_EXPAND_REPARSE=1` used to run CPP over the FINAL reparse buffer (after emit-splice + `cc__prepend_reparse_prelude` + parser-helper rewrites — earlier placement caused `__mbstate_t` double-decl). It validated end-to-end pipeline plumbing but regressed 4 smoke tests (`async_chan_await_works_smoke`, `async_channel_typed_lowered_smoke`, `call_site_noblock_smoke`, `ufcs_nested_std_io_smoke`) because CPP-expanded reparse output changed AST shapes in ways that confused async-AST and a few UFCS passes. The env var was removed; real fix requires the M1 visitor swap. |
...
make perf-regress    # verify via tools/cc_perf_check.ccscript
```
