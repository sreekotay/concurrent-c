# Concurrent-C Legacy Compiler Architecture

> **Scope: legacy front only.** Default architecture:
> [`ARCHITECTURE.md`](ARCHITECTURE.md) (native / `shadow_lower`). This ADR
> describes the older multipass text-rewrite / TCC stub-AST pipeline selected
> with `--frontend=legacy` / `CC_FRONTEND=legacy`.

**Status:** ADR for the legacy opt-out front.
**Last updated:** 2026-05-28 (post fossil audit — see §6).
**Audience:** anyone maintaining or extending `--frontend=legacy`. Read this **before** proposing a redesign of that path.

This document records the *why* of the **legacy** pipeline shape. The shape itself is documented in [`PIPELINE.md`](../src/visitor/PIPELINE.md) (call-site map) and [`PASS_INVENTORY.md`](../src/visitor/PASS_INVENTORY.md) (per-pass catalog). Operational status — what shipped, what's next, what's deferred — lives in [`COMPILER_CLEANUP_STATUS.md`](COMPILER_CLEANUP_STATUS.md).
