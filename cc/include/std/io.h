...
typedef struct {
    CCSlice data;       // allocated in arena
    size_t bytes_read;  // number of bytes read into data
    int eof;            // non-zero if EOF reached
} CCFileReadResult;
...
// Result types used by this header.
CC_DECL_RESULT(CCResultSliceIoError, CCSlice, CCIoError)
CC_DECL_RESULT(CCResultSizeIoError, size_t, CCIoError)
CC_DECL_RESULT(CCResultFileReadIoError, CCFileReadResult, CCIoError)
...
// Read up to n bytes; eof is signaled via result.eof when 0 bytes read.
static inline CCResultFileReadIoError cc_file_read(CCFile *file, CCArena *arena, size_t n) {
    CCFileReadResult r = {0};
    if (!file || !file->handle || !arena || n == 0) {
        return cc_err_CCResultFileReadIoError(cc_io_from_errno(EINVAL));
    }
    char *buf = (char *)cc_arena_alloc(arena, n, sizeof(char));
    if (!buf) {
        return cc_err_CCResultFileReadIoError((CCIoError){CC_IO_OUT_OF_MEMORY, ENOMEM});
    }
    size_t got = fread(buf, 1, n, file->handle);
    if (got == 0) {
        if (feof(file->handle)) { r.eof = 1; return cc_ok_CCResultFileReadIoError(r); }
        if (ferror(file->handle)) return cc_err_CCResultFileReadIoError(cc_io_from_errno(errno));
    }
    r.data = cc_slice_from_parts(buf, got, CC_SLICE_ID_NONE, n);
    r.bytes_read = got;
    r.eof = feof(file->handle) ? 1 : 0;
    return cc_ok_CCResultFileReadIoError(r);
}
...
// Read one line (includes delimiter). EOF with no data -> eof=1 and empty slice.
static inline CCResultFileReadIoError cc_file_read_line(CCFile *file, CCArena *arena) {
    CCFileReadResult r = {0};
    if (!file || !file->handle || !arena) {
        return cc_err_CCResultFileReadIoError(cc_io_from_errno(EINVAL));
    }
    CCString line = cc_string_new(arena, 128);
    if (!line.ptr) return cc_err_CCResultFileReadIoError((CCIoError){CC_IO_OUT_OF_MEMORY, ENOMEM});
    int c;
    while ((c = fgetc(file->handle)) != EOF) {
        cc_string_append_char(arena, &line, (char)c);
        if (c == '\n') break;
    }
    if (line.len == 0 && c == EOF) {
        r.eof = 1; return cc_ok_CCResultFileReadIoError(r);
    }
    r.data = cc_string_as_slice(&line);
    r.bytes_read = line.len;
    r.eof = feof(file->handle) ? 1 : 0;
    return cc_ok_CCResultFileReadIoError(r);
}
...
static inline CCResultSizeIoError cc_file_sync(CCFile *file) {
    if (!file || !file->handle) return cc_err_CCResultSizeIoError(cc_io_from_errno(EINVAL));
    if (fflush(file->handle) != 0) return cc_err_CCResultSizeIoError(cc_io_from_errno(errno));
    return cc_ok_CCResultSizeIoError(0);
}

static inline CCResultSizeIoError cc_file_seek(CCFile *file, long offset, int whence) {
    if (!file || !file->handle) return cc_err_CCResultSizeIoError(cc_io_from_errno(EINVAL));
    if (fseek(file->handle, offset, whence) != 0) return cc_err_CCResultSizeIoError(cc_io_from_errno(errno));
    return cc_ok_CCResultSizeIoError(0);
}

static inline CCResultSizeIoError cc_file_tell(CCFile *file) {
    if (!file || !file->handle) return cc_err_CCResultSizeIoError(cc_io_from_errno(EINVAL));
    long pos = ftell(file->handle);
    if (pos < 0) return cc_err_CCResultSizeIoError(cc_io_from_errno(errno));
    return cc_ok_CCResultSizeIoError((size_t)pos);
}
...
static inline CCSlice cc_path_join(CCArena *arena, CCSlice a, CCSlice b) {
    ...
    buf[off] = '\0';
    return cc_slice_from_parts(buf, off, CC_SLICE_ID_NONE, off + 1);
}
...
static inline CCSlice cc_path_dirname(CCArena *arena, CCSlice path) {
    ...
    memcpy(buf, p, i);
    buf[i] = '\0';
    return cc_slice_from_parts(buf, i, CC_SLICE_ID_NONE, i + 1);
}
...
static inline CCSlice cc_path_basename(CCArena *arena, CCSlice path) {
    ...
    buf[out_len] = '\0';
    return cc_slice_from_parts(buf, out_len, CC_SLICE_ID_NONE, out_len + 1);
}
...
static inline CCResultFileReadIoError cc_buf_reader_next(CCBufReader *r, size_t n) {
    CCFileReadResult res = {0};
    if (!r || !r->file || !r->file->handle || n == 0) return cc_err_CCResultFileReadIoError(cc_io_from_errno(EINVAL));
    if (r->eof) { res.eof = 1; return cc_ok_CCResultFileReadIoError(res); }
    if (n > r->cap) n = r->cap;
    size_t got = fread(r->buf, 1, n, r->file->handle);
    if (got == 0) {
        if (feof(r->file->handle)) { r->eof = 1; res.eof = 1; return cc_ok_CCResultFileReadIoError(res); }
        if (ferror(r->file->handle)) return cc_err_CCResultFileReadIoError(cc_io_from_errno(errno));
    }
    r->pos = 0;
    r->len = got;
    res.data = cc_slice_from_parts(r->buf, got, CC_SLICE_ID_NONE, r->cap);
    res.bytes_read = got;
    res.eof = feof(r->file->handle) ? 1 : 0;
    if (res.eof) r->eof = 1;
    return cc_ok_CCResultFileReadIoError(res);
}

static inline CCResultFileReadIoError cc_buf_reader_read_line(CCBufReader *r, CCArena *arena) {
    CCFileReadResult res = {0};
    if (!r || !r->file || !r->file->handle || !arena) return cc_err_CCResultFileReadIoError(cc_io_from_errno(EINVAL));
    CCString line = cc_string_new(arena, 128);
    if (!line.ptr) return cc_err_CCResultFileReadIoError((CCIoError){CC_IO_OUT_OF_MEMORY, ENOMEM});
    while (1) {
        if (r->pos >= r->len) {
            CCResultFileReadIoError fill = cc_buf_reader_next(r, r->cap);
            if (fill.is_err) return fill;
            if (fill.ok.eof && fill.ok.bytes_read == 0) { res.eof = 1; return cc_ok_CCResultFileReadIoError(res); }
        }
        if (r->pos >= r->len) break;
        char c = r->buf[r->pos++];
        cc_string_append_char(arena, &line, c);
        if (c == '\n') break;
    }
    res.data = cc_string_as_slice(&line);
    res.bytes_read = line.len;
    res.eof = r->eof;
    return cc_ok_CCResultFileReadIoError(res);
}
...
static inline CCResultSizeIoError cc_buf_writer_flush(CCBufWriter *w) {
    if (!w || !w->file || !w->file->handle) return cc_err_CCResultSizeIoError(cc_io_from_errno(EINVAL));
    size_t written = fwrite(w->buf, 1, w->len, w->file->handle);
    if (written != w->len) {
        if (ferror(w->file->handle)) return cc_err_CCResultSizeIoError(cc_io_from_errno(errno));
    }
    w->len = 0;
    return cc_ok_CCResultSizeIoError(written);
}

static inline CCResultSizeIoError cc_buf_writer_write(CCBufWriter *w, CCSlice data) {
    if (!w || !w->buf || !w->file || !w->file->handle) return cc_err_CCResultSizeIoError(cc_io_from_errno(EINVAL));
    size_t off = 0;
    while (off < data.len) {
        size_t space = w->cap - w->len;
        if (space == 0) {
            CCResultSizeIoError fl = cc_buf_writer_flush(w);
            if (fl.is_err) return fl;
            space = w->cap;
        }
    ...
    return cc_ok_CCResultSizeIoError(data.len);
}
