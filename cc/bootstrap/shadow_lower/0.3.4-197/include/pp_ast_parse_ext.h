        ...
        for (j = p->i; j < close; j++) {
            if (tok_eq(p->toks[j], TK_PUNCT, "{") ||
                ...
                d++;
                     ...
                     tok_eq(p->toks[j], TK_PUNCT, "]")) {
                if (d > 0) d--;
            } else if (d == 0 && tok_eq(p->toks[j], TK_PUNCT, ","))
                nested = 1;
            if (tok_eq(p->toks[j], TK_PUNCT, "{")) nested = 1;
        }
        /* Nested struct/union and comma-declarators stay on the beachhead.
         * Flatten is one name per field and 512 bytes — do not chop `p, n`
         * or a grammar union. */
        if (close >= p->i && !nested && shadow_toks_are_c(p, p->i, close))
