...
static int shadow_attach_cparse_field(Parser* p, AstNode* st, const CpFlat* f) {
    ...
    if (f->kind == CP_FLAT_PPDIR) {
         ...
         * name (`size_t bstack[512]`), so keep the full declarator as raw. */
        nlen = strlen(f->name);
        tlen = strlen(n->a);
        if (nlen > 0 && tlen >= nlen &&
 ...
 * File-scope fns stay on the beachhead until cparse owns stmt AST.
 * Taping C-looking mains dropped `[~`, variant `.arm`, safety, and
 * CC diagnostics. */
