...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_AT_STMT:
        ...
        if (strcmp(st->a, "restricted") == 0 && st->b[0] && st->d[0]) {
            /* Erased view: typedef Base Base_Restrict_Mode;
             * Optional typedef+define: e = "Mangled#Alias" / "Mangled#*Alias". */
            char mangled[160];
            const char* hash;
    ...
    if (fn_cleanup) {
        if (is_void || !have_val) {
            if (!cemit_fmt(out, "%s%s__cc_ret_set = 1;\n", body_ind, extra))
                ...
                "%s__cc_ret_set = 1;\n"
                "%s%s",
                body_ind, set_retval_expr, body_ind, body_ind, extra)) {
            return 0;
        }
