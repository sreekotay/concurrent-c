...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
     ...
     * Also walks `args[i].data` (skip `[…]`) and variant `.framed` / `.u`
    ...
    if (vty[0] && recv) {
        ...
        for (;;) {
            ...
            int fptr = 0;
            ShadowVariant* vv;
            /* Skip `[index]` between hops (`args[i].data`). */
            if (*p == '[') {
                int depth = 0;
                do {
                    if (*p == '[') depth++;
     ...
     * rewrite `.` into a pointer call. */
    {
        ...
        /* Field hop in recv (`p->out.meth`, `reply->data.meth`): the method
         * operator applies to the field, not the outer pointer bind. */
