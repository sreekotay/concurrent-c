...
#define SHADOW_UFCS_PTR_RECV()                                                   \
    ...
    if (!is_arrow && strcmp(meth_name, "remaining") == 0) {
        ...
        /* Flipped script print: data.println() → slice/string methods.
         * char* / string lit are not a peer UFCS family — coerce to CCSlice
         * then cc_slice_* (one path). `io.println(x)` still prints args. */
