            ...
            if (pars[i]->e[0]) {
                ...
                if (!g_shadow_pw_sh_typed) {
                    if (!cemit_str(out,
                            ...
                            "CCNursery* nur; CCChan* work; } __cc_pw_sh_t;\n"
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (!cemit_fmt(out, "%s{\n", indent))
        goto fail;
    if (!cemit_fmt(out, "%sCCNursery* __cc_pw_n_%d = NULL;\n", i1, id))
        goto fail;
    if (st->f[0]) {
        if (!cemit_fmt(out, "%sif ((%s)) __cc_pw_n_%d = cc_nursery_create(NULL);\n",
                       i1, st->f, id))
            goto fail;
    } else if (!cemit_fmt(out, "%s__cc_pw_n_%d = cc_nursery_create(NULL);\n",
                          i1, id))
        goto fail;
