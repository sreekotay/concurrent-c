...
static int shadow_par_go_collect(AstNode* st, char lhs[][128],
    ...
    *ncaps = 0;
    ...
    for (a = 0; a < n; a++) {
        ...
        int nc = 0, i, k;
        if (!shadow_par_scan_text(st->body[a]->b, NULL, names, tys, &nc,
                                  SHADOW_PAR_CAP_MAX))
            return 0;
        for (i = 0; i < nc; i++) {
            int skip = 0, j;
...
static int shadow_emit_par_spawn_body(
    ...
    char cap_tys[][SHADOW_PAR_CAP_MAX][160], int via_outs) {
    int a;
    for (a = 1; a < n; a++) {
        int i;
        int id = base + a - 1;
...
static int shadow_emit_parallel(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    for (a = 0; a < n; a++) {
        ...
        if (!shadow_par_rewrite_rhs(arm, rhs[a], sizeof(rhs[a]))) return 0;
        ncaps[a] = 0;
        if (a > 0) {
            if (!shadow_par_scan_text(arm->b, NULL, cap_names[a], cap_tys[a],
            ...
            if (!shadow_par_emit_assign_thunk(base + a - 1, lhs_ty[a], rhs[a],
