...
static AstNode* parse_typed_init(Parser* p) {
    ...
    if (!had_cl) {
        ...
        if (c0 >= j) {
            FileTape* ft = p->cache ? tape_by_id(p->cache, name.file_id) : NULL;
            const char* path = (ft && ft->path) ? ft->path : "<input>";
            const char* tests = strstr(path, "tests/");
            int oline = 1, ocol = 1;
            if (ft && ft->bytes)
                offset_to_linecol(ft->bytes, ft->len, name.offset, &oline,
                                  &ocol);
            fprintf(stderr, "%s:%d: error: expression expected before ';'\n",
...
static int shadow_is_grammar_static_helper(CCSlice name) {
    ...
    buf[n] = 0;
    /* Internal matchers / fills: Name__r_*, Name__m_*, Name__fill, …
     * Require a leading type prefix — not libc/CC helpers like `__cc_*`. */
    if (buf[0] != '_' && strstr(buf, "__")) return 1;
