...
static int parse_call_arg_closure(Parser* p, int c0, int end, char* dst,
    ...
    }
    {
        ...
        if (!bare && dst[0])
            snprintf(n->c, sizeof(n->c), "%s", dst);
        else
            n->c[0] = 0;
        n->e[0] = 0;
        /* Optional explicit captures: `=> [rx, &tx] { … }` */
        if (tok_eq(p_peek(p), TK_PUNCT, "[")) {
            p_next(p);
            while (!tok_eq(p_peek(p), TK_PUNCT, "]") &&
                ...
                /* Init-capture: `p = &x` / `sp = sump` → store `p=&x`. */
                if (!is_ref && tok_eq(p_peek(p), TK_PUNCT, "=")) {
                    int e0, e1, depth;
                    p_next(p); /* = */
                    e0 = p->i;
                    ...
                    e1 = p->i;
                    {
                        ...
                        snprintf(item, sizeof(item), "%s=%s", name, init);
                    }
                } else if (is_ref) {
        ...
        if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
            ...
            }
            {
                ...
                if (lp && rp && rp > lp && n->nbody < 8) {
                    ...
                    if (!s) return 0;
                    snprintf(s->a, sizeof(s->a), "%s", expr);
                    n->body[n->nbody++] = s;
                } else {
...
/* Alias name of a caps item: "&x"→x, "p=&x"→p, "y"→y. */
...
static int spawn_stmt_declares(AstNode* s, const char* name) {
    ...
    if (s->kind == AST_RESULT_LOCAL && strcmp(s->a, name) == 0)
        return 1;
    /* `for (int i = 0; …)` — loop var is body-local. */
    if (s->kind == AST_FOR && s->a[0]) {
        const char* h = s->a;
        char id[64];
        size_t ni = 0;
        while (*h == ' ' || *h == '\t') h++;
...
static char g_spawn_globals[64][64];
static int g_spawn_nglobals;

static void spawn_note_global(const char* name) {
    int i;
    if (!name || !name[0] || g_spawn_nglobals >= 64) return;
...
static void spawn_infer_value_caps_stmt(AstNode* n, AstNode* s) {
    ...
    if (strcmp(n->b, "spawn") == 0 || strcmp(n->b, "spawnhybrid") == 0 ||
        strcmp(n->b, "spawnhybrid_unsafe") == 0 ||
        strcmp(n->b, "create") == 0 ||
        strcmp(n->b, "send_task") == 0 ||
        strncmp(n->b, "callarg", 7) == 0) {
        ...
        if (s->kind == AST_UFCS_STMT || s->kind == AST_UFCS_EXPR) {
            spawn_infer_caps_from_text(n, s->a);
            spawn_infer_caps_from_text(n, s->c);
        }
...
/* Walk body text for channel-shaped free names only (tx/rx/*_tx/*_rx). */
static void spawn_infer_chan_caps_stmt(AstNode* n, AstNode* s) {
    int k;
    if (!n || !s) return;
    spawn_infer_chan_caps_from_text(n, s->a);
    spawn_infer_chan_caps_from_text(n, s->b);
    spawn_infer_chan_caps_from_text(n, s->c);
    for (k = 0; k < s->nbody; k++) spawn_infer_chan_caps_stmt(n, s->body[k]);
    for (k = 0; k < s->ndbody; k++) spawn_infer_chan_caps_stmt(n, s->dbody[k]);
    if (s->kids) {
        for (k = 0; k < s->nkids; k++) spawn_infer_chan_caps_stmt(n, s->kids[k]);
    }
}
...
static void spawn_infer_value_caps(AstNode* n) {
    ...
    if (n->e[0]) {
        for (int k = 0; k < n->nbody; k++)
            spawn_infer_chan_caps_stmt(n, n->body[k]);
        return;
    }
    for (int k = 0; k < n->nbody; k++)
...
static AstNode* parse_spawn_closure_core(Parser* p) {
    ...
    if (!tok_eq(p->toks[p->i + 1], TK_PUNCT, "->") &&
        ...
        return NULL;
    {
        Token meth = p->toks[p->i + 2];
        int is_spawn = (shadow_kw(meth) == SHADOW_KW_SPAWN);
        int is_hybrid = (meth.kind == TK_IDENT &&
                         spell_eq(meth.spell, "spawnhybrid"));
        if (!is_spawn && !is_hybrid) return NULL;
    }
    if (!tok_eq(p->toks[p->i + 3], TK_PUNCT, "(")) return NULL;
    ...
    int is_unsafe = 0;
    int is_hybrid =
        (p->toks[p->i + 2].kind == TK_IDENT &&
         spell_eq(p->toks[p->i + 2].spell, "spawnhybrid"));
    ...
    if (is_hybrid)
        snprintf(n->b, sizeof(n->b), "%s",
                 is_unsafe ? "spawnhybrid_unsafe" : "spawnhybrid");
    else
        snprintf(n->b, sizeof(n->b), "%s",
                 is_unsafe ? "spawn_unsafe" : "spawn");
    n->e[0] = 0;
...
/* toks[c0..c1) starts with recv->spawn|spawnhybrid(() => …) — if/while cond.
 * Also accepts `.` UFCS spelling. On success: *out_sp set, *out_after = index
 * after spawn's `)`, returns 1. */
static int try_parse_spawn_cond(Parser* p, int c0, int c1, AstNode** out_sp,
    ...
    if (!tok_eq(p->toks[c0 + 1], TK_PUNCT, "->") &&
        !tok_eq(p->toks[c0 + 1], TK_PUNCT, "."))
        return 0;
    {
        Token meth = p->toks[c0 + 2];
        int is_spawn = (shadow_kw(meth) == SHADOW_KW_SPAWN);
        int is_hybrid = (meth.kind == TK_IDENT &&
                         spell_eq(meth.spell, "spawnhybrid"));
        if (!is_spawn && !is_hybrid) return 0;
    }
...
/* recv.send_task[|_hybrid](() => [caps] { stmts }|expr); — ordered-task beachhead. */
static AstNode* parse_send_task_closure(Parser* p) {
    ...
    if (!tok_eq(p->toks[p->i + 1], TK_PUNCT, ".")) return NULL;
    if (p->toks[p->i + 2].kind != TK_IDENT) return NULL;
    if (spell_eq(p->toks[p->i + 2].spell, "send_task_hybrid"))
        is_hybrid = 1;
    else if (!spell_eq(p->toks[p->i + 2].spell, "send_task"))
        return NULL;
    if (!tok_eq(p->toks[p->i + 3], TK_PUNCT, "(")) return NULL;
    ...
    slice_to(n->a, sizeof(n->a), recv.spell);
    snprintf(n->b, sizeof(n->b), "%s",
             is_hybrid ? "send_task_hybrid" : "send_task");
    n->e[0] = 0;
    ...
    if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
        ...
        /* `tx.send_task(() => [v] compute(v));` expression body. */
        int e0 = p->i;
        int depth = 1;
        while (p->i < p->n && depth > 0) {
            Token t = p_peek(p);
        ...
        }
        {
            char expr[256];
            if (!span_text(p, e0, p->i, expr, sizeof(expr))) {
                parser_fail(p, p_peek(p), "send_task expression body too long");
                return NULL;
            }
            const char* lp = strchr(expr, '(');
            const char* rp = expr[0] ? strrchr(expr, ')') : NULL;
