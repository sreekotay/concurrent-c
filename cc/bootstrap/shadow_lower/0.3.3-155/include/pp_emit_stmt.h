...
/* Env member list shared (textually) by the thunk and the spawn loop.
 * Fixed header first so the join walk can read next/i/failed/err through
 * a header-only view of the same layout. */
static int shadow_pw_emit_env_members(CEmit* dst, const char* pad,
    ...
    if (!cemit_fmt(dst,
            ...
            "%sint __cc_pw_i;\n"
            "%sint __cc_pw_failed;\n"
            "%sCCError __cc_pw_err;\n"
            "%sCCTurnstile* __cc_pw_ts;\n"
            "%sCCNursery* __cc_pw_n;\n",
 ...
 * ordered spawn loop. Denial (seq cond false, or nursery creation failure)
 * runs the same body as a plain sequential for. Errors are stop-starting:
 * the first failure (lowest ticket wins) cancels the nursery so no new
 * tickets enter, in-flight iterations finish (discharging enter/leave and
 * stage passes), and the error re-raises after the brace into the
 * innermost CCError @errhandler. */
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    snprintf(htext, sizeof(htext),
             "__cc_pw_env->__cc_pw_err = __cc_pw_eb; "
             "__cc_pw_env->__cc_pw_failed = 1; "
             "cc_nursery_cancel(__cc_pw_env->__cc_pw_n); "
    ...
    /* Thunk: unpack env, run body, then always leave() the turnstile. */
    if (!cemit_fmt(h,
            "\nstatic void* __cc_pw_thunk_%d(void* arg) {\n"
            "    struct {\n",
            id))
        goto fail;
    if (!shadow_pw_emit_env_members(h, "        ", st->a, names, tys, ncaps))
        goto fail;
    if (!cemit_fmt(h,
            "    } *__cc_pw_env = arg;\n"
            "    int %s = __cc_pw_env->__cc_pw_i;\n",
            st->a))
        goto fail;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(h, "    %s %s = __cc_pw_env->%s;\n", tys[k], names[k],
                       names[k]))
            goto fail;
    }
    if (!cemit_str(h, "    {\n")) goto fail;
    ...
    if (!shadow_emit_try_call(h, &tctx, "    ",
                              "cc_turnstile_leave(__cc_pw_env->__cc_pw_ts)",
    ...
    if (!cemit_fmt(out,
            "%s}\n"
            "%s} else {\n"
            "%sCCTurnstile* __cc_pw_ts_%d = %s;\n"
            "%svoid* __cc_pw_head_%d = NULL;\n"
            "%sCCError __cc_pw_ferr_%d;\n"
            "%sint __cc_pw_fail_%d = 0;\n"
            "%sint %s;\n"
            "%smemset(&__cc_pw_ferr_%d, 0, sizeof(__cc_pw_ferr_%d));\n"
            "%sfor (%s = (int)(%s); %s < (int)(%s); %s++) {\n",
            i2, i1, i2, id, tsexpr, i2, id, i2, id, i2, id, i2, st->a, i2, id,
            id, i2, st->a, lo, st->a, hi, st->a))
        return 0;
    if (!cemit_fmt(out, "%sstruct {\n", i3)) return 0;
    if (!shadow_pw_emit_env_members(out, i3, st->a, names, tys, ncaps))
        return 0;
    if (!cemit_fmt(out,
            "%s} *__cc_pw_e;\n"
            "%sif (cc_nursery_is_cancelled(__cc_pw_n_%d)) break;\n",
            i3, i3, id))
        return 0;
    /* enter(i) on the caller — in-order arming is what makes stage
     * wait/pass line up with tickets. A failed enter takes no token:
     * record + break, then join and re-raise. */
    snprintf(htext, sizeof(htext),
             "__cc_pw_ferr_%d = __cc_pw_eb; __cc_pw_fail_%d = 1; break;", id,
             id);
    ...
    if (!cemit_fmt(out,
            ...
            "sizeof(*__cc_pw_e), 16);\n"
            "%sif (!__cc_pw_e) {\n"
            "%s    fprintf(stderr, \"cc: @parallel wait: env alloc failed\\n\");\n"
            "%s    abort();\n"
            "%s}\n"
            "%s__cc_pw_e->__cc_pw_next = __cc_pw_head_%d;\n"
            "%s__cc_pw_e->__cc_pw_i = %s;\n"
            "%s__cc_pw_e->__cc_pw_failed = 0;\n"
            "%s__cc_pw_e->__cc_pw_ts = __cc_pw_ts_%d;\n"
            "%s__cc_pw_e->__cc_pw_n = __cc_pw_n_%d;\n",
            i3, id, i3, i3, i3, i3, i3, id, i3, st->a, i3, i3, id, i3, id))
        return 0;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(out, "%s__cc_pw_e->%s = %s;\n", i3, names[k], names[k]))
            return 0;
    }
    /* Spawn denial: run the same thunk inline so leave() and the error
     * capture happen exactly as on the spawned path. */
    if (!cemit_fmt(out,
            "%s__cc_pw_head_%d = __cc_pw_e;\n"
            "%sif (cc_nursery_spawn(__cc_pw_n_%d, __cc_pw_thunk_%d, "
            "__cc_pw_e) != 0)\n"
            "%s    (void)__cc_pw_thunk_%d(__cc_pw_e);\n"
            "%s}\n",
            i3, id, i3, id, id, i3, id, i2))
        return 0;
    /* Join. Spawn failures already ran inline; wait's join status has no
     * separate signal here. */
    if (!cemit_fmt(out,
            "%s(void)cc_nursery_wait(__cc_pw_n_%d);\n"
            "%s{\n"
            "%sstruct { void* __cc_pw_next; int __cc_pw_i; int __cc_pw_failed; "
            "CCError __cc_pw_err; } *__cc_pw_it = __cc_pw_head_%d;\n"
            "%swhile (__cc_pw_it) {\n"
            "%s    if (__cc_pw_it->__cc_pw_failed) {\n"
            "%s        __cc_pw_ferr_%d = __cc_pw_it->__cc_pw_err;\n"
            "%s        __cc_pw_fail_%d = 1;\n"
            "%s    }\n"
            "%s    __cc_pw_it = __cc_pw_it->__cc_pw_next;\n"
            "%s}\n"
            "%s}\n"
            "%scc_nursery_free(__cc_pw_n_%d);\n",
            i2, id, i2, i3, id, i3, i3, i3, id, i3, id, i3, i3, i3, i2, i2,
            id))
        return 0;
    /* Re-raise the first error (lowest ticket) into the real handler. */
    rbind = reh->b[0] ? reh->b : "e";
    if (!cemit_fmt(out,
            "%sif (__cc_pw_fail_%d) {\n"
            "%sCCError %s = __cc_pw_ferr_%d;\n",
            i2, id, i3, rbind, id))
        return 0;
    {
