...

    {
        ...
        while (p->i < name_i) p_next(p);
        {
            ...
            if (at_create) {
                ...
                p_next(p); /* ( */
                c0 = lp;
                had_cl = parse_call_arg_closure(p, c0, j, tmp, sizeof(tmp),
                                                "create", &create_cl);
                if (p->err) return NULL;
                int r;
                if (had_cl) {
                    ...
                    if (!ast_spell_token_range(p, c0, j, tmp, sizeof(tmp)) &&
                        !span_text(p, c0, j, tmp, sizeof(tmp))) {
                ...
                p_next(p); /* = */
                c0 = p->i;
                had_cl = parse_call_arg_closure(p, c0, j, init, sizeof(init),
                                                "create", &create_cl);
                if (p->err) return NULL;
                if (!had_cl) {
                    while (p->i < j) p_next(p);
            ...
            {
        AstNode* ue = (!had_cl) ? parse_ufcs_expr_range(p, at_create ? lp : c0, j) : NULL;
        p_next(p); /* @ */
