...
/* `@restricted(Mode) Base` → single IDENT `Base_Restrict_Mode` (type sugar).
 * Leaves `@restricted Mode on Base {…}` alone (no '(' after the keyword). */
static int shadow_rewrite_restricted_type_sugar(Token* toks, int* pn) {
    ...
    while (i + 5 < n) {
        ...
        if (!(tok_eq(at, TK_PUNCT, "@") && kw.kind == TK_IDENT &&
              tok_eq(kw, TK_IDENT, "restricted") && tok_eq(lp, TK_PUNCT, "(") &&
              mode.kind == TK_IDENT && tok_eq(rp, TK_PUNCT, ")") &&
              base.kind == TK_IDENT)) {
            i++;
            continue;
        }
