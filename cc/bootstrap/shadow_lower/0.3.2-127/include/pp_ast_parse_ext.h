...
static AstNode* parse_at_stmt(Parser* p) {
    ...
    /* `@restricted` / `@typeview` [Mode] on Base { r:…; w:…; rw:…; as:… }
     * Allow-list views + optional is-a faces (draft_facets / draft_as).
     * `@typeview` is the preferred spelling; `@restricted` remains an alias.
     * Unnamed `@typeview on Base` applies to Base itself (no parallel type). */
    if (spell_eq(name.spell, "restricted") ||
        spell_eq(name.spell, "typeview")) {
        Token mode, on, base;
        int unnamed = 0;
        int body0 = -1, body1 = -1;
        char body[1024];
        ...
        }
        {
            ...
            if (!n) return NULL;
            /* Canonical AST tag — dispatch still keys on "restricted". */
...
static AstNode* parse_typedef_restricted(Parser* p) {
    ...
    if (p->toks[p->i + 2].kind != TK_IDENT ||
        !(spell_eq(p->toks[p->i + 2].spell, "restricted") ||
          spell_eq(p->toks[p->i + 2].spell, "typeview")))
        return NULL;
