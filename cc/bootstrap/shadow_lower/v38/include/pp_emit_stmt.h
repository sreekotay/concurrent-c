...
/* Soft-return: restore open call-local @scratch cps, unwind nested
 * block/loop @defer/@destroy (gated by __cc_shw_*), then either goto fn
 * __cc_cleanup (fn-level life) or plain return.
...
static int shadow_emit_soft_goto_cleanup(CEmit* out, ShadowCtx* ctx,
    ...
    /* Scratch is lexically inside the call site — restore before scope life. */
    if (need_scratch) {
    ...
    if (fn_cleanup) {
        ...
        /* Scope / scratch unwind only — no fn epilogue. */
        if (is_void || !have_val) {
