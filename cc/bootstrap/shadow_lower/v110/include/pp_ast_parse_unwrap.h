...
static AstNode* parse_println_bang(Parser* p) {
    ...
    if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
        ...
        while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF && !p->err) {
            ...
            if (!st) return NULL;
            if (n->nbody >= SHADOW_BODY_CAP) { parser_fail_body_cap(p, p_peek(p), "!>{} body"); return NULL; }
            n->body[n->nbody++] = st;
        }
...
static AstNode* parse_stmt_unwrap(Parser* p) {
    ...
    char call[2048];
    lhs[0] = 0;
    /* `name = callee(args) !>` — assign-unwrap (parse_assign defers on `!>`). */
    if (p_peek(p).kind == TK_IDENT && p->i + 3 < p->n &&
        ...
        if (k >= p->n || !tok_eq(p->toks[k], TK_PUNCT, "!>")) return NULL;
        slice_to(lhs, sizeof(lhs), p_peek(p).spell);
        p_next(p); /* name */
        p_next(p); /* = */
    }
    callee = p_peek(p);
    if (callee.kind != TK_IDENT) {
        p->i = save;
        return NULL;
    }
    /* `name(args) !>` or `name::[T](args) !>` (generic free-name factories). */
    {
        int par = p->i + 1;
        if (par < p->n && tok_eq(p->toks[par], TK_PUNCT, "::")) {
            int gend = peek_generic_type_end(p, p->i);
            if (gend < 0 || gend >= p->n || !tok_eq(p->toks[gend], TK_PUNCT, "(")) {
                p->i = save;
                return NULL;
            }
            par = gend;
        } else if (par >= p->n || !tok_eq(p->toks[par], TK_PUNCT, "(")) {
            p->i = save;
            return NULL;
        }
        j = par;
    ...
    if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
        ...
        if (lhs[0]) snprintf(n->e, sizeof(n->e), "%s", lhs);
        while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF && !p->err) {
            AstNode* st = parse_stmt(p);
            if (!st) return NULL;
            if (n->nbody >= SHADOW_BODY_CAP) {
    ...
    if (!n) return NULL;
    snprintf(n->a, sizeof(n->a), "%s", call);
    /* Bare `x = call() !>;` must assign `__r.u.value` on success — same
     * emit path as `!>` with an empty body (`bang_stmt` + errhandler).
     * Leaving mode empty fell through to try_call and discarded Ok. */
    snprintf(n->c, sizeof(n->c), "bang_stmt");
    if (lhs[0]) snprintf(n->e, sizeof(n->e), "%s", lhs);
    return n;
}
...
static AstNode* parse_field_result(Parser* p) {
    ...
    if (!n) return NULL;
    snprintf(n->a, sizeof(n->a), "%s", rname);
...
static AstNode* parse_result_local(Parser* p) {
    ...
    }
    {
        ...
        if (had_cl) {
            snprintf(n->d, sizeof(n->d), "%s", init);
            if (cl) {
                if (n->ndbody >= SHADOW_DBODY_CAP) {
                    parser_fail(p, name,
                                "too many result-local closure attachments");
                    return NULL;
                }
            ...
            p->i = c1;
            if (!ast_spell_token_range(p, c0, c1, n->d, sizeof(n->d)) &&
                !span_text(p, c0, c1, n->d, sizeof(n->d))) {
...
static int parse_unwrap_ufcs_chain_tail(Parser* p, AstNode* n, Token at) {
    ...
    if (saved_ty[0])
        snprintf(n->c, sizeof(n->c), "bang_chain:%s", saved_ty);
    else
...
static AstNode* parse_var_unwrap(Parser* p) {
    ...
    /* `char[:] name = … !>` — slice surface (→ CCSlice). */
    if (tykw == SHADOW_KW_CHAR && p->i + 4 < p->n &&
        tok_eq(p->toks[p->i + 1], TK_PUNCT, "[") &&
        tok_eq(p->toks[p->i + 2], TK_PUNCT, ":") &&
        tok_eq(p->toks[p->i + 3], TK_PUNCT, "]"))
        is_char_slice = 1;
    if (tykw != SHADOW_KW_INT && tykw != SHADOW_KW_SIZE_T &&
        tykw != SHADOW_KW_BOOL && !is_named_ty && !is_char_slice)
        return NULL;
    ...
    if (is_char_slice) {
        ...
        if (ty1 < 0) ty1 = ty0 + 1;
        name_i = ty1;
        /* `T[:] name = … !>` — typed-slice sugar for any element type
         * (`double[:]` → CCSlice_double), the same surface params and
         * brace-literal decls already speak. */
        if (name_i + 3 < p->n &&
    ...
    char tytxt[128];
    if (is_char_slice) {
        while (p->i < name_i) p_next(p); /* char [ : ] */
        snprintf(tytxt, sizeof(tytxt), "CCSlice");
    } else if (is_slice_sugar) {
        char elem[96];
        size_t k;
             ...
             * Multi-declarator trailer `, b = 3;` parks in d (starts with ','). */
            {
                ...
                if (form_p) {
                    ...
                    if (tok_eq(pk, TK_PUNCT, ",")) {
                        ...
                        if (!span_text(p, t0, p->i, n->d, sizeof(n->d))) {
    ...
    if (tok_eq(p_peek(p), TK_PUNCT, "(") && p->i + 2 < p->n &&
        ...
        snprintf(n->c, sizeof(n->c), "qmark_bind:%s", tytxt);
    } else {
        snprintf(n->c, sizeof(n->c), "qmark");
        snprintf(n->d, sizeof(n->d), "%s", tytxt);
    }
    int d0 = p->i;
    depth = 0;
    while (p->i < p->n) {
...
static int parse_destroy_tail(Parser* p, AstNode* n, char* mode, size_t mode_cap) {
    ...
    while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF && !p->err) {
        ...
        if (!s) return 0;
        if (n->ndbody >= SHADOW_DBODY_CAP) {
            parser_fail_dbody_cap(p, p_peek(p), "@destroy body");
            return 0;
        }
        n->dbody[n->ndbody++] = s;
    }
...
static AstNode* parse_ptr_unwrap(Parser* p) {
    ...
    if (!n) return NULL;
    {
        ...
        stars[si] = 0;
        if (has_const)
            snprintf(n->a, sizeof(n->a), "const %s%s", tbuf, stars);
        else
            snprintf(n->a, sizeof(n->a), "%s%s", tbuf, stars);
    }
    slice_to(n->b, sizeof(n->b), name.spell);
