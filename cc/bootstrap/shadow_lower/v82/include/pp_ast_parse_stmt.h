...
static AstNode* parse_static_fn(Parser* p) {
    ...
    }
    {
        int j;
        for (j = ri0; j < ri; j++) {
            if (p->toks[j].kind == TK_IDENT &&
                spell_eq(p->toks[j].spell, "unused")) {
                has_unused = 1;
                break;
            }
        }
    ...
    }
    {
        ...
        if (has_inline && has_unused)
            snprintf(n->e, sizeof(n->e), "inline unused");
        else if (has_unused)
            snprintf(n->e, sizeof(n->e), "unused");
        else if (has_inline)
            snprintf(n->e, sizeof(n->e), "inline");
        if (p->pending_fn_attrs) {
