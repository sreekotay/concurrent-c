...
static AstNode* parse_typed_init(Parser* p) {
    ...
    /* `const T* name[N];` (no initializer), `static T a[N], b;`, or `T name = …;`. */
    int has_init = (ni < p->n && tok_eq(p->toks[ni], TK_PUNCT, "="));
    int multi_noinit = 0;
    int multi_end = -1;
    if (ni >= p->n) return NULL;
    if (!has_init && tok_eq(p->toks[ni], TK_PUNCT, ","))
        multi_noinit = 1;
    if (!has_init && !tok_eq(p->toks[ni], TK_PUNCT, ";") && !multi_noinit)
        return NULL;
    if (multi_noinit) {
        int end_i = name_i;
        int depth = 0;
        ...
        while (end_i < p->n && !tok_eq(p->toks[end_i], TK_PUNCT, ";")) {
            Token t = p->toks[end_i];
            if (tok_eq(t, TK_PUNCT, "!>") || tok_eq(t, TK_PUNCT, "?>"))
                return NULL;
            if (tok_eq(t, TK_PUNCT, "@") && end_i + 1 < p->n &&
                shadow_kw(p->toks[end_i + 1]) == SHADOW_KW_DESTROY)
                return NULL;
            end_i++;
        }
        ...
        k = name_i;
        while (k < end_i) {
            while (k < end_i && tok_eq(p->toks[k], TK_PUNCT, "*")) k++;
            if (k >= end_i || p->toks[k].kind != TK_IDENT) {
                parser_fail(p, k < p->n ? p->toks[k] : p_peek(p),
                            "expected declarator name after ','");
                return NULL;
            }
            k++;
            while (k < end_i && tok_eq(p->toks[k], TK_PUNCT, "[")) {
                depth = 1;
                k++;
                while (k < end_i && depth > 0) {
                    if (tok_eq(p->toks[k], TK_PUNCT, "[")) depth++;
                    else if (tok_eq(p->toks[k], TK_PUNCT, "]")) depth--;
                    k++;
                }
            }
            if (k < end_i && tok_eq(p->toks[k], TK_PUNCT, ",")) {
                k++;
                continue;
            }
            if (k != end_i) {
...
static int parser_stmt_storage_end(Parser* p) {
    ...
    while (i < p->n) {
        if (shadow_kw(p->toks[i]) == SHADOW_KW_STATIC ||
            shadow_kw(p->toks[i]) == SHADOW_KW_CONST)
            i++;
        else if (p->toks[i].kind == TK_IDENT &&
                 (spell_eq(p->toks[i].spell, "volatile") ||
                  spell_eq(p->toks[i].spell, "_Atomic")))
            i++;
        else
            break;
    }
