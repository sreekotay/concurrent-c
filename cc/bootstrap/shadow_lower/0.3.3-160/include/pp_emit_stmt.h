...
/* Collect @stage nodes in a wait-for body (order of appearance) and
 * number them (st->d) for the per-ticket passed flags. Nested @parallel
 * bodies own their stages. */
static void shadow_pw_collect_stages(AstNode* s, AstNode** out, int* n,
                                     int cap) {
    int k;
    if (!s || s->kind == AST_PARALLEL || s->kind == AST_PARALLEL_FOR) return;
    if (s->kind == AST_STAGE) {
        if (*n < cap) {
            snprintf(s->d, sizeof(s->d), "%d", *n);
            out[(*n)++] = s;
        }
    }
    for (k = 0; k < s->nbody; k++)
        shadow_pw_collect_stages(s->body[k], out, n, cap);
    for (k = 0; k < s->ndbody; k++)
        shadow_pw_collect_stages(s->dbody[k], out, n, cap);
}
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    for (k = 0; k < st->nbody; k++)
        shadow_pw_collect_stages(st->body[k], stages, &nstages,
