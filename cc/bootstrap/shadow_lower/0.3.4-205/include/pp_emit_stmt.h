...
static int shadow_emit_return_cc(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (strcmp(st->a, "ok") == 0) {
        ...
        if (ctx && ctx->pw_body == 2)
            return shadow_pw_emit_exit(out, ctx, indent, 2, assign);
...
static int shadow_par_emit_serial_stmts(AstNode* arm, CEmit* dest,
    ...
    if (!arm) return 0;
    if (ctx)
        ok = shadow_emit_compound_body(arm->body, arm->nbody, dest, ctx, indent,
                                       use_cleanup, 1);
    else {
        int k;
        ok = 1;
...
static int shadow_par_emit_assign_thunk(int id, const char* lhs_ty,
...
static int shadow_par_emit_serial_thunk(int id, const char* lhs_ty,
...
static int shadow_emit_par_spawn_body(
    ...
    if (st && st->body[0] && st->body[0]->kind == AST_SERIAL) {
        if (via_outs) {
            ...
            CEmit t = {0};
            if (!shadow_par_emit_serial_via(st->body[0], &t, ctx, bi, lhs[0],
                                            lhs_ty[0], "__cc_par_o_0",
                                            use_cleanup)) {
                free(t.buf);
                return 0;
            }
            ...
            free(t.buf);
        } else if (!shadow_par_emit_serial_block(st->body[0], dest, ctx, bi,
                                                 use_cleanup))
            return 0;
    } else if (via_outs) {
        if (!cemit_fmt(dest, "%s*__cc_par_o_0 = ", bi)) return 0;
...
static int shadow_emit_parallel(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    for (a = 0; a < n; a++) {
        ...
        if (a > 0 && !st->forced_seq) {
            ...
            if (serial) {
                if (!shadow_par_emit_serial_thunk(base + a - 1, lhs_ty[a],
                                                  lhs[a], arm, cap_names[a],
                                                  cap_tys[a], ncaps[a], ctx,
                                                  use_cleanup))
                    return 0;
            } else if (!shadow_par_emit_assign_thunk(base + a - 1, lhs_ty[a],
                                                     rhs[a], cap_names[a],
                                                     cap_tys[a], ncaps[a]))
                return 0;
        }
...
static int shadow_pw_push_loop(ShadowCtx* ctx, int is_switch) {
    if (!ctx || (ctx->pw_body != 1 && ctx->pw_body != 2)) return 0;
...
static int shadow_pw_emit_exit(CEmit* out, ShadowCtx* ctx, const char* indent,
    ...
    if (!cemit_fmt(out, "%s{\n", indent)) return 0;
    if (kind > 0) {
        if (!cemit_fmt(out, "%s    __cc_pw_e->__cc_pw_xk = %d;\n", indent,
                       kind))
            return 0;
        if (ctx->pw_ivar[0] &&
            !cemit_fmt(out, "%s    __cc_pw_e->__cc_pw_xi = %s;\n", indent,
                       ctx->pw_ivar))
            return 0;
        if (kind == 2 && g_shadow_pw_rv_ty[0] && rv && rv[0] &&
            !cemit_fmt(out, "%s    __cc_pw_e->__cc_pw_rv = (%s);\n", indent,
                       rv))
            return 0;
        if ((kind == 1 || kind == 2) &&
            !cemit_fmt(out,
                       "%s    cc_nursery_cancel(__cc_pw_sh->nur);\n", indent))
            return 0;
    }
...
/* Synthetic `gate.wait(args)` / `gate.pass(args)` UFCS statement for
 * @stage. A bare pointer gate calls through arrow; compound receivers
 ...
 * Inside a wait-for ticket the enclosing emitter also discharges the pass
 * on the error / continue path (flag per stage). Sequential denial uses
 * the same flags so `continue` still pass()es skipped stages. */
static int shadow_emit_stage(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (!ctx || !ctx->pw_body) {
        fprintf(stderr,
                "error: @stage is only valid inside a '@parallel wait for' "
                "body\n");
        out->err = 1;
        return 0;
    }
    ...
    if (!shadow_emit_stmt_ctx(&call, out, ctx, indent, use_cleanup)) return 0;
    if (st->d[0] &&
        !cemit_fmt(out, "%s__cc_pw_stg_%s = 1;\n", indent, st->d))
        return 0;
    return 1;
}
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    /* Error-path stage discharge: an errored ticket still passes every
     * stage it has not passed, so successor tickets are armed. Built in
     * the same bind window (gate UFCS + capture rewrite apply). */
    for (k = 0; k < nstages; k++) {
        AstNode pcall;
...
static int shadow_emit_parallel_for(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    for (k = 0; k < st->nbody; k++) {
        char saved = st->body[k]->indent[0];
        st->body[k]->indent[0] = 0;
        if (!shadow_emit_stmt_ctx(st->body[k], &body, ctx, "            ",
                                  use_cleanup)) {
            st->body[k]->indent[0] = saved;
            g_shadow_nbinds = mark;
            free(body.buf);
...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_RETURN_INT:
        if (ctx && ctx->pw_body == 2)
            return shadow_pw_emit_exit(out, ctx, indent, 2,
                                       st->a[0] ? st->a : "0");
    ...
    case AST_RETURN_EXPR: {
        ...
        if (ctx && ctx->pw_body == 2 && strcmp(st->e, "bang_block") != 0 &&
            strcmp(st->e, "bang_stmt") != 0)
            return shadow_pw_emit_exit(out, ctx, indent, 2, expr);
