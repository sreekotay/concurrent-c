...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (!cemit_fmt(out,
            ...
            "!__cc_pw_args_%d || !__cc_pw_cxok_%d) {\n"
    ...
    if (!cemit_fmt(out, "%sstruct {\n", i3))
        return 0;
    if (!shadow_pw_emit_env_members(out, i3, names, tys, ncaps))
        return 0;
    if (!cemit_fmt(out, "%s} __cc_pw_e;\n", i3))
        return 0;
    ...
    if (!cemit_fmt(out,
            "%sif (__cc_pw_fail_%d && (__cc_pw_e.__cc_pw_xk == 0 || "
            "__cc_pw_best_%d < __cc_pw_e.__cc_pw_xi)) {\n",
            i3, id, id))
        return 0;
    ...
    if (!cemit_fmt(out, "%s} else if (__cc_pw_e.__cc_pw_xk == 2) {\n", i3))
        return 0;
    if (g_shadow_pw_rv_ty[0]) {
        if (ctx->goto_cleanup || ctx->defer_cleanup ||
            (ctx->defer_depth > 0)) {
            if (!shadow_emit_soft_goto_cleanup(out, ctx, i4,
                                               "__cc_pw_e.__cc_pw_rv", NULL,
                                               st->tok_off))
                return 0;
        } else if (!cemit_fmt(out, "%sreturn __cc_pw_e.__cc_pw_rv;\n", i4))
            return 0;
    ...
    if (dest && dest[0]) {
        if (!cemit_fmt(out, "%s} else {\n", i3))
            return 0;
        if (!cemit_fmt(out, "%s%s = (__cc_pw_e.__cc_pw_xk != 1);\n", i4, dest))
            return 0;
        if (!cemit_fmt(out, "%s}\n", i3))
            return 0;
        return cemit_fmt(out, "%s}\n%s}\n%s}\n", i2, i1, indent);
