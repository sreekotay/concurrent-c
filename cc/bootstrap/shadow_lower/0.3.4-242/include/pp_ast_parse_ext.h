...
/* User `@serial` stores the outer dest in a. Expression arms wrap as
 * AST_SERIAL with an empty name — that is not a marked caller strand. */
static int parallel_arm_is_caller_serial(AstNode* arm) {
    return arm && arm->kind == AST_SERIAL && arm->a[0];
}

/* Dest bound, one unmarked arm: kick is that arm, so the dest is never
...
static int parallel_reject_dead_dest(Parser* p, AstNode* n) {
    ...
    if (parallel_arm_is_caller_serial(n->body[0])) return 1;
