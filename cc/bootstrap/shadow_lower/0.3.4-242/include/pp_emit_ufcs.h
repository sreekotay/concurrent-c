 ...
 * CCArena stays a handle token — invented `cc_arena_*` names are macros
 * that already run CC__ARENA_HOST (detach/adopt/destroy take CCArena*). */
static int shadow_ufcs_ty_is_box(const char* vty, const ShadowBind* rb) {
    const char* t;
    if (shadow_ufcs_ty_is(vty, rb, "CCNursery")) return 1;
    if (shadow_ufcs_ty_is(vty, rb, "CCExclusive")) return 1;
...
#define SHADOW_UFCS_PTR_RECV() (ufcs_ptr_recv)
    /* CCBox / nursery / exclusive: Host* methods peel `.p`.
     * destroy / create_child / is_live / host take the handle token.
     * CCArena is not peeled — macros unwrap the handle. */
    box_peel[0] = 0;
    ...
    for (ai = 0; ai < g_shadow_nas; ai++) {
        ...
        }
        {
            ...
            if (strcmp(as->outer, "CCNursery") == 0 &&
                ...
                n = snprintf(peeled, sizeof(peeled), "(*cc_nursery_arena((%s).p))",
                             recv);
            } else {
...
/* Host* nursery entry points — peel a value-token first arg. Births and
 * destroy take the handle (or `CCNursery*`) and must not peel. */
static int shadow_nursery_fn_takes_host(const char* name, size_t nlen) {
    static const char* skip[] = {
        "create",
        "create_child",
        "spawn_child_closure0",
        "spawn_child_closure1",
        "spawn_child_closure2",
        "destroy",
        NULL
    };
    int i;
    if (!name || !nlen) return 0;
    for (i = 0; skip[i]; i++) {
        if (strlen(skip[i]) == nlen && memcmp(name, skip[i], nlen) == 0)
            return 0;
    }
    return 1;
}
...
/* Peel only a bound CCNursery value token. Host* / field / &handle stay. */
static int shadow_nursery_arg_is_value_token(const char* as, const char* ae) {
    char ident[80];
    size_t n = 0;
    const ShadowBind* b;
    if (!as || !ae || as >= ae) return 0;
    if (*as == '&') return 0;
    while (as < ae &&
           ((*as >= 'A' && *as <= 'Z') || (*as >= 'a' && *as <= 'z') ||
            (*as >= '0' && *as <= '9') || *as == '_')) {
        if (n + 1 >= sizeof(ident)) return 0;
        ident[n++] = *as++;
    }
    if (n == 0 || as != ae) return 0;
    ident[n] = 0;
    b = shadow_bind_lookup(ident);
    return b && strcmp(b->ty, "CCNursery") == 0;
}
...
static void shadow_peel_nursery_host_calls(char* expr, size_t cap) {
    ...
    while (*p && o + 1 < sizeof(out)) {
        int at_start = (p == expr) || !shadow_is_id(p[-1]);
        if (at_start && strncmp(p, "cc_nursery_", 11) == 0) {
            const char* name = p + 11;
            const char* q = name;
            while (*q && shadow_is_id(*q)) q++;
            if (*q == '(') {
                size_t nlen = (size_t)(q - name);
                if (shadow_nursery_fn_takes_host(name, nlen)) {
                    const char* arg0 = q + 1;
                    const char* r = arg0;
                    int depth = 1;
                    const char* arg_end = NULL;
                    ...
                    if (arg_end && arg_end > arg0) {
                        ...
                        while (ae > as && (ae[-1] == ' ' || ae[-1] == '\t' ||
                            ...
                            ae--;
                        alen = (size_t)(ae - as);
                        if (alen >= 2 && ae[-1] == 'p' && ae[-2] == '.')
                            already = 1;
                        if (alen >= 3 && ae[-1] == 'p' && ae[-2] == '>' &&
                            ae[-3] == '-')
                            already = 1;
                        if (!already && alen > 0 &&
                            shadow_nursery_arg_is_value_token(as, ae) &&
                            o + (size_t)(q - p) + 1 + alen + 8 < sizeof(out)) {
                            memcpy(out + o, p, (size_t)(q - p) + 1);
                            o += (size_t)(q - p) + 1;
                            memcpy(out + o, arg0, (size_t)(as - arg0));
                            o += (size_t)(as - arg0);
                            out[o++] = '(';
