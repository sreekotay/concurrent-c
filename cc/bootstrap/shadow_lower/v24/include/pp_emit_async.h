 ...
 * Returns 0 on OOM / overflow (never silently leave bare param names). */
static int shadow_async_rewrite_params(char* text, size_t cap, ShadowParam* ps,
                                       int np) {
    char* out;
    size_t out_cap;
    size_t o = 0;
    ...
    if (!ps || np <= 0) return 1;
    /* Prefix `__f->__p_` grows each hit; 3× is ample for realistic bodies. */
    out_cap = cap * 3;
    ...
    while (*p) {
        if (((*p >= 'A' && *p <= 'Z') || (*p >= 'a' && *p <= 'z') ||
            ...
            if (hit >= 0 && !member) {
                int w = snprintf(out + o, out_cap - o, "__f->__p_%s",
                                 ps[hit].name);
                if (w < 0 || (size_t)w >= out_cap - o) {
                    free(out);
                    return 0;
                }
                o += (size_t)w;
...
static int shadow_emit_async_fn(AstNode* it, CEmit* out, TapeCache* cache) {
    ...
    }
    {
        ...
        if (it->d[0] && it->nkids == 0) {
            ...
            if (!shadow_async_rewrite_params(text, sizeof(text), ps, np)) {
                fprintf(stderr,
                        "error: @async '%s': param rewrite overflow\n", it->b);
                out->err = 1;
                bind_ok = 0;
            ...
            if (bind_ok) {
                ...
                /* Match AST_RESULT_FN: bang/cc_ok pack through the real rname
                 * (`bool !>(E)` → CCResult_bool_CCError, not int default). */
                ...
                if (bind_ok && body.buf) {
                     ...
                     * Ensure dest cap ≥ 3×len so rewrite can land in-place.
                     * `need` is *extra* bytes beyond body.len. */
                    size_t want = body.len * 3 + 256;
                    size_t extra =
                        (want > body.cap) ? (want - body.len) : 256;
                    if (!cemit_reserve(&body, extra)) {
                        fprintf(stderr,
                                "error: @async '%s': body buffer OOM\n",
                                it->b);
                        out->err = 1;
                        bind_ok = 0;
                    } else if (!shadow_async_rewrite_params(body.buf, body.cap,
                                                           ps, np)) {
                        fprintf(stderr,
                                "error: @async '%s': param rewrite overflow "
                                "(body %zu bytes)\n",
