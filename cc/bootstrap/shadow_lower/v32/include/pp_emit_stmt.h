...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_PRINTLN_TPL:
        if (st->nbody > 0) {
            ...
            if (!cemit_fmt(out, "%s{\n", indent)) return 0;
            if (!cemit_fmt(out,
                           "%sCCArenaCheckpoint __cc_scratch_cp = "
            ...
            if (!cemit_fmt(out, "%s}\n%s}\n", i2, nested)) return 0;
            if (!cemit_fmt(out, "%scc_arena_restore(__cc_scratch_cp);\n", nested))
                return 0;
            return cemit_fmt(out, "%s}\n", indent);
        }
    ...
    case AST_BREAK: {
        ...
        if (to >= 0 && from >= to) {
            ...
            if (!cemit_fmt(out, "%s{\n", indent)) return 0;
            shadow_indent_nest(nested, sizeof(nested), indent, 1);
            if (!shadow_emit_scopes_exiting(ctx, out, nested, from, to, 1,
                                            st->tok_off))
                return 0;
...
/* Soft-return: unwind nested block/loop @defer/@destroy (gated by __cc_shw_*),
 * then either goto fn __cc_cleanup (fn-level life) or plain return.
    ...
    if (fn_cleanup) {
        if (is_void || !have_val) {
            if (!cemit_fmt(out,
                ...
                "%s{ __cc_retval = %s; __cc_ret_set = 1; %sgoto __cc_cleanup; }\n",
                body_ind, set_retval_expr, extra)) {
            return 0;
        }
    } else {
        /* Scope unwind only — no fn epilogue. */
        if (is_void || !have_val) {
