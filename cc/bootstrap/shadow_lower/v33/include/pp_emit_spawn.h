...
static int shadow_emit_scope_life(const ShadowDeferScope* sc, CEmit* out,
    ...
    for (i = sc->nlife - 1; i >= 0; i--) {
        ...
        if (before_tok_off != SHADOW_TOK_OFF_ALL &&
            n->tok_off >= before_tok_off)
            continue;
        if (is_def && always_only && n->c[0]) continue;
        if (!cemit_fmt(out, "%sif (__cc_shw%d > %d) {\n", indent, sc->shw_id,
                       i))
            return 0;
...
static int shadow_emit_compound_body(AstNode** body, int nbody, CEmit* out,
    ...
    sc->shw_id = depth;
    /* Collect @defer / @destroy in source order (kids order == tok order). */
    for (k = 0; k < nbody; k++) {
        int is_life = 0;
        if (body[k]->kind == AST_DEFER) is_life = 1;
        else if (shadow_stmt_is_destroy(body[k])) is_life = 1;
        if (!is_life) continue;
        if (nlife >= SHADOW_DEFER_SCOPE_DEFERS) {
            fprintf(stderr, "error: too many @defer/@destroy in scope\n");
            out->err = 1;
            return 0;
        }
        life[nlife++] = body[k];
