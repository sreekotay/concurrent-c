        ...
        if (vty[0] && (strncmp(vty, "CCVec_", 6) == 0 ||
            ...
            if (tag[0] >= 'A' && tag[0] <= 'Z' && strncmp(tag, "CC", 2) != 0 &&
                ...
                if (a[0]) {
                   ...
                   strcmp(meth_name, "insert_entry") == 0 ||
                   strcmp(meth_name, "key_logical_live_bytes") == 0) {
            /* Untyped recv (common in string-switch case blobs): prefer a
             * unique Type_meth from the ufn table over inventing store_*. */
            const char* u = shadow_ufn_unique_for_meth(meth_name);
            char storec[80];
            if (u) {
                if (a[0])
                    snprintf(dst, cap,
                             ptr_recv ? "%s(%s, %s)" : "%s(&%s, %s)", u, recv,
                             a);
                else
                    snprintf(dst, cap, ptr_recv ? "%s(%s)" : "%s(&%s)", u,
                             recv);
            } else {
                snprintf(storec, sizeof(storec), "store_%s", meth_name);
                if (a[0])
                    snprintf(dst, cap,
                             ptr_recv ? "%s(%s, %s)" : "%s(&%s, %s)", storec,
                             recv, a);
                else
                    snprintf(dst, cap, ptr_recv ? "%s(%s)" : "%s(&%s)", storec,
                             recv);
            }
        }
