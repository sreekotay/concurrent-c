            ...
            if (strstr(st->e, "!>") || st->nbody > 0) {
                ...
                while (*p) {
                     ...
                     * works for `.step` / `.as_int` when no bind matches. */
                    {
                         ...
                         * cc_js_iso* extracts through CCJsIsoVal (the wire
                         * tier's value), off other cc_js_* / CCJsVal through
                         * CCJsVal, off cc_py_* through CCPyObj (the historic
                         * default).  Iso first — "cc_js_iso" is inside
                         * "cc_js_" and CCJsIsoVal inside a CCJs match. */
                        int iso_prod = strstr(call, "cc_js_iso") != NULL ||
                                       strstr(call, "CCJsIso") != NULL ||
                                       strstr(ty, "CCJsIso") != NULL;
                        ...
                        if (strstr(recv, "__r") &&
                            ...
                            hop_ty = iso_prod ? "CCJsIsoVal"
                                   : js_prod  ? "CCJsVal"
                                              : "CCPyObj";
                        else if (iso_prod && !strstr(ty, "CCJsIso"))
                            hop_ty = "CCJsIsoVal";
