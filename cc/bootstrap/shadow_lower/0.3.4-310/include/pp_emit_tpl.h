    ...
    while (*p) {
        if (p[0] == '$' && p[1] == '{') {
            ...
            if (!shadow_tpl_parse_hole(&p, name, sizeof(name), prefix,
                                      sizeof(prefix), &hint)) {
                {
                    ...
                    snprintf(__diag, sizeof(__diag),
                             "emit template: malformed hole near '%.32s'",
                             start);
            ...
            if (hint == SHADOW_TPL_LINE || strcmp(name, "line") == 0) {
                ...
                if (!b || !b->value) {
                    shadow_err(NULL, NULL, "emit template: missing ${line} bind");
                    out->err = 1;
                    return 0;
                }
            ...
            if (hint == SHADOW_TPL_FILE || strcmp(name, "file") == 0) {
                ...
                if (!b || !b->value) {
                    shadow_err(NULL, NULL, "emit template: missing ${file} bind");
                    out->err = 1;
                    return 0;
                }
                if (!cemit_str(out, b->value)) return 0;
            ...
            if (!b) {
                {
                    char __diag[96];
                    snprintf(__diag, sizeof(__diag),
                             "emit template: unbound hole '${%s}'", name);
                    shadow_err(NULL, NULL, __diag);
                }
                out->err = 1;
                return 0;
                ...
                if (b->kind != SHADOW_TPL_STMTS) {
                    {
                        char __diag[128];
                        snprintf(__diag, sizeof(__diag),
                                 "emit template: hole '${%s}' needs STMTS "
                                 "bind (got kind %d)",
                                 name, b->kind);
                        shadow_err(NULL, NULL, __diag);
                    }
            ...
            if (b->kind != SHADOW_TPL_IDENT && b->kind != SHADOW_TPL_EXPR) {
                {
                    char __diag[128];
                    snprintf(__diag, sizeof(__diag),
                             "emit template: hole '${%s}' needs IDENT/EXPR "
                             "(got kind %d)",
                             name, b->kind);
                    shadow_err(NULL, NULL, __diag);
                }
...
static int shadow_tpl_kv(CEmit* out, const char* tpl, ...) {
    ...
    for (;;) {
        ...
        if (!name) break;
        if (n >= 48) {
            shadow_err(NULL, NULL, "shadow_tpl_kv: too many binds");
            out->err = 1;
            va_end(ap);
            return 0;
        }
...
static int cemit_fmt(CEmit* e, const char* fmt, ...) {
    ...
    while (*p) {
        ...
        if (nbind >= 48) {
            shadow_err(NULL, NULL, "cemit_fmt: too many holes");
            e->err = 1;
            va_end(ap);
            if (tpl != tpl_stack) free(tpl);
            return 0;
        ...
        if (*p == 's') {
        ...
        } else {
            {
                char __diag[96];
                snprintf(__diag, sizeof(__diag),
                         "cemit_fmt: unsupported format near '%%%s'", p);
                shadow_err(NULL, NULL, __diag);
            }
            e->err = 1;
            va_end(ap);
        ...
        if (p[0] == '$' && p[1] == '{') {
            ...
            if (!shadow_tpl_parse_hole(&p, name, sizeof(name), prefix,
                                      sizeof(prefix), &hint)) {
                {
                    char __diag[128];
                    snprintf(__diag, sizeof(__diag),
                             "emit template '%s': malformed hole near "
                             "'%.32s'",
                             id ? id : "?", start);
                    shadow_err(NULL, NULL, __diag);
                }
            ...
            if (!b) {
                {
                    char __diag[128];
                    snprintf(__diag, sizeof(__diag),
                             "emit template '%s': hole '${%s}' not in schema",
                             id ? id : "?", name);
                    shadow_err(NULL, NULL, __diag);
                }
                return 0;
            ...
            if (b->kind == SHADOW_TPL_STMTS || hint == SHADOW_TPL_STMTS ||
                ...
                if (prefix[0] &&
                    !shadow_tpl_find(schema, nschema, prefix, strlen(prefix))) {
                    {
                        char __diag[128];
                        snprintf(__diag, sizeof(__diag),
                                 "emit template '%s': stmts prefix '%s' not "
                                 "in schema",
                                 id ? id : "?", prefix);
                        shadow_err(NULL, NULL, __diag);
                    }
                       ...
                       b->kind != SHADOW_TPL_EXPR) {
                {
                    char __diag[128];
                    snprintf(__diag, sizeof(__diag),
                             "emit template '%s': '${%s}' schema kind must be "
                             "IDENT/EXPR",
                             id ? id : "?", name);
                    shadow_err(NULL, NULL, __diag);
                }
