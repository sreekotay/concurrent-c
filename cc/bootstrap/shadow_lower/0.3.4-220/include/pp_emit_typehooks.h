        ...
        if (in_typedef_struct && line[0] == '}') {
            ...
            if (((*a >= 'A' && *a <= 'Z') || (*a >= 'a' && *a <= 'z') ||
                ...
                if (alias[0]) {
                    ...
                    int fi;
                    for (fi = 0; fi < nfld; fi++) {
                        int same = !fld_outer[fi][0] ||
                                   (outer[0] &&
                                    strcmp(fld_outer[fi], outer) == 0);
                        if (same) {
                            if (!fld_outer[fi][0])
                                snprintf(fld_outer[fi], sizeof(fld_outer[0]),
                                         "%s", alias);
                ...
                if (fname[0] && strcmp(fname, fty) != 0) {
                    ...
                    if (nfld < 64) {
                        snprintf(fld_outer[nfld], sizeof(fld_outer[0]), "%s",
                                 outer);
                        snprintf(fld_name[nfld], sizeof(fld_name[0]), "%s",
                                 fname);
                        ...
                        for (i = 0; i < nfld; i++) {
                            ...
                            if (shadow_ty_is_glob(tv_base))
                                ...
                                match = strcmp(fld_outer[i], tv_base) == 0 &&
                                        strcmp(fld_name[i], fname) == 0;
                            if (match) {
                                shadow_as_register(fld_outer[i], fname,
                                                   fld_ty[i]);
                                matched = 1;
                                if (shadow_ty_is_glob(tv_base)) {
                                    int gi;
