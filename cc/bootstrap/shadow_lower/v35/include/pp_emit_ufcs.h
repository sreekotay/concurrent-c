...
#define SHADOW_UFCS_PTR_RECV()                                                       (is_arrow || path_is_ptr ||                                                        (!walked_fields && rb && strchr(rb->ty, '*') != NULL &&                      strchr(recv, '.') == NULL && strstr(recv, "->") == NULL) ||                recv[0] == '&')
...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
     ...
     * Arrow / final field pointer / already-&. Bare pointer bind only when
     * recv has no field path — `p->out.clear` uses path_is_ptr from walk. */
#define SHADOW_UFCS_PTR_RECV()                                                       (is_arrow || path_is_ptr ||                                                        (!walked_fields && rb && strchr(rb->ty, '*') != NULL &&                      strchr(recv, '.') == NULL && strstr(recv, "->") == NULL) ||                recv[0] == '&')
    ...
    if (!is_arrow && strcmp(meth_name, "remaining") == 0) {
        ...
        char tag[96];
        char mty[96];
        /* Field-walk PTR_RECV: c->out.push_slice is CCString value, not Conn*. */
        int ptr_recv = SHADOW_UFCS_PTR_RECV();
        tag[0] = mty[0] = 0;
