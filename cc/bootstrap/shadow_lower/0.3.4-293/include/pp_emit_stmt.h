...
static int shadow_emit_walk_set(CEmit* out, ShadowCtx* ctx, const char* indent,
    ...
    /* Re-read live .len; write through naked .access. Out of range is
     * the walk's error — not set(), not a skip. A hoisted @errhandler
     * writes the cell then goto (never returns); otherwise else-skip
     * the store. */
...
static int shadow_flush_par_helpers(CEmit* out) {
    int ok = 1;
    if (!out) return 0;
    if (g_shadow_par_helpers.len && g_shadow_par_helpers.buf) {
        if (!cemit_str(out, "\n/* --- @parallel thunks --- */\n"))
            ok = 0;
...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_FOR: {
        ...
        if (in_kind == 1 || in_kind == 3) {
            ...
            shadow_extent_load_c(&ex, recv, idx, load_c, sizeof(load_c));
            if (val_mut) {
            ...
            if (!cemit_fmt(out, "%sfor (size_t %s = 0; %s < (%s); ++%s) {\n",
            ...
            }
            {
                ...
                if (!cemit_fmt(out, "%s%s%s %s = %s;\n", nested,
                               val_mut ? "" : "const ", ex.elem, val, load_c)) {
                    shadow_pw_pop_loop(ctx, pw, 0);
                    return 0;
                }
                nbinds_mark = g_shadow_nbinds;
                if (in_kind == 3) shadow_bind_name(fi.bind[0], "size_t", 0);
        ...
        if (in_kind == 4) {
            ...
            shadow_extent_load_c(&ex_a, recv_a, idx, load_a, sizeof(load_a));
            shadow_extent_load_c(&ex_b, recv_b, idx, load_b, sizeof(load_b));
            if (!cemit_fmt(out,
            ...
            if (!cemit_fmt(out,
                           "%s    } else {\n"
                           "%s    for (size_t %s = 0; %s < __cc_zl_%d; ++%s) "
                           "{\n",
                           i1, i1, idx, idx, id, idx)) {
                shadow_pw_pop_loop(ctx, pw, 0);
                return 0;
            }
            {
                ...
                shadow_indent_nest(nested, sizeof(nested), indent, 2);
                if (!cemit_fmt(out, "%s%s%s %s = %s;\n%s%s%s %s = %s;\n",
                               nested, fi.mut[0] ? "" : "const ", ex_a.elem,
                               fi.bind[0], load_a, nested,
                               fi.mut[1] ? "" : "const ", ex_b.elem, fi.bind[1],
                               load_b)) {
                    shadow_pw_pop_loop(ctx, pw, 0);
                    return 0;
                }
                ...
                if (fi.mut[0]) shadow_bind_walk_cursor(fi.bind[0], recv_a, idx);
                if (fi.mut[1]) shadow_bind_walk_cursor(fi.bind[1], recv_b, idx);
