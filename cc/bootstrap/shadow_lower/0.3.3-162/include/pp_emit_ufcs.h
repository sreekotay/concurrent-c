...
#define SHADOW_UFCS_PTR_RECV() (ufcs_ptr_recv)
    ...
    if (vty[0]) {
        ...
        {
            int hook_by_val = 0;
            if (shadow_ufcs_hook_resolve(hbase, meth_name, a, hcal, sizeof(hcal),
                                        &hook_by_val)) {
     ...
     * Fast path for CCFile IO on a plain ident + dot. Arrow / other
     * methods retry every as: face after a Type_meth miss (below). */
...
/* Extracted header protos are not in the TU AST — consult the .cch. */
static int shadow_slc_header_arg(const char* callee, int arg_index) {
    char ty[160];
    if (!callee || arg_index < 0) return 0;
    if (!cc_included_cch_fn_param(callee, arg_index, ty, sizeof(ty))) return 0;
    return shadow_slc_type_spelling(ty);
}
                    ...
                    al++;
                {
                    ...
                    if (*r == ')' && al > 0 && al < sizeof(arg)) {
                        ...
                        if (b && strncmp(b->ty, "CCSlice_", 8) == 0 &&
                            ...
                            if (!skip && !fp) {
                                char fparam[128];
                                if (!(cc_included_cch_fn_first_param(
                                          callee, fparam, sizeof(fparam)) &&
                                      strncmp(fparam, "CCSlice", 7) == 0 &&
                                      !((fparam[7] >= 'A' && fparam[7] <= 'Z') ||
                                        (fparam[7] >= 'a' && fparam[7] <= 'z') ||
                                        (fparam[7] >= '0' && fparam[7] <= '9') ||
                                        fparam[7] == '_')))
                                    skip = 1;
                            }
                            if (!skip) {
