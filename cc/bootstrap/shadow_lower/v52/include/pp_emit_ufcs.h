...
#define SHADOW_UFCS_PTR_RECV()                                                   \
    ...
    if (!is_arrow && strcmp(meth_name, "remaining") == 0) {
        ...
        /* Flipped script print: data.println() (stdio.cch).
         * char* / const char* must not lose `*` via bind_base_ty — callees
         * are cc_char_* / cc_const_char_* taking the pointer by value.
         * `io.println(x)` (args present, non-fprint) still prints args. */
        ...
        if (!io_handle && (is_cstr || is_const_cstr)) {
            const char* pref = is_const_cstr ? "cc_const_char_" : "cc_char_";
            if (a && a[0])
                snprintf(dst, cap, "%s%s(%s, %s)", pref, meth_name, recv, a);
            else
                snprintf(dst, cap, "%s%s(%s)", pref, meth_name, recv);
        } else if (io_handle) {
