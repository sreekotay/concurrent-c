...
#define SHADOW_UFCS_PTR_RECV()                                                   \
    ...
    if (!is_arrow && strcmp(meth_name, "remaining") == 0) {
        ...
        else if (shadow_ufcs_hdr_member(vty, rb, meth_name, hdrm,
                                        sizeof(hdrm))) {
            /* Header-installed as_slice — an arena argument (CCJsVal
             * carries no home arena) passes through when spelled. */
            if (a[0])
                snprintf(dst, cap, "%s(&%s, %s)", hdrm, recv, a);
            else
                snprintf(dst, cap, "%s(&%s)", hdrm, recv);
