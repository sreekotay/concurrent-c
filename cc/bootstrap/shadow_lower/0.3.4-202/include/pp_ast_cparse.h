...
static int shadow_cparse_try_file_fn(Parser* p) {
    ...
    }
    {
        ...
        if (cparse_match_func(ft->bytes, (int)ft->len, cpt, ntok, name,
                              ...
                              (int)sizeof(err)) != 0) {
            free(cpt);
            /* Overlay shapes (Result / defaults / @ / UFCS): beachhead
             * still attaches kids. Pure-C mismatch is loud. */
            if (overlay) {
...
/* After '{': attach body kids via cparse stmt spans + overlay parse_stmt.
 * Soft miss tapes that one stmt only. Leaves p at closing '}'. Returns 1. */
static int shadow_cparse_attach_fn_body(Parser* p, AstNode* n, int fn_start,
    ...
    if (rb < 0) {
        parser_fail(p, p->toks[body0 > 0 ? body0 - 1 : body0], "unclosed function");
        return 0;
    }
    ...
    /* No spans: soft-parse each stmt; tape the remainder as one kid. */
    saved_soft = p->soft_stmt;
    saved_err = p->err;
    p->i = body0;
    p->soft_stmt = 1;
    while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF &&
        ...
        p->err = saved_err;
        s = shadow_tape_tok_range(p, s0, rb);
        if (!s || !ast_kids_push(p, s)) {
            p->soft_stmt = saved_soft;
            return 0;
        }
        n->nkids++;
