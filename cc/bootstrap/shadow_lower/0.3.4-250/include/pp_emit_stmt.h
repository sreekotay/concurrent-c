...
static int shadow_emit_par_protos(CEmit* out, AstNode** pars, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (pars[i]->kind == AST_PARALLEL_FOR) {
            ...
            int a, nspawn = pars[i]->nbody > 1 ? pars[i]->nbody - 1 : 0;
            for (a = 0; a < nspawn; a++) {
...
static int shadow_emit_par_spawn_body(
    ...
    }
    {
        ...
        if (st && st->body[0] && st->body[0]->kind == AST_SERIAL) {
            if (via_outs) {
...
static int shadow_emit_parallel(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (!st->par_wake) {
        ...
        if (n == 1 && st->body[0] &&
            !(st->body[0]->kind == AST_SERIAL && st->body[0]->a[0])) {
            fprintf(stderr,
    ...
    for (a = 0; a < n; a++) {
        ...
        if (a > 0 && !st->forced_seq) {
            ...
            if (serial) {
                ...
                if (!shadow_par_emit_serial_thunk(base + a - 1, base,
                                                  lhs_ty[a], lhs[a], arm,
                ...
                shadow_pj_restore(ctx, pb, pt, pex, pjn);
            } else if (!shadow_par_emit_assign_thunk(base + a - 1, base,
                                                     lhs_ty[a], rhs[a],
                                                     cap_names[a],
                                                     cap_tys[a], ncaps[a]))
                return 0;
