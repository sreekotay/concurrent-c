...
/* Complementary send/recv across two arms of one brace join: never deny.
 * Inlining the consumer while the kick parks on send is a silent hang. */
static int shadow_par_rendezvous_never_deny(AstNode* st) {
    int a;
    int send_arm = -1;
    int recv_arm = -1;
    if (!st) return 0;
    for (a = 0; a < st->nbody; a++) {
        int op = shadow_par_arm_chan_ops(st->body[a]);
        if (op & CC__PAR_CH_SEND) send_arm = a;
        if (op & CC__PAR_CH_RECV) recv_arm = a;
    }
    return send_arm >= 0 && recv_arm >= 0 && send_arm != recv_arm;
}
...
static int shadow_emit_par_spawn_body(
    ...
    int never_deny = shadow_par_rendezvous_never_deny(st);
    for (a = spawn0; a < n; a++) {
