...
#define SHADOW_UFCS_PTR_RECV()                                                   \
    ...
    if (!is_arrow && strcmp(meth_name, "remaining") == 0) {
        ...
        if (!io_handle && (is_cstr || is_lit)) {
            ...
            const char* amp =
                (is_arrow || SHADOW_UFCS_PTR_RECV()) ? "" : "&";
            /* Rvalue stmt-expr: take address of the temporary. */
            int needs_paren = (strchr(recv, '(') != NULL);
            if (a && a[0]) {
                if (needs_paren && !amp[0])
                    snprintf(dst, cap, "cc_string_%s(%s, %s)", meth_name, recv,
                             a);
                else if (needs_paren)
                    snprintf(dst, cap, "cc_string_%s(&(%s), %s)", meth_name,
                             recv, a);
                else
                    snprintf(dst, cap, "cc_string_%s(%s%s, %s)", meth_name, amp,
                             recv, a);
            } else if (needs_paren && amp[0])
                snprintf(dst, cap, "cc_string_%s(&(%s))", meth_name, recv);
            else
                snprintf(dst, cap, "cc_string_%s(%s%s)", meth_name, amp, recv);
        } else if (a && a[0] &&
                   (strcmp(meth_name, "fprint") == 0 ||
                    strcmp(meth_name, "fprintln") == 0)) {
