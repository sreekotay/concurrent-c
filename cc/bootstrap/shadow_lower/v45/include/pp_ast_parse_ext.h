...
static AstNode* parse_global_var(Parser* p) {
    ...
    if (tkw == SHADOW_KW_NONE && !scope_is_typedef(p, ty.spell)) {
        ...
        if (!(tname[0] == 'C' && tname[1] == 'C') &&
            ...
            strcmp(tname, "atomic_int") != 0 &&
            strcmp(tname, "atomic_bool") != 0 &&
            strcmp(tname, "atomic_size_t") != 0 &&
            strcmp(tname, "atomic_long") != 0 &&
            strcmp(tname, "uint8_t") != 0 &&
            strcmp(tname, "uint16_t") != 0 &&
            strcmp(tname, "uint32_t") != 0 &&
            strcmp(tname, "uint64_t") != 0 &&
            strcmp(tname, "int8_t") != 0 &&
            strcmp(tname, "int16_t") != 0 &&
            strcmp(tname, "int32_t") != 0 &&
            strcmp(tname, "int64_t") != 0 &&
            strcmp(tname, "size_t") != 0 &&
            strcmp(tname, "uintptr_t") != 0 &&
            strcmp(tname, "intptr_t") != 0)
            return NULL;
    }
    int ni = ti + 1;
    int is_ptr = 0;
...
static AstNode* parse_at_stmt(Parser* p) {
    ...
    if (spell_eq(name.spell, "arena_init")) {
        ...
        int line = 1, col = 1;
        if (ft && ft->bytes)
            offset_to_linecol(ft->bytes, ft->len, name.offset, &line, &col);
        (void)col;
        fprintf(stderr,
                "%s:%d: error: async: `@arena_init(...) { ... }` is retired; "
                "use `CCArena a = @create(buf, size) @destroy` or "
    ...
    /* `@restricted [Mode] on Base { r:…; w:…; rw:… }` — allow-list (draft_facets).
     * Unnamed `@restricted on Base` applies to Base itself (no parallel type). */
    if (spell_eq(name.spell, "restricted")) {
        ...
        if (mode.kind == TK_IDENT && spell_eq(mode.spell, "on")) {
            ...
            if (mode.kind != TK_IDENT) {
                parser_fail(p, mode, "expected mode name or 'on' after @restricted");
                return NULL;
            }
            on = p_next(p);
            if (on.kind != TK_IDENT || !spell_eq(on.spell, "on")) {
                parser_fail(p, on, "expected 'on' after @restricted mode name");
                return NULL;
            }
        }
        base = p_next(p);
        if (base.kind != TK_IDENT) {
            parser_fail(p, base, "expected base type after @restricted … on");
            return NULL;
        }
        if (!tok_eq(p_peek(p), TK_PUNCT, "{")) {
            parser_fail(p, p_peek(p), "expected '{' allow-list after @restricted");
            return NULL;
        }
        body0 = p->i;
        p_next(p);
        ...
        if (body0 >= 0 && body1 > body0) {
            if (!ast_spell_token_range(p, body0, body1, body, sizeof(body)) &&
                ...
                parser_fail(p, base, "@restricted allow-list too long");
                return NULL;
            }
        }
        n = ast_new(p, AST_AT_STMT);
        if (!n) return NULL;
        snprintf(n->a, sizeof(n->a), "restricted");
        if (unnamed) n->b[0] = 0;
        ...
        if (unnamed) {
            ...
            slice_to(n->e, sizeof(n->e), base.spell);
        } else {
...
/* typedef @restricted Mode on Base { … } [*]? Alias ;
...
static AstNode* parse_typedef_restricted(Parser* p) {
    ...
    if (p->toks[p->i + 2].kind != TK_IDENT ||
        !spell_eq(p->toks[p->i + 2].spell, "restricted"))
        return NULL;
    /* Sugar `typedef @restricted(Mode) Base* Alias` is rewritten on the tape
     * before parse; the define form is `@restricted Mode on Base {`. */
    ...
    if (body0 >= 0 && body1 > body0) {
        if (!ast_spell_token_range(p, body0, body1, body, sizeof(body)) &&
            !span_text(p, body0, body1, body, sizeof(body))) {
            parser_fail(p, mode, "@restricted allow-list too long");
            return NULL;
        }
    }
    if (tok_eq(p_peek(p), TK_PUNCT, "*")) {
    ...
    alias = p_next(p);
    if (alias.kind != TK_IDENT) {
        parser_fail(p, alias, "expected typedef alias after @restricted body");
        return NULL;
    }
    if (!p_accept(p, TK_PUNCT, ";")) {
        parser_fail(p, p_peek(p), "expected ';' after typedef @restricted");
        return NULL;
    }
    snprintf(mangled, sizeof(mangled), "%.*s_Restrict_%.*s",
             (int)base.spell.len, base.spell.ptr, (int)mode.spell.len,
...
static AstNode* parse_external_inner(Parser* p) {
    ...
    AstNode* n;
    /* cpp-transparent `#if`/`#elif`/`#else`/`#endif` lines from stage-2. */
...
static int parse_tu(Parser* p, AstNode*** out_items, int* out_n) {
     ...
     * their typedef aliases before the CamelCase unknown-type gate. */
