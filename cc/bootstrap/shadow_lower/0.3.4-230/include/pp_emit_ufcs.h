...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
    ...
    char a[4096];
    char recv_buf[192];
    char nursery_peel[256];
    char exclusive_peel[256];
    const ShadowBind* rb;
    char vty[128];
    char hdrm[224];
    int ufcs_ptr_recv = 0;
...
#define SHADOW_UFCS_PTR_RECV() (ufcs_ptr_recv)
    /* CCNursery is a value handle; most C entry points take CCNurseryHost*.
     * create_child / destroy take the token (or `CCNursery*`), not Host*. */
    nursery_peel[0] = 0;
    if (shadow_ufcs_ty_is(vty, rb, "CCNursery") &&
        strcmp(meth_name, "create_child") != 0 &&
        strcmp(meth_name, "destroy") != 0) {
        const char* last = recv + strlen(recv);
        int already = 0;
        int star = 0;
        int npeel;
        while (last > recv && (last[-1] == ' ' || last[-1] == '\t')) last--;
        if (last - recv >= 2 && last[-2] == '.' && last[-1] == 'n')
            already = 1;
        if (!walked_fields && rb && strchr(rb->ty, '*') != NULL) star = 1;
        if (vty[0] && vty[strlen(vty) - 1] == '*') star = 1;
        if (!already) {
            npeel = snprintf(nursery_peel, sizeof(nursery_peel),
                             star ? "(%s)->n" : "(%s).n", recv);
            if (npeel > 0 && (size_t)npeel < sizeof(nursery_peel)) {
                recv = nursery_peel;
                ufcs_ptr_recv = 1;
            }
        }
    }
    /* CCArena: no peel. Invented `cc_arena_*` names are macros that
     * already run CC__ARENA_HOST — pass the handle (`&field` / recv).
     * CCExclusive C entry points take Host*; destroy takes the token. */
    exclusive_peel[0] = 0;
    if (shadow_ufcs_ty_is(vty, rb, "CCExclusive") &&
        strcmp(meth_name, "destroy") != 0) {
        const char* last = recv + strlen(recv);
        int already = 0;
        int star = 0;
        int npeel;
        while (last > recv && (last[-1] == ' ' || last[-1] == '\t')) last--;
        if (last - recv >= 2 && last[-2] == '.' && last[-1] == 'e')
            already = 1;
        if (!walked_fields && rb && strchr(rb->ty, '*') != NULL) star = 1;
        if (vty[0] && vty[strlen(vty) - 1] == '*') star = 1;
        if (!already) {
            npeel = snprintf(exclusive_peel, sizeof(exclusive_peel),
                             star ? "(%s)->e" : "(%s).e", recv);
            if (npeel > 0 && (size_t)npeel < sizeof(exclusive_peel)) {
                recv = exclusive_peel;
                ufcs_ptr_recv = 1;
            }
        }
    ...
    /* Nursery.wait / .cancel: typed CC* invent → cc_nursery_{wait,cancel}. */
        ...
        /* Closure form: `() => …` literal, CCClosure0 value, or make(). */
    ...
    for (ai = 0; ai < g_shadow_nas; ai++) {
        ...
        }
        {
            ...
            if (strcmp(as->outer, "CCNursery") == 0 &&
                ...
                n = snprintf(peeled, sizeof(peeled), "(*cc_nursery_arena((%s).n))",
                             recv);
            } else {
                ...
                if (shadow_nursery_fn_takes_host(name, nlen)) {
                    ...
                    if (arg_end && arg_end > arg0) {
                        ...
                        while (ae > as && (ae[-1] == ' ' || ae[-1] == '\t' ||
                            ...
                            ae--;
                        alen = (size_t)(ae - as);
                        if (alen >= 2 && ae[-1] == 'n' && ae[-2] == '.')
                            already = 1;
                        if (alen >= 3 && ae[-1] == 'n' && ae[-2] == '>' &&
                            ae[-3] == '-')
                            already = 1;
                        if (!already && alen > 0 &&
