 ...
 * ~20 outer locals; 16 overflowed on pigz_wait's end state. */
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (st->f[0]) {
        if (!cemit_fmt(out,
                       ...
                       "cc_nursery_create(); "
                       "if (__cc_pw_nr_%d.ok) __cc_pw_n_%d = __cc_pw_nr_%d.u.value.n; }\n",
                       i1, st->f, id, id, id, id))
            goto fail;
    } else if (!cemit_fmt(out,
     ...
     * when the admitter is inlined (i386 -O2 pigz_wait). */
    ...
    if (!cemit_fmt(out, "%s__cc_pw_args_%d[__cc_pw_w_%d].slot = __cc_pw_w_%d;\n",
        ...
        return 0;
    if (!cemit_fmt(out,
            "%sif (cc_nursery_spawn(__cc_pw_n_%d, __cc_pw_worker_%d, "
            "&__cc_pw_args_%d[__cc_pw_w_%d]) == 0)\n",
            i4, id, id, id, id))
        return 0;
    ...
    case AST_IF: {
        ...
        if (st->e[0] && strcmp(st->e, "spawn_cond") == 0) {
            ...
            if (!sp) return 0;
            {
                ...
                {
                    char nrecv[192];
                    shadow_nursery_host_recv(sp->a, nrecv, sizeof(nrecv));
                    if (args[0])
                        snprintf(cond, sizeof(cond),
                                 "%s(%s, cc_closure__N%s_make(%s))%s", nur_spawn,
                                 nrecv, sp->d[0] ? sp->d : "1", args, st->a);
                    else
                        snprintf(cond, sizeof(cond),
                                 "%s(%s, cc_closure__N%s_make())%s", nur_spawn,
                                 nrecv, sp->d[0] ? sp->d : "1", st->a);
                }
            }
        } else {
...
static int shadow_emit_one_destroy(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (is_nur && !is_ptr) {
        /* Value handle: wait + free + null `.n` via cc_nursery_destroy. */
        if (!bare) {
