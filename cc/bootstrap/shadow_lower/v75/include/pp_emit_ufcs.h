...
static int shadow_ufcs_try_dyn_sink(const char* recv, const char* meth_name,
    ...
    if (ds->dest_aware && g_shadow_sink_dest[0]) {
        ...
        if (mangled[0]) {
            /* Installed py callm variants (py.cch) — invent when ufn table misses. */
            int inventable =
            ...
            /* Broader scalar set — plain sink cannot initialize these. */
            int scalar_dest =
                inventable || strcmp(mangled, "short") == 0 ||
            ...
            /* Installed js callm variants (js.cch) — the header's own set. */
            int js_installed =
                strcmp(mangled, "double") == 0 ||
                strcmp(mangled, "int64_t") == 0 ||
                strcmp(mangled, "float") == 0 ||
                strcmp(mangled, "int") == 0 ||
                strcmp(mangled, "long_long") == 0;
    ...
    if (!is_arrow && strcmp(meth_name, "remaining") == 0) {
        ...
        if (vty[0] && (strncmp(vty, "CCVec_", 6) == 0 ||
            ...
            if (a[0])
               ...
               (strcmp(vty, "CCJsVal") == 0 ||
                shadow_bind_ty_has(rb, "CCJsVal"))) {
        /* Before the py chain-temp heuristic: a bind typed CCJsVal (a JS
         * sink or eval hop) names its own extractor. */
        snprintf(dst, cap, "cc_js_val_%s(&%s)", meth_name, recv);
    } else if (!is_arrow && strcmp(meth_name, "as_f64") == 0 &&
               (strcmp(vty, "CCPyObj") == 0 ||
                shadow_bind_ty_has(rb, "CCPyObj") ||
                strstr(recv, "__r") != NULL)) {
        /* Same extractor rule for the py twin — without it an as_f64 hop
         * sinks into a Python attribute call that does not exist. */
        snprintf(dst, cap, "cc_py_obj_as_f64(&%s)", recv);
        ...
        else if ((strcmp(vty, "CCJsVal") == 0 ||
                  shadow_bind_ty_has(rb, "CCJsVal")) && a[0])
            /* String(value) into the caller's arena — CCJsVal carries no
             * home arena, so the argument is required. */
            snprintf(dst, cap, "cc_js_val_as_slice(&%s, %s)", recv, a);
