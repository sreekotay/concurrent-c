...
static int shadow_emit_par_spawn_body(
    ...
    if (hdest && hdest[0]) {
        ...
        if (!cemit_fmt(dest,
                       "%s    %s = cc_parallel_empty();\n"
                       "%s    %s.joined = 1;\n"
                       "%s} else {\n"
                       "%s    %s = cc_parallel_empty();\n",
                       bi, hdest, bi, hdest, bi, bi, hdest))
            return 0;
        for (a = 1; a < n; a++) {
            int id = base + a - 1;
            if (!cemit_fmt(dest,
                    ...
                    "%s        free(__cc_par_e_%d);\n"
                    "%s    }\n",
                    bi, id, bi, id, id, bi, id, bi, id, id, bi, bi, id,
                    bi, id, id, bi, id, bi, hdest, bi, hdest, hdest, id,
                    bi, hdest, hdest, id, bi, hdest, bi, bi, id, bi, id,
                    bi))
                return 0;
        }
        return cemit_fmt(dest, "%s}\n", bi);
