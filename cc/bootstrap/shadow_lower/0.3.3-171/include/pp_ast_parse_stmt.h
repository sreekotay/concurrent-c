...
static AstNode* parse_static_fn(Parser* p) {
    ...
    }
    {
         ...
         * tape — that leaves !>/@/=>/[:] / :: unlowered. Plain-C bodies may
         * rewind. */
        if (!ok_stmts) {
            int j = body0;
            int d = 1;
    ...
    if (a0 >= a1) {
        ...
        if (had_cl) {
            ...
            }
            {
                ...
                /* Spell omits trivia before `)`. Primary span_text keeps it
                 * via exclusive-end extension; restore after spell so a dotted
                 * arg (`ru.ru_maxrss`) cannot force `#endif)`. */
                if (!from_primary_span &&
                    !ast_append_trail_before(p, a1 - 1, a1, n->b,
                                             sizeof(n->b))) {
                    parser_fail(p, callee, "call args too long");
                    return NULL;
                }
