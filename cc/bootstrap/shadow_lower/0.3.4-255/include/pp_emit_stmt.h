...
static int shadow_pj_emit_cell(CEmit* out, const char* pad, int id,
    ...
    if (!cemit_fmt(out, "%s__cc_pj_ex_%d_t __cc_pj_ex_%d;\n", pad, id, id))
        return 0;
    if (!cemit_fmt(out,
                   "%smemset(&__cc_pj_ex_%d, 0, sizeof(__cc_pj_ex_%d));\n"
        ...
        if (init && init[0] == '{') {
            ...
            if (!cemit_fmt(out, "%s__cc_pw_cx_%s[__cc_pw_ck] = %s;\n", i2,
                ...
                return 0;
                       ...
                       "%smemset(&__cc_pw_cx_%s[__cc_pw_ck], 0, "
                       "sizeof(%s));\n",
                       i2, names[k], tys[k]))
            return 0;
        if (!cemit_fmt(out,
...
static int shadow_emit_stage_region(AstNode** body, int nbody, CEmit* dest,
    ...
    if (!cemit_fmt(dest, "%sCCError __cc_stg_err_%d;\n", i1, id))
        goto fail;
    if (!cemit_fmt(dest, "%smemset(&__cc_stg_err_%d, 0, sizeof(__cc_stg_err_%d));\n",
                   i1, id, id))
        goto fail;
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (!cemit_fmt(out, "%sint %s;\n", i2, st->a))
        return 0;
    if (!cemit_fmt(out, "%smemset(&__cc_pw_ferr_%d, 0, sizeof(__cc_pw_ferr_%d));\n",
                   i2, id, id))
        return 0;
    ...
    if (!cemit_fmt(out, "%s__cc_pw_sh_%d.work = __cc_pw_work_%d;\n", i3, id, id))
        return 0;
    if (!cemit_fmt(out,
            "%smemset(__cc_pw_slots_%d, 0, (size_t)__cc_pw_nw_%d * "
            "sizeof(*__cc_pw_slots_%d));\n",
            i3, id, id, id))
        return 0;
    if (!cemit_fmt(out, "%smemset(__cc_pw_e, 0, sizeof(*__cc_pw_e));\n", i3))
        return 0;
...
static int shadow_emit_parallel_for(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (!cemit_fmt(out,
            "%smemset(&__cc_pf_ex_%d, 0, sizeof(__cc_pf_ex_%d));\n"
