...
static int shadow_ufcs_map_elem_desc(const char* t, int* esz, int* isf) {
    ...
    if (strcmp(t, "int64_t") == 0 || strcmp(t, "long long") == 0 ||
        strcmp(t, "long") == 0) { *esz = 8; *isf = 0; return 1; }
...
static int shadow_ufcs_emit_map_rows(const char* recv, const char* base,
    ...
    n = snprintf(dst + o, cap - o, "cc__%s_map_raw(%s%s, %.*s, %d, %d, %d, "
                 "(const %s[]){ ",
                 snake + 3, strstr(recv, ".u.value") ? "" : "&", recv,
...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
     ...
     * carries no type token.  A hop-hint elem_ty (a CC type, not a scalar)
     * falls through on the desc probe. */
    if (!is_arrow && elem_ty && elem_ty[0] && strcmp(meth_name, "map") == 0) {
        char mbase[96];
        const ShadowDynSink* mds;
        int esz = 0, isf = 0;
        if (shadow_ufcs_map_elem_desc(elem_ty, &esz, &isf)) {
            ...
            if (mds &&
                shadow_ufcs_emit_map_rows(recv, mbase, mds->wrap, esz, isf, a,
                                          dst, cap))
                return 1;
        }
    }
