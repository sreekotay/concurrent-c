...
static int shadow_strsw_emit_head(CEmit* out, ShadowCtx* ctx, AstNode* st,
    ...
    /* Tiny tables: length+cc__bytes_cmp beats FNV MPH (JSON schema keys,
     * short command names). Larger sets keep the perfect-hash probe.
     * Do not emit memcmp — that would require string.h in every TU. */
    ...
    if (use_linear) {
        ...
        for (i = 0; i < nkeys; i++) {
            ...
            if (!cemit_fmt(out,
                           "%s  %s (%s_k.len == %s && cc__bytes_cmp(%s_p, \"%s\", %s) "
                           "== 0) %s_idx = %d;\n",
