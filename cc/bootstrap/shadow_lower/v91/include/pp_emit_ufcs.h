...
#define SHADOW_UFCS_PTR_RECV()                                                   \
    ...
    if (shadow_ufcs_emit_arena_meth(meth_name, vty, rb, recv, a,
    ...
    } else if (strcmp(meth_name, "open") == 0 ||
        ...
        /* File / socket / listener / channel — typed only; miss → fail loud. */
        if (!shadow_ufcs_emit_io_meth(meth_name, vty, rb, recv, a,
                                     is_arrow || SHADOW_UFCS_PTR_RECV(), dst,
                                     cap))
            return 0;
    ...
    /* twice/halve/fabs/strlen: scalar family + bare (system ufn seed). */
        ...
        /* On a table miss the resolver returns its argument; copying that
         * back into rty is a self-aliased snprintf (UB — glibc yields ""),
         * which silently emptied rty and dropped every `->len()` receiver
         * with no typedef alias (CC_MAP_DECL_* handles) as unlowerable. */
