...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
    ...
    /* @typeview: allow-list filter; resolve remaining methods as Base*. */
    char view_mode[64];
    view_mode[0] = 0;
    {
        char leaf[96];
        ...
        if (rr && !shadow_restrict_trusted_for(rr)) {
            if (!shadow_restrict_allows_use(rr, meth_name)) {
                fprintf(stderr,
                        "error: restricted mode '%s' on '%s' does not allow "
                        "method '%s' (flush-under-hold / capability deny)\n",
                        shadow_restrict_mode_label(rr), rr->base, meth_name);
...
#define SHADOW_UFCS_PTR_RECV() (ufcs_ptr_recv)
    ...
    /* Nursery.wait / .cancel: typed CC* invent → cc_nursery_{wait,cancel}. */
        ...
        snprintf(dst, cap, "cc_channel_send_task_hybrid(%s, %s)", recv, a);
               ...
               !shadow_ufcs_ty_uses_name_meth(vty)) {
        /* Non-family typed / untyped only — Vec/Map/ArrayMap Name_meth invent.
         * Beats the ArrayMap `store_*` family for Capitals / CC* receivers. */
        ...
        if (vty[0]) {
            ...
            if (tag[0] >= 'A' && tag[0] <= 'Z' && strncmp(tag, "CC", 2) != 0 &&
                ...
                if (callee) {
                    ...
                    /* Do not invent Tag_meth — require a declared UFCS /
                     * header symbol (same gate as Map/Vec). */
