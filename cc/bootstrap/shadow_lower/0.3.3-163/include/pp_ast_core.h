...
typedef enum {
     ...
     * @parallel [seq (cond)] wait (gate) for … — e=gate name, f=seq cond;
...
struct AstNode {
    ...
    int forced_seq;
    /* `@cache` on a declaration: contents are cold (the initializer) or
     * warm (an earlier iteration's leavings); which instance a ticket
     * sees is unobservable. Sequential paths keep the one instance. */
    int is_cache;
