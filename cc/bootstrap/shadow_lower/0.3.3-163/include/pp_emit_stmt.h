...
static void shadow_collect_binds_node(AstNode* st) {
    ...
    if (st->kind == AST_TYPED_INIT || st->kind == AST_VAL_DESTROY ||
        ...
        if (strstr(bind_ty, "cc_atomic_int") || strstr(bind_ty, "atomic"))
            flags |= SHADOW_BIND_ATOMIC;
        if (st->is_cache)
            flags |= SHADOW_BIND_CACHE;
        if (strncmp(bind_ty, "CCSlice_", 8) == 0)
            shadow_slice_need(bind_ty + 8);
        shadow_collect_bind_one(st->b, bind_ty, flags);
        if (st->is_cache) {
            int ci, hit = 0;
            for (ci = 0; ci < g_shadow_ncache_decl; ci++) {
                if (g_shadow_cache_decl[ci] == st) {
                    hit = 1;
                    break;
                }
            }
            if (!hit && g_shadow_ncache_decl < SHADOW_CACHE_DECL_CAP)
        ...
        shadow_collect_bind_one(st->b, ty,
                               st->is_cache ? SHADOW_BIND_CACHE : 0);
        if (st->is_cache) {
            int ci, hit = 0;
            for (ci = 0; ci < g_shadow_ncache_decl; ci++) {
                if (g_shadow_cache_decl[ci] == st) {
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (!cemit_fmt(out, "%sif (__cc_pw_fail_%d) {\n", i3, id))
        return 0;
