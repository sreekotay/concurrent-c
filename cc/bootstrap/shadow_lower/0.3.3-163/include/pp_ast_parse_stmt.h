...
/* `@cache Type name = init [@destroy {…}];` — storage-class mark on a
 * declaration. Sequential paths keep the one instance; a wait-for may
 * instantiate extras. The mark is not a statement of its own. */
static AstNode* parse_cache_decl(Parser* p) {
    ...
    parser_fail(p, at, "@cache must precede a declaration");
    return NULL;
}
