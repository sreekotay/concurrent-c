...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_FOR: {
        ...
        if (in_kind == 1 || in_kind == 3) {
            ...
            if (!shadow_for_lookup_extent(fi.subj[0], &ex)) {
                shadow_pw_pop_loop(ctx, pw, 0);
                return 0;
            }
            if (in_kind == 3)
            ...
            shadow_extent_len_c(&ex, fi.subj[0], len_c, sizeof(len_c));
            shadow_extent_load_c(&ex, fi.subj[0], idx, load_c, sizeof(load_c));
            if (!cemit_fmt(out, "%sfor (size_t %s = 0; %s < (%s); ++%s) {\n",
                           indent, idx, idx, len_c, idx)) {
                shadow_pw_pop_loop(ctx, pw, 0);
                return 0;
            }
            {
                ...
                shadow_bind_name(val, ex.elem, 0);
                bind_mark = shadow_push_block_binds(st->body, st->nbody);
                if (ctx) {
                    ok = shadow_emit_compound_body(st->body, st->nbody, out, ctx,
                                                   nested, use_cleanup, 1);
        ...
        if (in_kind == 4) {
            ...
            char i1[80], i2[80];
            int id = g_shadow_for_in_seq++;
            if (!shadow_for_lookup_extent(fi.subj[0], &ex_a) ||
                !shadow_for_lookup_extent(fi.subj[1], &ex_b)) {
                shadow_pw_pop_loop(ctx, pw, 0);
                return 0;
            }
            snprintf(idx, sizeof(idx), "__cc_fi_%d", id);
            shadow_extent_len_c(&ex_a, fi.subj[0], len_a, sizeof(len_a));
            shadow_extent_len_c(&ex_b, fi.subj[1], len_b, sizeof(len_b));
            shadow_extent_load_c(&ex_a, fi.subj[0], idx, load_a, sizeof(load_a));
            shadow_extent_load_c(&ex_b, fi.subj[1], idx, load_b, sizeof(load_b));
            shadow_indent_nest(i1, sizeof(i1), indent, 1);
            shadow_indent_nest(i2, sizeof(i2), indent, 2);
            if (!cemit_fmt(out,
                           "%s{\n"
                           ...
                           indent, i1, id, len_a, i1, id, len_b, i1, id, id)) {
                shadow_pw_pop_loop(ctx, pw, 0);
                return 0;
            }
            ...
            }
            {
                ...
                shadow_indent_nest(nested, sizeof(nested), indent, 2);
                if (!cemit_fmt(out, "%s%s %s = %s;\n%s%s %s = %s;\n", nested,
                               ex_a.elem, fi.bind[0], load_a, nested, ex_b.elem,
                               fi.bind[1], load_b)) {
                    shadow_pw_pop_loop(ctx, pw, 0);
                    return 0;
                }
                nbinds_mark = g_shadow_nbinds;
                shadow_bind_name(fi.bind[0], ex_a.elem, 0);
                shadow_bind_name(fi.bind[1], ex_b.elem, 0);
                bind_mark = shadow_push_block_binds(st->body, st->nbody);
                if (ctx) {
                    ok = shadow_emit_compound_body(st->body, st->nbody, out, ctx,
                                                   nested, use_cleanup, 1);
    ...
    case AST_ASSIGN: {
        ...
        if (shadow_emit_assign_maybe_autoblock(st, out, ctx, indent))
            return 1;
        /* LHS may be a peel plus C postfix (`s.as_ptr()[i] -= 32`). Text
