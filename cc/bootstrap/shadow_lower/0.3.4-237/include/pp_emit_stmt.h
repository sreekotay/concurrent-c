...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_VAL_DESTROY: {
        ...
        if (cl) shadow_splice_closure_arg(init, sizeof(init), cl);
        if (has_bang) {
            /* Result: field unwrap + @errhandler. Plain values (infallible
             * create hook, cc_adopt): identity — bang is vacuous / null-check
             * folklore. Nursery births are `T !>(E)` — still unwrap. */
            char ty_lhs[160];
            ...
            if (is_ptr)
                ...
                snprintf(ty_lhs, sizeof(ty_lhs), "%s", st->a);
            if (chook && chook[0] && strstr(init, chook) != NULL &&
                strcmp(st->a, "CCNursery") != 0)
                plain = 1;
    ...
    case AST_FOR: {
        ...
        char nested[80];
        shadow_indent_nest(nested, sizeof(nested), indent, 1);
        /* Same as AST_BLOCK: bind body locals so UFCS sees the receiver type. */
