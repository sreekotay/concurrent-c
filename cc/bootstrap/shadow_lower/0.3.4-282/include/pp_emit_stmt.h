            ...
            if (pars[i]->e[0]) {
                ...
                if (!g_shadow_pw_sh_typed) {
                    if (!cemit_str(out,
                            ...
                            "CCNurseryHost* nur; CCChan* work; } __cc_pw_sh_t;\n"
...
static int shadow_emit_par_spawn_body(
    ...
    for (a = spawn0; a < n; a++) {
        ...
        if (!shadow_par_emit_cap_pack(dest, bi, dot, cap_names[a], ncaps[a],
                                      via_outs))
            return 0;
        if (via_outs) {
        ...
        if (!cemit_fmt(dest, "%s%s__cc_pj_ex = __cc_pj_ex;\n", bi, dot))
            return 0;
        /* Inline deny gate: once the runtime classifies this site as
         * churn, the deny verdict is taken here without the cross-TU
         * spawn call; the invalid task routes the arm through the same
         * run-inline join fallback a runtime denial uses. */
        if (heap) {
            if (!cemit_fmt(dest,
...
static int shadow_emit_parallel_for(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (!cemit_fmt(out,
            ...
            i1, id, id, i1, id, id, i1, id, lo, hi, id))
        return 0;
    {
        const char* rty = (ctx && ctx->soft_ret_ty && ctx->soft_ret_ty[0] &&
                           strcmp(ctx->soft_ret_ty, "void") != 0)
