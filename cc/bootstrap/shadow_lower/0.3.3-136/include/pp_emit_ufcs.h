...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
     ...
     * (transparent — arm binds are registered on the outer schema type). */
    int path_is_ptr = 0;
    int walked_fields = 0;
    if (vty[0] && recv) {
        const char* p = recv;
        ...
        for (;;) {
            ...
            fname[ni] = 0;
            if (!fname[0]) break;
            /* Variant union / arm tags: stay on the outer type so arm binds
             * (`framed.args`) resolve against the schema field table. */
