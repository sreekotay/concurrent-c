...
static int span_text(Parser* p, int i0, int i1_excl, char* dst, size_t cap) {
    ...
    if (!p || i0 < 0 || i1_excl > p->n || i0 >= i1_excl) return 1;
    {
        ...
        /* Include trivia before the excluded delimiter (`)` / `;` / …).
         * Call args that end on a cpp-transparent `#endif` must keep the
         * newline before `)` — otherwise emit glues `#endif)`. */
        if (same_file && i1_excl < p->n) {
            Token nx = p->toks[i1_excl];
            if (nx.file_id == a.file_id && nx.offset >= end &&
                nx.offset <= end + 512u && ft && ft->bytes &&
                nx.offset <= ft->len) {
                size_t k;
                int only_ws = 1;
                for (k = end; k < nx.offset; k++) {
                    char c = ft->bytes[k];
                    if (c != ' ' && c != '\t' && c != '\n' && c != '\r') {
                        only_ws = 0;
                        break;
...
static int ast_spell_token_range(Parser* p, int start, int end, char* dst,
    ...
    /* Trailing trivia before the excluded end token (same `#endif)` case). */
    if (have_prev && end < p->n) {
        Token nx = p->toks[end];
        if (ast_tok_verbatim(p, nx) && nx.offset >= prev_end)
            (void)ast_copy_ws_gap(p, nx, prev_end, nx.offset, dst, &o, cap);
    }
    return 1;
}
