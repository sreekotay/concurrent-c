...
static int shadow_emit_async_fn(AstNode* it, CEmit* out, TapeCache* cache) {
    ...
    int npars = 0;
    if (!it || !out) return 0;
    snprintf(params, sizeof(params), "%s", it->c && it->c[0] ? it->c : "void");
    ...
    is_void = (strcmp(ast_slot(it->a), "void") == 0);
    is_result_ret = (strncmp(ast_slot(it->a), "CCResult_", 9) == 0) ||
                    (it->e && it->e[0] && strcmp(ast_slot(it->e), "result") == 0);
    ...
    for (k = 0; k < it->nkids; k++) {
        if (!shadow_collect_spawns(it->kids[k], spawns, &nspawns, 16)) {
            shadow_err(NULL, it, "too many spawns in @async for shadow");
            out->err = 1;
            shadow_pop_fn_binds(bind_mark);
            return 0;
        }
        if (!shadow_collect_parallels(it->kids[k], pars, &npars, 16)) {
            shadow_err(NULL, it, "too many @parallel in @async for shadow");
            out->err = 1;
            shadow_pop_fn_binds(bind_mark);
            return 0;
        }
    ...
    for (k = 0; k < nspawns; k++) shadow_cap_collect_closure_locals(spawns[k]);
    ...
    if (!shadow_emit_par_protos(out, pars, npars)) {
        shadow_pop_fn_binds(bind_mark);
        return 0;
    }
    ...
    }
    {
        ...
        if (it->d && it->d[0] && it->nkids == 0) {
            ...
            for (k = 0; bind_ok && k < it->nkids; k++) {
                if (!shadow_collect_scope_defers(&it->kids[k], 1, defers,
                                                 &ndefers, 16)) {
                    shadow_err(NULL, it, "too many @defer in @async body");
                    out->err = 1;
                    bind_ok = 0;
                    break;
                }
            ...
            if (bind_ok) {
                ...
                sctx.soft_ret_ty = is_void ? "void" : (it->a && it->a[0] ? it->a : "int");
                /* cc_ok/cc_err rewrite keys off rname; without it, Result
                 * bodies default to CCResult_int_CCError (bool!>/… broke). */
                if (is_result_ret && it->a && it->a[0]) sctx.rname = it->a;
                ...
                if (sctx.defer_cleanup) {
                    if (is_void) {
                        if (!cemit_fmt(&body,
                            ...
                            "%sint __cc_ret_err = 0;\n"
                            "%sint __cc_defer_hw = 0;\n",
                            body_ind, body_ind, body_ind, body_ind))
                        bind_ok = 0;
                    if (bind_ok &&
                        !shadow_emit_named_defer_flags(&body, defers, ndefers,
                                                       body_ind))
                        bind_ok = 0;
                }
                if (bind_ok &&
                ...
                for (k = 0; bind_ok && k < it->nkids; k++) {
                    ...
                    if (bind_ok && sctx.defer_cleanup &&
                        !shadow_emit_hw_bump(&body, &sctx, it->kids[k],
                                             body_ind))
                        bind_ok = 0;
                }
                if (bind_ok &&
                    !shadow_eh_emit_trailers(&body, &sctx, body_ind,
        ...
        if (!shadow_flush_par_helpers(out)) {
            shadow_pop_fn_binds(bind_mark);
            return 0;
        }
        shadow_pop_fn_binds(bind_mark);
    ...
    /* Poll/drop/task-make: pin every physical line to the @async fn site so
     * cpp does not walk past the body's last `#line` toward source EOF. */
    {
        CEmit syn = {0};
        ShadowCtx pin = { .cache = cache, .site = it };
        int ok = 1;
        if (!cemit_fmt(&syn,
        ...
        if (ok && !cemit_str(&syn,
            ...
            ok = 0;
        if (ok && is_void) {
            if (!cemit_fmt(&syn, "      %s_body(__f);\n      __f->__r = 0;\n",
                           base))
                ok = 0;
        } else if (ok && is_result_ret) {
            if (!cemit_fmt(&syn,
                    "      {\n"
                    "        %s __cc_ar = %s_body(__f);\n"
                    "        %s* __cc_arp = (%s*)cc__heap_alloc(sizeof(*__cc_arp));\n"
                    "        if (__cc_arp) *__cc_arp = __cc_ar;\n"
                    "        __f->__r = (intptr_t)(void*)__cc_arp;\n"
                    "      }\n",
                    it->a, base, it->a, it->a))
                ok = 0;
        } else if (ok && !cemit_fmt(&syn,
                                    "      __f->__r = (intptr_t)%s_body(__f);\n",
                                    base)) {
            ok = 0;
        }
        if (ok && !cemit_str(&syn,
                ...
                "}\n\n"))
            ok = 0;
        if (ok && !cemit_fmt(&syn, "static void %s_drop(void* __p) {\n", base))
            ok = 0;
        if (ok && !cemit_fmt(&syn, "  %s_frame* __f = (%s_frame*)__p;\n", base,
                             base))
            ok = 0;
        if (ok && !cemit_str(&syn,
                "  if (!__f) return;\n"
                ...
                "}\n\n"))
            ok = 0;
        if (ok && !cemit_fmt(&syn, "CCTaskIntptr %s(%s) {\n", it->b, params))
            ok = 0;
        if (ok && !cemit_fmt(&syn,
                "  %s_frame* __f = (%s_frame*)cc__heap_calloc(1, sizeof(%s_frame));\n",
                base, base, base))
            ok = 0;
        if (ok && !cemit_str(&syn,
                "  if (!__f) {\n"
        ...
        if (ok && !cemit_fmt(&syn,
            ...
            ok = 0;
        if (ok && syn.buf)
            ok = shadow_emit_pinned_block(out, &pin, it, "", syn.buf);
        free(syn.buf);
        if (!ok) return 0;
    }
    return 1;
