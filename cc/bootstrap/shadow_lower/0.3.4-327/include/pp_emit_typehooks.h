...
static int g_shadow_ufcs_did_register;
...
/* Compile `.ufcs` for project TU / local .cch, and for stdlib postlude.cch
 * (the appendix that holds hooks so decl headers stay collect-only). */
static int shadow_ufcs_compile_for_path(const char* path) {
    ...
    /* `postlude.cch` and `postlude.h` — do not write a `.cch"` literal;
     * header extract rewrites that to `.h"`. */
    if (strncmp(base, "postlude.", 9) == 0) return 1;
    if (strstr(path, "/out/include/ccc/") || strstr(path, "out/include/ccc/"))
        return 0;
    if (strstr(path, "/cc/include/ccc/") || strstr(path, "cc/include/ccc/"))
        return 0;
    if (strstr(path, "/include/ccc/") || strstr(path, "include/ccc/"))
        return 0;
    if (strncmp(path, "ccc/", 4) == 0) return 0;
    n = strlen(path);
    if (n >= 2 && path[n - 2] == '.' && path[n - 1] == 'h') return 0;
    return 1;
}

static int shadow_ufcs_rel_wants_hook_compile(const char* rel) {
    char alt[320];
    if (!rel || !rel[0]) return 0;
    if (shadow_ufcs_compile_for_path(rel)) return 1;
    /* `ccc/cc_arena.cch` etc. are already compile=0; do not strip the prefix
     * and re-probe bare names (that would compile all stdlib `.ufcs` hooks). */
    if (strncmp(rel, "ccc/", 4) == 0) return 0;
    snprintf(alt, sizeof(alt), "ccc/%s", rel);
    return shadow_ufcs_compile_for_path(alt);
}

static void shadow_ufcs_seen_key(const char* path, char* out, size_t cap) {
    size_t n;
    if (!out || !cap) return;
    out[0] = 0;
    if (!path || !path[0]) return;
    /* Lowered `.h` and live `.cch` are one hook site; `.h` scans must not
     * block a later registered-source `.cch` compile pass. */
    n = strlen(path);
    if (n > 2 && path[n - 2] == '.' && path[n - 1] == 'h') {
        snprintf(out, cap, "%.*s.cch", (int)(n - 2), path);
        return;
    }
    snprintf(out, cap, "%s", path);
}

static int shadow_ufcs_path_seen(const char* path, int compile_ufcs) {
    int i;
    char key[256];
    shadow_ufcs_seen_key(path, key, sizeof(key));
    if (!key[0]) return 0;
    for (i = 0; i < g_shadow_ufcs_nseen; i++) {
        if (strcmp(g_shadow_ufcs_seen[i].path, key) == 0) {
            if (compile_ufcs && !g_shadow_ufcs_seen[i].ufcs_compiled) return 0;
            return 1;
        }
    }
    if (g_shadow_ufcs_nseen >= SHADOW_UFCS_SEEN_CAP) return 1;
    snprintf(g_shadow_ufcs_seen[g_shadow_ufcs_nseen].path,
             sizeof(g_shadow_ufcs_seen[0].path), "%s", key);
    g_shadow_ufcs_seen[g_shadow_ufcs_nseen].ufcs_compiled = 0;
    g_shadow_ufcs_nseen++;
    return 0;
}

static void shadow_ufcs_path_mark_compiled(const char* path) {
    int i;
    char key[256];
    shadow_ufcs_seen_key(path, key, sizeof(key));
    if (!key[0]) return;
    for (i = 0; i < g_shadow_ufcs_nseen; i++) {
        if (strcmp(g_shadow_ufcs_seen[i].path, key) == 0) {
            g_shadow_ufcs_seen[i].ufcs_compiled = 1;
            return;
        }
    }
}
...
static void shadow_ufcs_hooks_load(const char* path, const char* collect_src,
    ...
    if (shadow_ufcs_path_seen(path, compile_ufcs)) return;
    g_shadow_ufcs_did_register = 0;
    ...
    if (compile_ufcs && !g_shadow_ufcs_miss && g_shadow_ufcs_did_register)
        shadow_ufcs_path_mark_compiled(path);
...
/* The header scan reads one line at a time and expects one field per
 * line. A struct written on one line (`typedef struct C { Vec::[int] a; } C;`)
 * would register nothing, so struct bodies are re-laid out first: inside a
 * `struct ... {` body, `{` and `;` end a line and `}` starts one. Text
 * outside struct bodies, comments and string literals are copied as is. */
static char* shadow_struct_lines_normalize(const char* text) {
    size_t n = text ? strlen(text) : 0;
    /* Worst case every byte gains a newline. */
    char* out = (char*)malloc(n * 2 + 1);
    size_t o = 0;
    size_t i = 0;
    int depth = 0;          /* brace depth inside a struct body; 0 = outside */
    int pending_open = 0;   /* saw `struct` / `union` on this line, `{` not yet */
    if (!out) return NULL;
    if (!text) { out[0] = 0; return out; }
    while (i < n) {
        char c = text[i];
        /* Comments and literals pass through untouched. */
        if (c == '/' && text[i + 1] == '*') {
            const char* e = strstr(text + i + 2, "*/");
            size_t len = e ? (size_t)(e + 2 - (text + i)) : n - i;
            memcpy(out + o, text + i, len);
            o += len; i += len;
            continue;
        }
        if (c == '/' && text[i + 1] == '/') {
            const char* e = strchr(text + i, '\n');
            size_t len = e ? (size_t)(e - (text + i)) : n - i;
            memcpy(out + o, text + i, len);
            o += len; i += len;
            continue;
        }
        if (c == '"' || c == '\'') {
            ...
            if (j < n && text[j] == c) j++;
            memcpy(out + o, text + i, j - i);
            o += j - i; i = j;
            continue;
        }
        if (depth == 0) {
            if ((c == 's' && strncmp(text + i, "struct", 6) == 0 && !shadow_is_ident_char(text[i + 6]) &&
                 (i == 0 || !shadow_is_ident_char(text[i - 1]))) ||
                (c == 'u' && strncmp(text + i, "union", 5) == 0 && !shadow_is_ident_char(text[i + 5]) &&
                 (i == 0 || !shadow_is_ident_char(text[i - 1]))))
                pending_open = 1;
            else if (c == '\n' || c == ';')
                pending_open = 0;
            else if (c == '{' && pending_open) {
                depth = 1;
                pending_open = 0;
                out[o++] = '{';
                out[o++] = '\n';
        ...
        if (c == '{') {
            ...
            out[o++] = ';';
            /* Only add a break when more text follows on this line. */
            {
                size_t j = i + 1;
...
static void shadow_as_scan_header_text(const char* text) {
    ...
    char* normalized;
    if (!text) return;
    normalized = shadow_struct_lines_normalize(text);
        ...
        if (in_typedef_struct) {
            ...
            if (fty[0] && *mid == '(') {
                ...
                fname[fi] = 0;
                    ...
                    /* `T name[N]` — keep [] on the type so @destroy
                     * does not treat the slot as a value embed. */
                    if (mid[fi] == '[' && ti + 2 < sizeof(fty)) {
                        fty[ti++] = '[';
                        fty[ti++] = ']';
                        fty[ti] = 0;
                    }
                    ...
                    /* A `Map::[K,V]` / `Vec::[T]` field names an instance the
                     * TU must emit before the header is included. */
                    shadow_ginst_need_from_type(fty);
...
static void shadow_as_scan_nested_incs(const char* text, int depth,
    ...
    while ((p = strstr(p, "#include")) != NULL) {
        ...
        shadow_as_scan_rel(rel, from_dir,
                           shadow_ufcs_rel_wants_hook_compile(rel), depth - 1);
        p = gt + 1;
    }
}
...
static void shadow_as_scan_pass_inc(char** pass_inc, int npass_inc,
    ...
    for (i = 0; i < npass_inc; i++) {
        ...
        shadow_as_scan_rel(rel, from_dir,
                           shadow_ufcs_rel_wants_hook_compile(rel), 2);
    }
}
...
static void shadow_as_scan_tapes(TapeCache* cache) {
    ...
    for (i = 0; i < cache->n; i++) {
        ...
        shadow_ufcs_hooks_collect_text(ft->path, text, tlen,
                                       shadow_ufcs_rel_wants_hook_compile(ft->path));
        free(text);
    }
}
...
static void shadow_as_scan_included_cch(void) {
    ...
    for (i = 0; i < n; i++) {
        ...
        shadow_as_scan_header_text(text);
        shadow_ufcs_hooks_collect_text(path, text, tlen,
                                       shadow_ufcs_compile_for_path(path));
        free(text);
    }
}
