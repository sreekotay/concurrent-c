...
static AstNode* parse_fn(Parser* p) {
    ...
    gret[0] = 0;
    {
        ...
        /* Owner splice destatics `static long long f` → `long long f`. */
        if (!has_struct &&
...
static const char* shadow_par_handle_dest(AstNode* n) {
    int k;
    if (!n || n->kind != AST_PARALLEL) return NULL;
    for (k = 0; k < n->ndbody; k++) {
        AstNode* d = n->dbody[k];
        if (d && d->kind == AST_VAR_DECL && d->b[0])
            return d->b;
 ...
 * promise. Wait-for bool dests live on AST_PARALLEL_FOR. */
static int parallel_dest_is_handle(AstNode* n) {
    int k;
    if (!n || n->kind != AST_PARALLEL) return 0;
    for (k = 0; k < n->ndbody; k++) {
        AstNode* d = n->dbody[k];
        if (d && d->kind == AST_VAR_DECL && d->b[0])
            return 1;
