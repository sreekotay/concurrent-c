...
static int shadow_ab_arg_pack_kind(const char* callee, int argi,
    ...
    /* String / char lits must refuse before header fnparam rows: a known
     * `const char*` would otherwise pack `(intptr_t)("hi")` — the exact
     * silent cast `autoblock_string_arg_fail` guards. */
    if (*p == '"' || *p == '\'') return -1;
    fp = callee ? shadow_fnparam_lookup(callee, argi) : NULL;
    if (fp && fp->base[0]) {
