...
static int shadow_par_emit_serial_thunk(int id, int pj_id, const char* lhs_ty,
    ...
    if (!shadow_par_emit_cap_spills(h, "    ", "e", names, tys, ncaps))
        return 0;
    if (!shadow_par_emit_serial_via(arm, &t, ctx, "    ", dest, lhs_ty,
...
static int shadow_pj_emit_after(CEmit* out, ShadowCtx* ctx, const char* indent,
    ...
    if (pr < 0) {
        if (rty && ctx && (ctx->goto_cleanup || ctx->defer_cleanup ||
                           ctx->defer_depth > 0)) {
            if (!shadow_emit_soft_goto_cleanup(out, ctx, indent, rvname, NULL,
                                               st ? st->tok_off : 0))
                return 0;
        } else if (rty) {
...
static int shadow_emit_par_spawn_body(
    ...
    for (a = 1; a < n; a++) {
        ...
        if (!shadow_par_emit_cap_pack(dest, bi, dot, cap_names[a], ncaps[a],
            ...
            return 0;
        if (via_outs) {
            if (!cemit_fmt(dest, "%s__cc_par_e_%d.out = __cc_par_o_%d;\n", bi,
                           id, a))
                return 0;
        } else if (!cemit_fmt(dest, "%s__cc_par_e_%d.out = &(%s);\n", bi, id,
                              lhs[a]))
            return 0;
        if (!cemit_fmt(dest, "%s__cc_par_e_%d.__cc_pj_ex = __cc_pj_ex;\n", bi,
                       id))
            return 0;
        if (!cemit_fmt(dest,
    ...
    }
    {
        ...
        if (st && st->body[0] && st->body[0]->kind == AST_SERIAL) {
            if (via_outs) {
                ...
                CEmit t = {0};
                if (!shadow_par_emit_serial_via(st->body[0], &t, ctx, bi,
                                                lhs[0], lhs_ty[0],
                                                "__cc_par_o_0",
                                                use_cleanup)) {
...
static int shadow_emit_parallel(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    for (a = 0; a < n; a++) {
        ...
        b = shadow_bind_lookup(lhs[a]);
        if (!b || !b->ty[0]) {
            fprintf(stderr, "error: @parallel target '%s' has no known type\n",
                    lhs[a]);
            out->err = 1;
            return 0;
        }
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (g_shadow_pw_rv_ty[0]) {
        ...
        if (ctx->goto_cleanup || ctx->defer_cleanup ||
            (ctx->defer_depth > 0)) {
            if (!shadow_emit_soft_goto_cleanup(out, ctx, i4, rvname, NULL,
                                               st->tok_off))
                return 0;
...
static int shadow_emit_parallel_for(AstNode* st, CEmit* out, ShadowCtx* ctx,
        ...
        return 0;
    {
        ...
        if (pr < 0) {
            if (rty && ctx && (ctx->goto_cleanup || ctx->defer_cleanup ||
                               ctx->defer_depth > 0)) {
                if (!shadow_emit_soft_goto_cleanup(out, ctx, i1, rvname, NULL,
                                                   st->tok_off))
                    return 0;
            } else if (rty) {
                if (!cemit_fmt(out, "%s    return %s;\n", i1, rvname))
