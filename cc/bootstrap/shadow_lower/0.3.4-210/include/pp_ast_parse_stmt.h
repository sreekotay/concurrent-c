...
/* @cancel name; — disarm the live named @defer. Name is required. */
static AstNode* parse_cancel(Parser* p) {
    Token name;
    AstNode* n;
    if (!tok_eq(p_peek(p), TK_PUNCT, "@")) return NULL;
    if (p->i + 1 >= p->n || p->toks[p->i + 1].kind != TK_IDENT ||
        !spell_eq(p->toks[p->i + 1].spell, "cancel"))
        return NULL;
    p_next(p); /* @ */
    p_next(p); /* cancel */
    if (p_peek(p).kind != TK_IDENT) {
        parser_fail(p, p_peek(p), "@cancel requires a name");
        return NULL;
    }
    name = p_next(p);
    n = ast_new(p, AST_CANCEL);
    if (!n) return NULL;
    slice_to(n->a, sizeof(n->a), name.spell);
    if (!p_accept(p, TK_PUNCT, ";")) {
        parser_fail(p, p_peek(p), "expected ';' after @cancel");
        return NULL;
    }
    return n;
}
