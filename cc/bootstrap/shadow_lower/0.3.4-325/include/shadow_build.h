...
/* True when bin exists and is not older than tu.o / runtime.o. */
static int shadow_bin_fresh(const char* bin_path, const char* o_path,
                            const char* runtime_o) {
    ShadowFileSig bin, obj, rt;
    if (shadow_stat_sig(bin_path, &bin) != 0 || bin.size <= 0) return 0;
    if (shadow_stat_sig(o_path, &obj) != 0) return 0;
    if (bin.mtime_sec < obj.mtime_sec) return 0;
    if (runtime_o && runtime_o[0]) {
        if (shadow_stat_sig(runtime_o, &rt) != 0) return 0;
        if (bin.mtime_sec < rt.mtime_sec) return 0;
    }
    return 1;
}
...
static int shadow_build_host(const char* in_path, const char* bin_path,
    ...
    /* Emit/obj reuse still used to relink every process. Skip when the
     * binary is already newer than tu.o and runtime.o. */
    if (cache_ok && shadow_bin_fresh(bin_path, o_path, runtime_o))
        return 0;
    {
        long long t_ld = shadow_now_ms();
