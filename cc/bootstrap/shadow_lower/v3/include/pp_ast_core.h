 ...
 * snake `family::[T]` spellings such as py_expose::[Counter]. */
...
/* Cap for body[] / dbody[] (both sized 64). `what` names the list, e.g. "block". */
enum { SHADOW_BODY_CAP = 64 };
...
/* From libshadow_comptime (emit_plan). Weak stubs so spike TUs that do not
 * link the comptime engine still resolve; strong defs win when linked. */
__attribute__((weak)) size_t cc_emit_plan_comptime_fragment_count(void) {
    return 0;
}
__attribute__((weak)) const char* cc_emit_plan_comptime_fragment_text(
    size_t frag_index) {
    (void)frag_index;
    return NULL;
}
__attribute__((weak)) int cc_emit_plan_has_generic_factory(const char* name) {
    (void)name;
    return 0;
}
__attribute__((weak)) int cc_emit_plan_generic_factory_names_csv(char* out,
                                                                 size_t cap) {
    if (out && cap) out[0] = 0;
    return 0;
}
...
static size_t span_text_len(Parser* p, int i0, int i1_excl) {
    int i;
...
static int peek_result_shape(Parser* p) {
    ...
    /* `unsigned long long` / `long long` / `unsigned int` via shared peek. */
    j = peek_c_int_type_end(p, p->i);
    if (j < 0) {
        t = p->toks[p->i];
...
static void ast_compact_type_args(Parser* p, int a0, int a1, char* compact,
    ...
    for (i = a0; i < a1 && ci + 1 < cap; ) {
        ...
        if (t.kind == TK_IDENT || shadow_kw(t) == SHADOW_KW_INT ||
            ...
            size_t n = t.spell.len;
            int stars = 0;
            /* Nested `CCVec::[int]` leaves multiple idents — join with `_`. */
    ...
    while (i < end) {
         ...
         * sync context). @create → __cc_at_create (resolved at emit). */
        {
            ...
            if (await_i >= 0) {
                 ...
                 * lower channel/task awaits to cc_block_on(intptr_t, …).
                ...
                }
                {
                    ...
                    int did = 0;
                    wrap[0] = 0;
                    /* await recv.send/recv(...) → block_on(channel_*_task). */
