...
/* Named pointer whose C methods take Host*. One `.p` peel.
 * CCNursery / CCExclusive / CCArena are handle tokens (surface takes the
 * box). Generic CCBox_* still peel. */
static int shadow_ufcs_ty_is_box(const char* vty, const ShadowBind* rb) {
    ...
    if (t && strncmp(t, "CCBox_", 6) == 0) return 1;
    return 0;
}
...
/* CCNursery is a boxed handle. Pass the token. Destroy is the slot. */
static int shadow_ufcs_nursery_slot_meth(const char* meth) {
    if (!meth) return 0;
    return strcmp(meth, "destroy") == 0;
}

static int shadow_ufcs_nursery_pass_handle(const char* vty, const ShadowBind* rb,
                                          const char* meth) {
    if (!shadow_ufcs_ty_is(vty, rb, "CCNursery")) return 0;
    return !shadow_ufcs_nursery_slot_meth(meth);
}

/* CCExclusive surface takes the box; destroy is the slot. */
static int shadow_ufcs_exclusive_slot_meth(const char* meth) {
    if (!meth) return 0;
    return strcmp(meth, "destroy") == 0;
}

static int shadow_ufcs_exclusive_pass_handle(const char* vty,
                                            const ShadowBind* rb,
                                            const char* meth) {
    if (!shadow_ufcs_ty_is(vty, rb, "CCExclusive")) return 0;
    return !shadow_ufcs_exclusive_slot_meth(meth);
}

/* CCArena is a boxed handle. Pass the token; macros run CC__ARENA_HOST.
 * Slot methods still take CCArena* so they can null the binding. */
static int shadow_ufcs_arena_slot_meth(const char* meth) {
    if (!meth) return 0;
    return strcmp(meth, "destroy") == 0 || strcmp(meth, "free") == 0 ||
           strcmp(meth, "detach") == 0 || strcmp(meth, "adopt") == 0 ||
           strcmp(meth, "try_adopt") == 0;
}
...
static int shadow_ufcs_arena_pass_handle(const char* recv, const char* vty,
                                        const ShadowBind* rb,
                                        const char* meth) {
    int is_arena = 0;
    if (recv && strstr(recv, "cc_nursery_arena(")) is_arena = 1;
    if (shadow_ufcs_ty_is_arena_handle(vty, rb)) is_arena = 1;
    if (!is_arena) return 0;
    return !shadow_ufcs_arena_slot_meth(meth);
}
    ...
    view_mode[0] = 0;
    {
        ...
        if (rr && !shadow_restrict_is_unnamed(rr) &&
            ...
            if (vty[0] && vty[strlen(vty) - 1] == '*')
                ...
                snprintf(vty, sizeof(vty), "%s", rr->base);
        } else if (shadow_ty_restrict_base(leaf, rbase, sizeof(rbase))) {
            if (vty[0] && vty[strlen(vty) - 1] == '*')
...
#define SHADOW_UFCS_PTR_RECV() (ufcs_ptr_recv)
     ...
     * unwraps it. */
    if (shadow_ufcs_arena_pass_handle(recv, vty, rb, meth_name) ||
        shadow_ufcs_nursery_pass_handle(vty, rb, meth_name) ||
        shadow_ufcs_exclusive_pass_handle(vty, rb, meth_name))
        ufcs_ptr_recv = 1;
                ...
                if (shadow_ufn_exists(hcal) ||
                    ...
                    return shadow_ufcs_fmt_call(
                        ...
                        (hook_by_val || SHADOW_UFCS_PTR_RECV() ||
                         shadow_ufcs_arena_pass_handle(recv, vty, rb,
                                                       meth_name) ||
                         shadow_ufcs_nursery_pass_handle(vty, rb,
                                                         meth_name) ||
                         shadow_ufcs_exclusive_pass_handle(vty, rb,
                                                           meth_name))
                            ? ""
                            : "&",
                        recv, a);
            ...
            if (tag[0] && tag[0] >= 'A' && tag[0] <= 'Z' &&
                ...
                const char* amp =
                    (is_arrow || SHADOW_UFCS_PTR_RECV() ||
                     shadow_ufcs_arena_pass_handle(recv, vty, rb, meth_name) ||
                     shadow_ufcs_nursery_pass_handle(vty, rb, meth_name) ||
                     shadow_ufcs_exclusive_pass_handle(vty, rb, meth_name))
                        ? ""
                        : "&";
...
static int shadow_ufcs_try_as_retry_on(const char* recv, const char* meth_name,
    ...
    for (ai = 0; ai < g_shadow_nas; ai++) {
        ...
        /* Value embed: inner UFCS is always '.'. Accessor peels have no
         * bind — hint the face target so CCArena.alloc still composes. */
        {
            const char* saved_hint = g_shadow_ufcs_hint_vty;
            g_shadow_ufcs_hint_vty = as->target;
            ok = shadow_ufcs_lower_parts(peeled, meth_name, args, 0, elem_ty,
                                         cand, sizeof(cand));
            g_shadow_ufcs_hint_vty = saved_hint;
        }
        if (!ok)
            ok = shadow_ufcs_try_as_retry_on(peeled, meth_name, args, 0,
                                             elem_ty, as->target, cand,
                                             sizeof(cand));
        if (!ok) continue;
        if (nhit) {
            fprintf(stderr,
