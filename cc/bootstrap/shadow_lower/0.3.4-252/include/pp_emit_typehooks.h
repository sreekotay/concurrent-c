...
static void shadow_as_scan_typedef_typeviews(const char* text) {
    ...
    while (*p) {
        ...
        alias[alias_n] = 0;
        (void)shadow_restrict_register(base, mode, body);
        if (stars)
...
static int shadow_cast_is_factory_alias(const char* dest) {
    return dest && (strcmp(dest, "CCArena") == 0 ||
                    strcmp(dest, "CCNursery") == 0 ||
                    strcmp(dest, "CCExclusive") == 0);
}
...
static int shadow_for_parse_in(const char* h, ShadowForIn* fi) {
    ...
    for (i = 0; i < fi->nbind; i++) {
        if (!shadow_extent_is_ident(cells[i])) {
            fprintf(stderr, "error: for-in binder '%s' is not a name\n",
                    cells[i]);
            g_shadow_ufcs_miss = 1;
            return -1;
        }
        snprintf(fi->bind[i], sizeof(fi->bind[i]), "%s", cells[i]);
    }
    ...
    fprintf(stderr,
            "error: for-in is `for (v in s)`, `for (i, v in s)`, or "
            "`for (a, b in s, t) { ... } !>;`\n");
    g_shadow_ufcs_miss = 1;
    return -1;
}
...
/* Name or field path off a bind (`t->words`, `t.words`). Not a call. */
static int shadow_for_subj_ty(const char* expr, char* ty, size_t tcap,
    ...
    const ShadowBind* sb;
    int stars = 0;
    if (!expr || !ty || !tcap || !array_flag) return 0;
    ty[0] = 0;
    *array_flag = 0;
    p = expr;
    while (*p == ' ' || *p == '\t') p++;
...
static int shadow_for_lookup_extent(const char* name, ShadowExtent* ex) {
    ...
    if (!shadow_for_subj_ty(name, subj_ty, sizeof(subj_ty), &array_flag)) {
        if (!shadow_extent_is_ident(name))
            fprintf(stderr,
                    "error: for-in subject must be a name or field path "
                    "(`s`, `t->words`)\n");
        else
