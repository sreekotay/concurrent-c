...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    if (st->g[0]) shadow_bind_name(st->g, "int", 0);
    ...
    if (st->forced_seq) {
        ...
        if (!cemit_fmt(out, "%sint %s;\n", i1, st->a))
            goto fail;
        if (st->g[0] && !cemit_fmt(out, "%sint %s = 0;\n", i1, st->g))
            goto fail;
    ...
    if (!cemit_fmt(h,
            ...
            st->a))
        goto fail;
    if (st->g[0] && !cemit_fmt(h, "    int %s = __cc_pw_slot;\n", st->g))
        goto fail;
    for (k = 0; k < nstages; k++) {
...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_VAL_DESTROY: {
        ...
        char raw[2048];
        int is_ptr = (st->d[0] == '*');
        int has_bang = (st->f[0] && strstr(st->f, "bang") != NULL);
