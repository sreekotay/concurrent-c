...

    {
        ...
        if (bang_at >= 0) {
            ...
            if (p_accept(p, TK_PUNCT, "(")) {
                ...
                if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
                    ...
                    while (!tok_eq(p_peek(p), TK_PUNCT, "}") &&
                        ...
                        if (eh->nbody >= SHADOW_BODY_CAP) {
                            parser_fail_body_cap(p, p_peek(p),
                                                 "!>(...) body");
                            return NULL;
                        }
                        eh->body[eh->nbody++] = st;
                ...
                if (n->ndbody >= SHADOW_DBODY_CAP) {
                    parser_fail(p, name, "too many create unwrap attachments");
                    return NULL;
                }
                n->dbody[n->ndbody++] = eh;
            }
    ...
    }
    {
        ...
        if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
            ...
            while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF &&
                ...
                if (!s) return 0;
                if (eh->nbody >= SHADOW_BODY_CAP) {
                    parser_fail_body_cap(p, p_peek(p), "for-in !> body");
                    return 0;
                }
                eh->body[eh->nbody++] = s;
            }
        ...
        if (n->ndbody >= SHADOW_DBODY_CAP) {
            parser_fail(p, at, "too many for-in attachments");
            return 0;
        }
        n->dbody[n->ndbody++] = eh;
        snprintf(n->e, sizeof(n->e), "zip_bang");
...
static int ast_attach_ufcs_kid(Parser* p, AstNode* parent, AstNode* ue) {
    ...
    if (parent->ndbody >= SHADOW_DBODY_CAP) return 0;
