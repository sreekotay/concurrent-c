...
static int shadow_par_chan_meth(const char* m) {
    ...
    if (strcmp(m, "send") == 0 || strcmp(m, "send_take") == 0 ||
        strcmp(m, "try_send") == 0 || strcmp(m, "send_task") == 0 ||
        strcmp(m, "send_task_hybrid") == 0)
        return CC__PAR_CH_SEND;
    if (strcmp(m, "recv") == 0 || strcmp(m, "try_recv") == 0 ||
        strcmp(m, "ordered_recv") == 0)
...
static int shadow_par_chan_text(const char* s) {
    ...
    if (strstr(s, ".send(") || strstr(s, "->send(") ||
        ...
        strstr(s, "send_take") || strstr(s, "try_send") ||
        strstr(s, "send_task"))
        op |= CC__PAR_CH_SEND;
    if (strstr(s, ".recv(") || strstr(s, "->recv(") ||
        strstr(s, "cc_chan_recv") || strstr(s, "chan_recv") ||
        strstr(s, "try_recv") || strstr(s, "ordered_recv"))
        op |= CC__PAR_CH_RECV;
    return op;
}
...
/* Channel in this join: any arm op, or a captured channel (helpers).
 * Deny of a rendezvous sibling is a silent hang — spawn instead.
 * CPU trees have no channel and still adapt. */
...
static int shadow_par_join_never_deny(AstNode* st, int n,
                                      int ncaps[],
...
static int shadow_emit_par_spawn_body(
    ...
    int never_deny = shadow_par_join_never_deny(st, n, ncaps, cap_tys);
    if (hdest && hdest[0]) {
