...
static int shadow_ufcs_try_as_retry(const char* recv, const char* meth_name,
    ...
    for (ai = 0; ai < g_shadow_nas; ai++) {
        ...
        if (shadow_ufcs_recv_is_ident(recv))
            n = snprintf(peeled, sizeof(peeled), ptr ? "%s->%s" : "%s.%s", recv,
                         as->field);
        else
            n = snprintf(peeled, sizeof(peeled), ptr ? "(%s)->%s" : "(%s).%s",
                         recv, as->field);
        ...
        ok = shadow_ufcs_lower_parts(peeled, meth_name, args, 0, elem_ty, cand,
                                     sizeof(cand));
        if (!ok)
            ok = shadow_ufcs_try_as_retry(peeled, meth_name, args, 0, elem_ty,
                                          cand, sizeof(cand));
        if (!ok) continue;
        if (nhit) {
            fprintf(stderr,
