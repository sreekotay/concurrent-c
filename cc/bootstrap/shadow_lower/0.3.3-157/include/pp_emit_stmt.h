...
static int shadow_emit_par_protos(CEmit* out, AstNode** pars, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (pars[i]->kind == AST_PARALLEL_FOR) {
            if (pars[i]->e[0]) {
                /* wait (gate) form: one ordered-spawn thunk.
                 * Shared cell type is loop-invariant (one gate, one nursery). */
                if (!g_shadow_pw_sh_typed) {
                    if (!cemit_str(out,
                            "typedef struct { CCTurnstile* gate; "
                            "CCNursery* nur; } __cc_pw_sh_t;\n"))
                        return 0;
                    g_shadow_pw_sh_typed = 1;
                }
                if (!cemit_fmt(out, "static void* __cc_pw_thunk_%d(void*);\n",
                               id))
                    return 0;
            } else if (!cemit_fmt(out,
...
static int shadow_par_emit_assign_thunk(int id, const char* lhs_ty,
    ...
    if (!cemit_fmt(h, "        %s* out;\n"
        ...
        return 0;
    for (i = 0; i < ncaps; i++) {
        if (!cemit_fmt(h, "    %s %s = e->%s;\n", tys[i], names[i], names[i]))
            return 0;
    }
    return cemit_fmt(h, "    *e->out = %s;\n    return NULL;\n}\n", rhs);
}
...
static int shadow_par_emit_serial_thunk(int id, const char* lhs_ty,
    ...
    if (!shadow_par_emit_serial_via(arm, h, ctx, "    ", dest, lhs_ty, "e->out",
                                    use_cleanup))
        return 0;
...
static int shadow_par_go_fmt_params(char* dst, size_t cap, char lhs[][128],
    ...
    for (a = 0; a < ncaps; a++) {
        int r = snprintf(dst + o, cap - o, ", %s %s", cap_tys[a],
                         cap_names[a]);
        if (r < 0 || (size_t)r >= cap - o) return 0;
        o += (size_t)r;
    }
...
static int shadow_emit_par_spawn_body(
    ...
    for (a = 1; a < n; a++) {
        ...
        for (i = 0; i < ncaps[a]; i++) {
            if (!cemit_fmt(dest, "%s__cc_par_e_%d.%s = %s;\n", bi, id,
                           cap_names[a][i], cap_names[a][i]))
                return 0;
        }
        if (via_outs) {
    ...
    if (st && st->body[0] && st->body[0]->kind == AST_SERIAL) {
        if (via_outs) {
            if (!shadow_par_emit_serial_via(st->body[0], dest, ctx, bi, lhs[0],
        ...
        if (!cemit_fmt(dest, "%s*__cc_par_o_0 = %s;\n", bi, rhs[0])) return 0;
...
static int shadow_emit_parallel(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (st->e[0]) {
        ...
        if (shadow_par_go_collect(st, glhs, glty, &nlhs, gnames, gtys, &ng) &&
            ...
            for (a = 0; a < ng; a++) {
                int r = snprintf(args + ao, sizeof(args) - ao, ", %s",
                                 gnames[a]);
                if (r < 0 || (size_t)r >= sizeof(args) - ao) return 0;
                ao += (size_t)r;
            }
...
/* Per-ticket env. Loop-invariant gate + nursery live in one shared
 * cell (`wait (ts)` is one turnstile; the construct owns one nursery).
 * Header first so the join walk can read next/i/failed/err. */
static int shadow_pw_emit_env_members(CEmit* dst, const char* pad,
    ...
    if (!cemit_fmt(dst,
        ...
        return 0;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(dst, "%s%s %s;\n", pad, tys[k], names[k])) return 0;
    }
    return 1;
}
 ...
 * the handwritten nursery loop those annotations already name:
 *   seq false / no nursery → the same body as a plain for;
 *   else enter(i) on the caller, spawn, leave() after the body.
 * One shared {gate, nur} cell — `wait (ts)` is one turnstile.
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    /* Thunk body — synthetic handler captures the first error into the
     * iteration env, cancels the nursery (stop-starting), and jumps to the
     * thunk epilogue so leave() still runs. */
    snprintf(htext, sizeof(htext),
             "__cc_pw_e->err = __cc_pw_eb; "
             "__cc_pw_e->failed = 1; "
             "cc_nursery_cancel(__cc_pw_e->sh->nur); "
    ...
    /* Thunk: unpack env, run body, fall through to leave() (error jumps here). */
    if (!cemit_fmt(h, "\nstatic void* __cc_pw_thunk_%d(void* arg) {\n    struct {\n",
                   id))
        goto fail;
    ...
    if (!cemit_fmt(h,
        ...
        goto fail;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(h, "    %s %s = __cc_pw_e->%s;\n", tys[k], names[k],
                       names[k]))
            goto fail;
    }
    if (!cemit_str(h, "    {\n")) goto fail;
    if (thb.buf && thb.len && !cemit_buf(h, thb.buf, thb.len)) goto fail;
    if (!cemit_fmt(h, "    }\n__cc_pw_done_%d:;\n", id))
        goto fail;
    snprintf(htext, sizeof(htext),
             "if (!__cc_pw_e->failed) { "
    ...
    if (!shadow_emit_try_call(h, &tctx, "    ",
                              "cc_turnstile_leave(__cc_pw_e->sh->gate)",
                              "cc_turnstile_leave", "__cc_pw_eb", 1))
        goto fail;
    if (!cemit_str(h, "    return NULL;\n}\n")) goto fail;
    free(thb.buf);
    thb.buf = NULL;

    /* Caller: seq denial branch, then the ordered spawn loop. */
    ...
    if (!cemit_fmt(out, "%s__cc_pw_sh_%d.nur = __cc_pw_n_%d;\n", i2, id, id))
        return 0;
    if (!cemit_fmt(out, "%svoid* __cc_pw_head_%d = NULL;\n", i2, id))
        return 0;
    ...
    if (!cemit_fmt(out, "%sif (cc_nursery_is_cancelled(__cc_pw_n_%d)) break;\n",
                   i3, id))
        return 0;
    /* Alloc before enter: OOM takes no token. */
    if (!cemit_fmt(out,
            "%s__cc_pw_e = cc_nursery_closure_env_alloc(__cc_pw_n_%d, "
            "sizeof(*__cc_pw_e), 16);\n",
            i3, id))
        return 0;
    if (!cemit_fmt(out, "%sif (!__cc_pw_e) {\n", i3))
        return 0;
    if (!cemit_fmt(out,
            "%s    __cc_pw_ferr_%d = CC_ERROR(CC_ERR_OUT_OF_MEMORY, "
            "\"@parallel wait env\");\n",
            i3, id))
        return 0;
    ...
    if (!cemit_fmt(out, "%s    break;\n%s}\n", i3, i3))
        return 0;
    if (!cemit_fmt(out, "%s__cc_pw_e->next = NULL;\n", i3))
        return 0;
    if (!cemit_fmt(out, "%s__cc_pw_e->i = %s;\n", i3, st->a))
        return 0;
    if (!cemit_fmt(out, "%s__cc_pw_e->failed = 0;\n", i3))
        return 0;
    if (!cemit_fmt(out, "%s__cc_pw_e->sh = &__cc_pw_sh_%d;\n", i3, id))
        return 0;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(out, "%s__cc_pw_e->%s = %s;\n", i3, names[k], names[k]))
            return 0;
    }
    snprintf(htext, sizeof(htext),
    ...
    snprintf(call, sizeof(call), "cc_turnstile_enter(__cc_pw_ts_%d, %s)", id,
             st->a);
    if (!shadow_emit_try_call(out, &tctx, i3, call, "cc_turnstile_enter",
                              "__cc_pw_eb", 1))
        return 0;
    if (!cemit_fmt(out, "%s__cc_pw_e->next = __cc_pw_head_%d;\n", i3, id))
        return 0;
    if (!cemit_fmt(out, "%s__cc_pw_head_%d = __cc_pw_e;\n", i3, id))
        return 0;
    if (!cemit_fmt(out,
            "%sif (cc_nursery_spawn(__cc_pw_n_%d, __cc_pw_thunk_%d, "
            "__cc_pw_e) != 0)\n"
            "%s    (void)__cc_pw_thunk_%d(__cc_pw_e);\n"
            "%s}\n",
            i3, id, id, i3, id, i2))
        return 0;
    /* wait's int is not the CCError — env slots are. Lowest failing ticket. */
    if (!cemit_fmt(out, "%s(void)cc_nursery_wait(__cc_pw_n_%d);\n", i2, id))
        return 0;
    if (!cemit_fmt(out, "%s{\n", i2))
        return 0;
    if (!cemit_fmt(out,
            "%sstruct { void* next; int i; int failed; CCError err; } "
            "*__cc_pw_it = __cc_pw_head_%d;\n",
            i3, id))
        return 0;
    if (!cemit_fmt(out, "%swhile (__cc_pw_it) {\n", i3))
        return 0;
    if (!cemit_fmt(out,
            "%s    if (__cc_pw_it->failed &&\n"
    ...
    if (!cemit_fmt(out, "%s    }\n", i3))
        return 0;
    if (!cemit_fmt(out, "%s    __cc_pw_it = __cc_pw_it->next;\n", i3))
        return 0;
    if (!cemit_fmt(out, "%s}\n%s}\n", i3, i2))
        return 0;
        ...
        return 0;
    {
        ...
        ctx->eh = reh;
        if (!shadow_emit_handler(out, ctx, rbind, i3)) {
            ctx->eh = saved_eh;
            return 0;
        }
        ctx->eh = saved_eh;
...
static int shadow_emit_parallel_for(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (body.buf && body.len) {
        if (!cemit_buf(h, body.buf, body.len)) {
            free(body.buf);
            return 0;
        }
    }
    ...
    if (!cemit_fmt(out, "%sstruct {\n", i1)) return 0;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(out, "%s    %s %s;\n", i1, tys[k], names[k])) return 0;
    }
    if (!cemit_fmt(out, "%s    int lo;\n%s    int hi;\n%s} __cc_parfor_e_%d;\n",
                   i1, i1, i1, id))
        return 0;
    for (k = 0; k < ncaps; k++) {
        if (!cemit_fmt(out, "%s__cc_parfor_e_%d.%s = %s;\n", i1, id, names[k],
                       names[k]))
            return 0;
    }
    return cemit_fmt(out,
