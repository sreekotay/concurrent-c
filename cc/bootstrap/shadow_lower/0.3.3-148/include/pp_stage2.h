...
/* Tool umbrellas must stay at the #include site (after @grammar KEEP_*).
 * Quoted-header rewrite may have already turned `.cch` into an absolute `.h`. */
static int pp_quote_include_is_umbrella(CCSlice spell) {
    const char* p;
    size_t n, i, bl;
    const char* base;
    pp_quote_include_path(spell, &p, &n);
    ...
    for (i = 0; i < n; i++)
        if (p[i] == '/') base = p + i + 1;
    bl = n - (size_t)(base - p);
    if (bl >= 11 && strncmp(base, "c_pp_spike.", 11) == 0) return 1;
    if (bl >= 13 && strncmp(base, "shadow_build.", 13) == 0) return 1;
    return 0;
}
...
static int handle_include(Pp* pp) {
    ...
    if (tok_eq(hdr, TK_PUNCT, "<")) {
        ...
        angle[al] = 0;
        if (pp->nf == 1 && pp->root_if_depth > 0 && pp->skip_depth == 0) {
    ...
    if (!pp_quote_include_is_cc_face(hdr.spell)) {
        ...
        if (!pp_quote_include_passthrough_line(hdr.spell, line, sizeof(line)))
            return 0;
        if (pp_quote_include_is_umbrella(hdr.spell)) {
            pp_skip_line(pp);
...
static int handle_define(Pp* pp) {
    ...
    /* Inside a root-tape `#if`/`#ifdef` tree: emit the `#define` in place
