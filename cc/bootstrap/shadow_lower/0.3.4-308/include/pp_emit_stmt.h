    ...
    if (ctx && ctx->eh && ctx->eh->nbody > 0) {
        ...
        if (ctx->eh_in_trailer) {
            ...
            if (ctx->eh->b[0] && bind && bind[0] &&
                ...
                const char* ty = ctx->eh->a[0] ? ctx->eh->a : "CCError";
...
static void shadow_collect_binds_node(AstNode* st) {
    ...
    if (st->kind == AST_AT_STMT && strcmp(st->a, "typeview") == 0 &&
        st->e[0]) {
    ...
    if (st->kind == AST_TYPED_INIT || st->kind == AST_VAL_DESTROY ||
        ...
        if (st->kind == AST_VAR_DECL && st->c[0] == '[')
            flags |= SHADOW_BIND_ARRAY;
        if (st->kind == AST_TYPED_INIT || st->kind == AST_VAL_DESTROY)
            flags |= SHADOW_BIND_CONSTRUCTED;
        ...
        if (st->c[0] == '*')
            ...
            snprintf(bind_ty, sizeof(bind_ty), "%s", st->a);
        if (st->c[0] == '[') flags |= SHADOW_BIND_ARRAY;
        if (strstr(bind_ty, "cc_atomic_int") || strstr(bind_ty, "atomic"))
            flags |= SHADOW_BIND_ATOMIC;
        if (strncmp(bind_ty, "CCSlice_", 8) == 0)
            shadow_slice_need(bind_ty + 8);
        ...
        for (i = 0; i < st->nkids; i++) {
            ...
            if (f->a[0]) shadow_ginst_need_from_type(f->a);
        ...
        shadow_register_from_text(st->a);
        if (st->a[0]) shadow_ginst_need_from_type(st->a);
...
static int shadow_emit_send_task_fiber(CEmit* out, const char* indent,
    ...
    if (caps_args[0])
        return cemit_fmt(out,
            ...
            "%s} while (0);\n",
            indent, i1, cl->d[0] ? cl->d : "1", caps_args, i1, spawn_fn, i1, tx,
            i1, indent);
    return cemit_fmt(out,
        "%sdo {\n"
        ...
        "%sif (__cc_st_e != 0) { (void)cc_block_on_intptr(__cc_st_t); }\n"
        "%s} while (0);\n",
        indent, i1, cl->d[0] ? cl->d : "1", i1, spawn_fn, i1, tx, i1, indent);
}
...
static int shadow_collect_parallels(AstNode* st, AstNode** out, int* n, int cap) {
    ...
    if (st->kind == AST_PARALLEL || st->kind == AST_PARALLEL_FOR) {
        ...
        if (st->kind == AST_PARALLEL && st->nbody > 1)
            need = st->nbody - 1;
        if (*n >= cap) return 0;
        snprintf(st->d, sizeof(st->d), "%d", g_shadow_par_id);
...
static int shadow_par_arm_waits_dest(AstNode* s, const char* dest) {
    ...
    if ((s->kind == AST_UFCS_STMT || s->kind == AST_UFCS_EXPR) &&
        strcmp(s->b, "wait") == 0 && shadow_par_recv_is_name(s->a, dest))
        return 1;
    for (k = 0; k < s->nbody; k++) {
...
static int shadow_emit_parallel_onto(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (!st || !st->par_onto || !st->a[0] || st->nbody != 1 || !st->body[0])
        return 0;
    arm = st->body[0];
    hdest = st->a;
    if (st->forced_seq)
    ...
    id = st->d[0] ? atoi(st->d) : g_shadow_par_id++;
    ...
    if (!st->par_wake) {
        ...
        }
        {
            ...
            for (hk = 0; hk < st->ndbody; hk++) {
                ...
                if (d && d->kind == AST_VAR_DECL && d->b[0] && d->a[0])
                    hdty = d->a;
            }
...
static void shadow_pw_collect_labels(AstNode* s, char labs[][64], int* n,
    ...
    if (s->kind == AST_LABEL && s->a[0] && *n < cap) {
...
static int shadow_pw_emit_cache_destroys(CEmit* out, ShadowCtx* ctx,
    ...
    for (k = 0; k < ncaps; k++) {
        ...
        if (d->kind != AST_VAL_DESTROY || strcmp(d->e, "_D") != 0)
            continue;
        if (!cemit_fmt(out, "%sif (__cc_pw_cx_%s) {\n", pad, names[k]))
            return 0;
        if (!cemit_fmt(out, "%sint __cc_pw_ck;\n", pad1))
...
static void shadow_pw_make_handler(AstNode* h, AstNode* raw, const char* text) {
    ...
    snprintf(h->a, sizeof(h->a), "CCError");
    snprintf(h->b, sizeof(h->b), "__cc_pw_eb");
    raw->kind = AST_RAW_LINE;
    snprintf(raw->a, sizeof(raw->a), "%s", text);
    h->body[0] = raw;
    h->nbody = 1;
}
...
static void shadow_stage_call(AstNode* n, const AstNode* st,
    ...
    snprintf(n->a, sizeof(n->a), "%s", st->a);
    snprintf(n->b, sizeof(n->b), "%s", meth);
    snprintf(n->c, sizeof(n->c), "%s", st->b);
...
static int shadow_emit_stage_region(AstNode** body, int nbody, CEmit* dest,
    ...
    rbind = (saved_eh && saved_eh->b[0]) ? saved_eh->b : "e";
    ...
    if (!st->forced_seq) {
        ...
        if (!reh) {
            for (k = ctx->nehs - 1; k >= 0; k--) {
                ...
                if (eh && (!eh->a[0] || strcmp(eh->a, "CCError") == 0)) {
                    reh = eh;
                    break;
                }
            }
            if (!reh && ctx->eh &&
                (!ctx->eh->a[0] || strcmp(ctx->eh->a, "CCError") == 0))
                reh = ctx->eh;
        }
        if (!reh) {
...
static int shadow_emit_stmt_ctx(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    case AST_VAL_DESTROY: {
        ...
        if (has_bang) {
            ...
            if (site_eh) {
                const char* bind = site_eh->b[0] ? site_eh->b : "e";
                char i1[80], i2[80];
                ShadowTplBind env[7];
                shadow_indent_nest(i1, sizeof(i1), indent, 1);
                shadow_indent_nest(i2, sizeof(i2), indent, 2);
    ...
    case AST_UFCS_STMT: {
        ...
        char call[8192];
        {
            ...
            if (st_cl && (strcmp(st->b, "send_task") == 0 ||
                          strcmp(st->b, "send_task_hybrid") == 0)) {
                if (strncmp(st_cl->b, "callarg", 7) == 0)
                    snprintf(st_cl->b, sizeof(st_cl->b), "%s", st->b);
                ...
                strcmp(st->b, "send_task_hybrid") == 0);
        {
            ...
            if (args[0])
                return cemit_fmt(out,
                    ...
                    nrecv, st->d[0] ? st->d : "1", args);
            return cemit_fmt(out, "%s%s(%s, cc_closure__N%s_make());\n", indent,
                             nur_spawn, nrecv, st->d[0] ? st->d : "1");
        }
    }
    case AST_CALL_NUM: {
    ...
    case AST_AT_STMT:
        ...
        if (strcmp(st->a, "typeview") == 0 && st->d[0]) {
