...
static void shadow_cap_val_type(const char* name, char* dst, size_t cap) {
    ...
    /* No silent int degrade — same class as cap_binds overflow. */
    if (!g_shadow_ufcs_miss) {
        fprintf(stderr,
                "error: shadow capture '%s' has unknown type (no bind)\n",
                lookup ? lookup : (name ? name : "?"));
        g_shadow_ufcs_miss = 1;
    }
    snprintf(dst, cap, "int");
}
...
static int shadow_emit_closure_body(AstNode* cl, CEmit* out, TapeCache* cache) {
    ...
    bind_mark = g_shadow_nbinds;
    {
        ...
        while (p && *p) {
            ...
            shadow_cap_item_parts(item, cname, sizeof(cname), &init);
            if (shadow_cap_item_is_ref(item)) {
                /* Entry emits `#define name (*__cc_ref_name)` — UFCS sees T. */
