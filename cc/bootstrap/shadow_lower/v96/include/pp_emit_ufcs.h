...
static int shadow_ufcs_emit_len_cap(const char* meth, const char* vty,
    ...
    /* Prefix families before exact table (ArrayMap_CCSlicePacked_* vs packed). */
    if (strcmp(meth, "len") == 0 && rty[0]) {
        if (strncmp(rty, "CCSlice_", 8) == 0)
            return shadow_ufcs_emit_slice_meth(rty, "len", recv, NULL, dst, cap);
        if (strncmp(rty, "CCVec_", 6) == 0 || strncmp(rty, "Map_", 4) == 0 ||
            strncmp(rty, "ArrayMap_", 9) == 0 || shadow_ufcs_is_map_ty(rty)) {
            char tryc[192];
            snprintf(tryc, sizeof(tryc), "%s_len", rty);
            return shadow_ufcs_fmt_call(dst, cap, tryc,
                                       is_arrow_or_ptr ? "" : "&", recv, NULL);
        }
    }
...
/* CCVec_* methods — typed prefix only; callee is `{vty}_{meth}`. */
typedef struct {
    const char* meth;
    int needs_args;
} ShadowUfcsVecMeth;

static const ShadowUfcsVecMeth g_shadow_ufcs_vec_meth[] = {
    { "push",   1 },
    { "mean",   0 },
    { "median", 0 },
    { NULL, 0 },
...
static int shadow_ufcs_emit_vec_meth(const char* meth, const char* vty,
                                    const char* recv, const char* args,
                                    int is_arrow_or_ptr, char* dst,
                                    size_t cap) {
    char rty[160];
    ...
    if (resolved && resolved[0] && resolved != rty)
        snprintf(rty, sizeof(rty), "%s", resolved);
    if (!rty[0] || strncmp(rty, "CCVec_", 6) != 0) return 0;
    for (i = 0; g_shadow_ufcs_vec_meth[i].meth; i++) {
        const ShadowUfcsVecMeth* row = &g_shadow_ufcs_vec_meth[i];
        if (strcmp(meth, row->meth) != 0) continue;
        if (row->needs_args && !args[0]) return 0;
        snprintf(tryc, sizeof(tryc), "%s_%s", rty, meth);
        return shadow_ufcs_fmt_call(dst, cap, tryc, is_arrow_or_ptr ? "" : "&",
                                   recv, row->needs_args ? args : NULL);
    }
    return 0;
}
...
#define SHADOW_UFCS_PTR_RECV()                                                   \
    ...
    /* twice/halve/fabs/strlen: scalar family + bare (system ufn seed). */
        ...
        /* Typed len table + compiler-shape overlays; miss → fail loud. */
        if (!shadow_ufcs_emit_len_cap("len", vty, rb, recv,
                                     is_arrow || SHADOW_UFCS_PTR_RECV(), dst,
                                     cap))
            return 0;
    ...
    } else if (shadow_ufcs_emit_vec_meth(meth_name, vty, recv, a,
                                        is_arrow || SHADOW_UFCS_PTR_RECV(), dst,
                                        cap)) {
        /* CCVec_ push/mean/median — typed prefix; miss falls through. */
