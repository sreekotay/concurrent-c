...
static int shadow_pw_emit_exit(CEmit* out, ShadowCtx* ctx, const char* indent,
    ...
    if (kind > 0 &&
        !cemit_fmt(out, "%s    cc_nursery_cancel(__cc_pw_sh->nur);\n", indent))
        return 0;
...
static int shadow_pw_emit_cache_alloc(CEmit* out, const char* pad,
    ...
    for (k = 0; k < ncaps; k++) {
        ...
        if (!cemit_fmt(out,
                ...
                "%s__cc_pw_cx_%s = cc_nursery_closure_env_alloc("
...
static int shadow_emit_stage(AstNode* st, CEmit* out, ShadowCtx* ctx,
    ...
    if (ctx->pw_body == 2) {
        if (!cemit_fmt(out,
                "%sif (!cc_nursery_is_cancelled(__cc_pw_sh->nur)) {\n",
                indent))
            return 0;
    }
    if (!cemit_fmt(out, "%s{\n", indent)) return 0;
...
static int shadow_emit_parallel_wait_for(AstNode* st, CEmit* out,
    ...
    snprintf(htext, sizeof(htext),
             ...
             "__cc_pw_sl->failed = 1; "
             "__cc_pw_sl->i = %s; "
             "cc_nursery_cancel(__cc_pw_sh->nur); "
    ...
    if (!cemit_fmt(out,
            "%sif (__cc_pw_work_%d)\n"
            "%s__cc_pw_slots_%d = cc_nursery_closure_env_alloc("
    ...
    if (!cemit_fmt(out,
            ...
            "%s__cc_pw_args_%d = cc_nursery_closure_env_alloc("
            "__cc_pw_n_%d, (size_t)__cc_pw_nw_%d * "
    ...
    if (!cemit_fmt(out,
            "%sif (__cc_pw_args_%d)\n"
            "%s__cc_pw_e = cc_nursery_closure_env_alloc("
    ...
    if (!cemit_fmt(out, "%s__cc_pw_args_%d[__cc_pw_w_%d].slot = __cc_pw_w_%d;\n",
        ...
        return 0;
    if (!cemit_fmt(out,
            "%sif (cc_is_ok(cc_nursery_spawn(__cc_pw_n_%d, __cc_pw_worker_%d, "
    ...
    if (!cemit_fmt(out, "%sfor (%s = (int)(%s); %s < (int)(%s); %s++) {\n",
        ...
        return 0;
    if (!cemit_fmt(out, "%sif (cc_nursery_is_cancelled(__cc_pw_n_%d)) break;\n",
                   i5, id))
        return 0;
    ...
    if (!cemit_fmt(out, "%scc_chan_close(__cc_pw_work_%d);\n", i3, id))
        return 0;
    if (!cemit_fmt(out, "%s(void)cc_nursery_wait(__cc_pw_n_%d);\n", i3, id))
        return 0;
    ...
    if (!cemit_fmt(out, "%scc_chan_free(__cc_pw_work_%d);\n", i3, id))
        return 0;
    if (!cemit_fmt(out, "%scc_nursery_free(__cc_pw_n_%d);\n", i3, id))
        return 0;
