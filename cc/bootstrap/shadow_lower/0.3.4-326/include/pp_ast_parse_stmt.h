...
static AstNode* parse_chan_var(Parser* p) {
    ...
    if (ety.kind != TK_IDENT && ekw != SHADOW_KW_INT && ekw != SHADOW_KW_CHAR &&
        ...
        return NULL;
    if (p->i + 5 >= p->n) return NULL;
    /* Element type span: single token, or multi-token C int type
     * (`long long`, `unsigned int`, …) via peek_c_int_type_end. */
    multi_end = peek_c_int_type_end(p, p->i);
    if (multi_end > p->i) {
        elem_from = p->i;
        elem_to = multi_end;
        bi = multi_end;
    ...
    while (p->i < elem_to) p_next(p); /* elem type tokens */
    ...
    if (is_result) {
        ...
        elem_txt[0] = 0;
        if (!ast_spell_token_range(p, elem_from, elem_to, elem_txt,
                                   sizeof(elem_txt)) &&
            !span_text(p, elem_from, elem_to, elem_txt, sizeof(elem_txt)))
...
static void shadow_result_note_ok_qual(Parser* p, Token at, const char* okty,
    ...
    for (i = 0; i < g_shadow_nrq; i++) {
        ...
        if (g_shadow_rq_qual[i] == quals) return;
        {
            ...
            char lfile[1024];
            const char* path = ft ? tape_diag_file(ft, at.offset, lfile,
                                                   sizeof(lfile))
                                  : "<input>";
...
static AstNode* parse_result_fn(Parser* p) {
    ...
    if (!shadow_parse_result_ok_slice_suffix(p, okty, sizeof(okty))) {
        parser_fail(p, p_peek(p), "bad slice sugar on result ok-type");
        return NULL;
    }
    ...
    }
    {
        ...
        snprintf(ok_host, sizeof(ok_host), "%s", okty);
        shadow_result_ok_ty_host(ok_host, sizeof(ok_host));
        shadow_result_note_ok_qual(p, name, ok_host, errty, had_const);
    }
    ...
    }
    {
        ...
        int fn_start = specs0;
        if (shadow_cparse_fn_in_attach) {
            if (!shadow_cparse_attach_fn_body(p, n, fn_start, body0))
                return NULL;
        } else {
...
static int __attribute__((unused)) parse_at_string_scratch(Parser* p, char* dst,
                                                           size_t cap) {
...
static AstNode* parse_if_stmt(Parser* p) {
    ...
    if (!body) return NULL;
    if (!ast_body_push(p, n, body)) return NULL;
    if (shadow_kw(p_peek(p)) == SHADOW_KW_ELSE) {
        ...
        if (shadow_kw(p_peek(p)) == SHADOW_KW_IF)
            ...
            eb = parse_stmt(p);
        if (!eb) return NULL;
        if (!ast_body_push(p, n, eb)) return NULL;
    }
    return n;
}
...
static AstNode* parse_defer(Parser* p) {
    ...
    if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
        ...
        for (int k = 0; k < blk->nbody; k++) {
            if (!ast_body_push(p, n, blk->body[k])) return NULL;
        }
        p_accept(p, TK_PUNCT, ";"); /* `@defer {…};` */
        return n;
    }
    AstNode* st = parse_stmt(p);
    if (!st) return NULL;
...
/* @cancel_defer name; — disarm the live named @defer. Name is required. */
static AstNode* parse_cancel_defer(Parser* p) {
    Token name;
    AstNode* n;
    if (!tok_eq(p_peek(p), TK_PUNCT, "@")) return NULL;
    if (p->i + 1 >= p->n || p->toks[p->i + 1].kind != TK_IDENT ||
        !spell_eq(p->toks[p->i + 1].spell, "cancel_defer"))
        return NULL;
    p_next(p); /* @ */
    p_next(p); /* cancel_defer */
    if (p_peek(p).kind != TK_IDENT) {
        parser_fail(p, p_peek(p), "@cancel_defer requires a name");
        return NULL;
    }
    name = p_next(p);
...
static AstNode* parse_val_destroy(Parser* p) {
    ...
    ti = p->i;
    ty0tok = p->toks[ti];
    if (ty0tok.kind != TK_IDENT && shadow_kw(ty0tok) != SHADOW_KW_BOOL &&
        shadow_kw(ty0tok) != SHADOW_KW_SIZE_T &&
        shadow_kw(ty0tok) != SHADOW_KW_INT &&
        shadow_kw(ty0tok) != SHADOW_KW_CHAR)
        return NULL;
    ty_end = peek_generic_type_end(p, ti);
    if (ty_end < 0) {
        int iend = peek_c_int_type_end(p, ti);
        ty_end = (iend > ti) ? iend : ti + 1;
    }
    ni = ty_end;
    nstars = 0;
    ...
    if (tok_eq(p->toks[ni], TK_PUNCT, "=")) {
        j = ni + 1;
    } else if (tok_eq(p->toks[ni], TK_PUNCT, "@") && ni + 1 < p->n &&
     ...
     * `!>` / `!>(e){…}` at depth 0 is the create unwrap — not part of init. */
    depth = 0;
    saw = 0;
    is_detach = 0;
    int bang_at = -1;
    ...
    while (j < p->n) {
        ...
        if (tok_eq(t, TK_PUNCT, "(") || tok_eq(t, TK_PUNCT, "{") || tok_eq(t, TK_PUNCT, "["))
            ...
            if (at_create) {
                parser_fail(p, p->toks[name_i],
                            "created with '@(...)' requires explicit "
...

    {
        ...
        if (ty_end > ti + 1) {
            ...
            slice_to(tytxt, sizeof(tytxt), ty0tok.spell);
        {
            size_t tn = strlen(tytxt);
            while (tn && (tytxt[tn - 1] == ' ' || tytxt[tn - 1] == '\t'))
                tytxt[--tn] = 0;
            while (tn && tytxt[tn - 1] == '*') {
                nstars++;
                tytxt[--tn] = 0;
        ...
        while (p->i < name_i) p_next(p);
        {
            ...
            if (at_create) {
                ...
                p_next(p); /* ( */
                c0 = lp;
                had_cl = parse_call_arg_closure(p, c0, init_end, tmp, sizeof(tmp),
                                                "create", &create_cl);
                if (p->err) return NULL;
                int r;
                if (had_cl) {
                    if (tmp[0] == '(')
                        r = snprintf(init, sizeof(init), "__cc_at_create%s", tmp);
                    else if (tmp[0])
                        r = snprintf(init, sizeof(init), "__cc_at_create(%s", tmp);
                    else
                        r = snprintf(init, sizeof(init), "__cc_at_create(");
                } else {
                    if (!ast_spell_token_range(p, c0, init_end, tmp, sizeof(tmp)) &&
                        !span_text(p, c0, init_end, tmp, sizeof(tmp))) {
                        parser_fail(p, name, "val @destroy/@detach init too long");
                        return NULL;
                    }
                    r = snprintf(init, sizeof(init), "__cc_at_create%s", tmp);
                    while (p->i < init_end) p_next(p);
                }
                /* Prefix can push an exact-fit span past cap — fail loud. */
                if (r < 0 || (size_t)r >= sizeof(init)) {
                    parser_fail(p, name, "val @destroy/@detach init too long");
                    return NULL;
                }
            } else {
                p_next(p); /* = */
                c0 = p->i;
                had_cl = parse_call_arg_closure(p, c0, init_end, init, sizeof(init),
                                                "create", &create_cl);
                if (p->err) return NULL;
                if (!had_cl) {
                    while (p->i < init_end) p_next(p);
                    if (!ast_spell_token_range(p, c0, init_end, init, sizeof(init)) &&
                        !span_text(p, c0, init_end, init, sizeof(init))) {
                        parser_fail(p, name,
                                    "val @destroy/@detach init too long");
                        return NULL;
                    }
                }
            }
            {
        AstNode* ue = (!had_cl)
            ? parse_ufcs_expr_range(p, at_create ? lp : c0, init_end)
            : NULL;
        ...
        if (create_cl) {
            ...
            if (!ast_dbody_push(p, n, create_cl)) return NULL;
        }
        if (bang_at >= 0) {
            ...
            if (p_accept(p, TK_PUNCT, "(")) {
                ...
                if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
                    ...
                    while (!tok_eq(p_peek(p), TK_PUNCT, "}") &&
                        ...
                        AstNode* st = parse_stmt(p);
                        if (!st) return NULL;
                        if (!ast_body_push(p, eh, st)) return NULL;
                    }
                    if (!p_accept(p, TK_PUNCT, "}")) {
                        parser_fail(p, p_peek(p),
                                    "expected '}' after !>(...) { ... }");
                ...
                if (!ast_dbody_push(p, n, eh)) return NULL;
            }
        }
        p_next(p); /* @ */
...
/* Word `in` in a for header (not an ident fragment). */
static const char* shadow_for_hdr_in(const char* h) {
    const char* p;
    if (!h) return NULL;
    for (p = h; *p; p++) {
        if (*p == ';') return NULL;
...
static int shadow_for_csv_count(const char* s, const char* lim) {
    int n = 0;
    int depth = 0;
    int any = 0;
    const char* p;
    if (!s) return 0;
    for (p = s; *p && (!lim || p < lim); p++) {
        if (*p == '(' || *p == '[' || *p == '{') depth++;
        else if (*p == ')' || *p == ']' || *p == '}') depth--;
        else if (depth == 0 && *p == ',') {
            n++;
...
static int shadow_for_is_zip(const char* h) {
    const char* in = shadow_for_hdr_in(h);
    if (!in) return 0;
    return shadow_for_csv_count(h, in) == 2 &&
           shadow_for_csv_count(in + 2, NULL) == 2;
}

static int shadow_for_hdr_is_range(const char* h) {
    const char* in;
    const char* q;
    int depth = 0;
    in = shadow_for_hdr_in(h);
    if (!in) return 0;
    for (q = in + 2; *q; q++) {
        if (*q == '(' || *q == '[' || *q == '{') depth++;
        else if (*q == ')' || *q == ']' || *q == '}') {
...
/* `&name` in the binder list (before `in`). Not `&&`. */
static int shadow_for_has_mut_bind(const char* h) {
    const char* in;
    const char* p;
    int depth = 0;
    in = shadow_for_hdr_in(h);
...
/* Zip is void !>(CCError). Mut walk is not. `!>;` → enclosing
 * @errhandler. `!>(e){…}` attaches a site handler. Same channel as
 * `@parallel {…} !>`. */
static int parse_for_zip_bang_tail(Parser* p, AstNode* n) {
    Token at;
    if (!tok_eq(p_peek(p), TK_PUNCT, "!>")) {
        parser_fail(p, p_peek(p), "unconsumed Result");
        return 0;
    }
    at = p_next(p);
    if (p_accept(p, TK_PUNCT, ";")) {
    ...
    if (!p_accept(p, TK_PUNCT, "(")) {
        parser_fail(p, at, "expected '!>;' or '!>(e) { ... }'");
        return 0;
    }
    {
        Token bind = p_next(p);
        AstNode* eh;
        if (bind.kind != TK_IDENT) {
            parser_fail(p, bind, "expected bind name in for-in !>(...)");
            return 0;
        }
        if (!p_accept(p, TK_PUNCT, ")")) {
            parser_fail(p, p_peek(p), "expected ')' after for-in !>(bind");
            return 0;
        }
        eh = ast_new(p, AST_ERRHANDLER);
        if (!eh) return 0;
        ...
        if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
            ...
            if (!p_accept(p, TK_PUNCT, "}")) {
                parser_fail(p, p_peek(p), "expected '}' after for-in !>(...)");
                return 0;
            }
            p_accept(p, TK_PUNCT, ";");
        } else {
...
/* cc_for: `@for (name in s)` walk / zip / range. Bare `for` is C-only. */
static AstNode* parse_for_loop(Parser* p, int cc_for) {
    if (cc_for) {
        if (!tok_eq(p_peek(p), TK_PUNCT, "@")) return NULL;
        p_next(p); /* @ */
    }
    ...
    if (shadow_for_has_mut_bind(n->a) && shadow_for_hdr_is_range(n->a)) {
        parser_fail(p, p_peek(p),
                    "range for-in is `@for (i in lo..hi)` (`&i` is not a slot)");
        return NULL;
    }
    if (shadow_for_is_zip(n->a) || shadow_for_has_mut_bind(n->a)) {
        if (!parse_for_zip_bang_tail(p, n)) return NULL;
    } else if (tok_eq(p_peek(p), TK_PUNCT, "!>")) {
        parser_fail(p, p_peek(p),
                    "`!>` unwraps a Result; this for is not one");
        return NULL;
    }
    return n;
}

static AstNode* parse_for(Parser* p) { return parse_for_loop(p, 0); }

static AstNode* parse_cc_for(Parser* p) { return parse_for_loop(p, 1); }
...
static AstNode* parse_typed_init(Parser* p) {
    ...
    int start_i = p->i;
    int ti = start_i;
    /* Optional local `static` (e.g. `static char buf[4096];` in a fn body).
     * Without this, soft_stmt misses and the whole static fn falls to raw
     * tape — leaving `char[:]` unlowered. */
    if (ti < p->n && shadow_kw(p->toks[ti]) == SHADOW_KW_STATIC) {
        has_static = 1;
        ti++;
    }
    ...
    /* `const T* name[N];`, `static T a[N], b;`, `T *a = NULL, *b;`, or
     * `T name = …;`. Multi lists (with or without init) share the walker. */
    int one = peek_one_declarator(p, decl_i, p->n,
                                 SHADOW_DECL_INIT | SHADOW_DECL_NO_BANG);
    int has_init = 0;
    int multi_end = -1;
    int j = ni;
    ...
    if (one < p->n && tok_eq(p->toks[one], TK_PUNCT, ",")) {
        multi_end = peek_decl_list_semi(
            p, decl_i, SHADOW_DECL_INIT | SHADOW_DECL_NO_BANG | SHADOW_DECL_FAIL);
        if (multi_end < 0) return NULL;
    ...
    if (is_anon_struct) {
        ...
        /* `Family_rest` in a type position is an instance (not a method). */
        if (shadow_ident_matches_factory_instance(tytxt))
            shadow_seed_spelled_type_name(p, tytxt);
        /* Bare CamelCase is a C type spelling. A passthrough #include
         * does not register those typedefs here — seed the name so emit
         * keeps it, and let host cc typecheck. */
    ...
    if (multi_end >= 0) {
        ...
        char pref[64];
        size_t pi = 0;
        /* Include leading `*` / cv so `void *name, *mod` / `uint32_t *a, *b`
         * keep the first star (span from the name alone dropped it). */
        if (!span_text(p, decl_i, multi_end, names, sizeof(names))) {
            parser_fail(p, p->toks[name_i], "declarator list too long");
            return NULL;
        }
        while (p->i < multi_end) p_next(p);
        p_next(p); /* ; */
        n = ast_new(p, AST_VAR_DECL);
        if (!n) return NULL;
        pref[0] = 0;
        if (has_static) {
            memcpy(pref + pi, "static ", 7);
            pi += 7;
        }
        if (has_atomic) {
    ...
    /* Match AstNode.c (2048) — @string / py -c bodies overflowed 512. */
    char expr[2048];
    ...
    if (has_init) {
        ...
        if (!had_cl) {
            ...
            if (c0 >= j) {
                ...
                char lfile[1024];
                const char* path = ft ? tape_diag_file(ft, name.offset, lfile,
                                                       sizeof(lfile))
                                      : "<input>";
                const char* tests = strstr(path, "tests/");
                int oline = 1, ocol = 1;
    ...
    if (call_cl) {
        ...
        if (!ast_dbody_push(p, n, call_cl)) return NULL;
    }
    return n;
}
...
static AstNode* ast_make_ufcs(Parser* p, AstKind kind, int r0, int call_op,
    ...
    /* Spell into AstNode.a capacity — long `"….println()"` receivers are real
     * script surface (dev.shcc tip), not a 127-byte soft miss. */
    char recv[2048];
    char targs[128];
    Token op, meth;
    AstNode* n;
...
static int ast_attach_ufcs_kid(Parser* p, AstNode* parent, AstNode* ue) {
    ...
    return ast_dbody_push(p, parent, ue);
}

/* recv.method(args);  or  store->field.method(args);  (+ optional !>;)
 * Also `@string(...).method(...)[!>;]` — template receiver.
 * Also `@slice(...).method(...)[!>;]` — script surface.
 * Also `"lit".method(...)[!>;]` — script surface (cstr print family). */
static AstNode* parse_ufcs_stmt(Parser* p) {
    ...
    }
    {
        int at_tmpl = 0;
        if (tok_eq(p_peek(p), TK_PUNCT, "@") && p->i + 1 < p->n) {
            if (shadow_kw(p->toks[p->i + 1]) == SHADOW_KW_STRING)
                at_tmpl = 1;
            else if (p->toks[p->i + 1].kind == TK_IDENT &&
                     spell_eq(p->toks[p->i + 1].spell, "slice"))
                at_tmpl = 1;
        }
        if (!(p_peek(p).kind == TK_IDENT || p_peek(p).kind == TK_STR ||
              at_tmpl)) {
            p->i = save;
            return NULL;
        }
    ...
    int a1 = p->i - 1;
    /* The call is a C expression. Leftover postfix / assign (`s.ptr[i]`,
     * `p++`, `x += 1`) is legal C — restore so assign / expr_stmt own it.
     * Do not make that C illegal by demanding `;` here. */
    if (!tok_eq(p_peek(p), TK_PUNCT, ";") &&
        !tok_eq(p_peek(p), TK_PUNCT, "!>")) {
        p->i = save;
        return NULL;
...
static AstNode* parse_assign_stmt(Parser* p) {
    {
        AstNode* par = parse_parallel_result_assign(p);
        if (par || p->err) return par;
    }
...
/* Optional `[…]`… after a declarator name. Appends into names[*nl]. */
static int parse_var_decl_append_dims(Parser* p, char* names, size_t cap,
                                      size_t* nl) {
    int d0;
    if (!p || !names || !nl) return 0;
    ...
    d0 = p->i;
    {
        ...
        do {
            Token t = p_peek(p);
            if (t.kind == TK_EOF) {
                parser_fail(p, t, "unterminated '[' in declarator");
                return 0;
            }
            if (tok_eq(t, TK_PUNCT, "[")) depth++;
            else if (tok_eq(t, TK_PUNCT, "]")) depth--;
    ...
    if (!span_text(p, d0, p->i, names + *nl, cap - *nl)) {
        parser_fail(p, p->toks[d0], "declarator too long");
        return 0;
    }
    *nl = strlen(names);
    return 1;
...
static AstNode* parse_var_decl(Parser* p) {
    ...
    if (si < p->n &&
        ...
        if (!parse_var_decl_append_dims(p, names, sizeof(names), &nl))
            return NULL;
        while (p_accept(p, TK_PUNCT, ",")) {
            int st = 0;
            Token n2;
    ...
    if (!has_struct && !is_type_slice && tok_eq(p->toks[si], TK_PUNCT, "[")) {
        ...
        /* `T a[N];` or multi `T a[N], b;` / `T a[N], b[M];`. */
        if (j >= p->n ||
            (!tok_eq(p->toks[j], TK_PUNCT, ";") &&
             !tok_eq(p->toks[j], TK_PUNCT, ","))) {
            parser_fail(p, j < p->n ? p->toks[j] : p_peek(p),
        ...
        if (ty_end > 0) return NULL; /* generic + C array dims unsupported here */
        {
            ...
            /* Multi: pack `kinds[16], nk` / `x, ys[4]` / `key[32], val[32]`.
             * After `T name[` this is a declaration — do not fall through
             * to an expression (host "undeclared" on the leftover names). */
            end_i = name_i;
            while (end_i < p->n && !tok_eq(p->toks[end_i], TK_PUNCT, ";")) {
                ...
                while (end_i < p->n && tok_eq(p->toks[end_i], TK_PUNCT, "*"))
                    end_i++;
                if (end_i >= p->n || p->toks[end_i].kind != TK_IDENT) {
                    parser_fail(p, end_i < p->n ? p->toks[end_i] : p_peek(p),
            ...
            if (!span_text(p, name_i, end_i, names, sizeof(names))) {
                parser_fail(p, p->toks[name_i], "declarator list too long");
                return NULL;
            }
...
static int shadow_is_grammar_static_helper(CCSlice name) {
    ...
    buf[n] = 0;
    /* Internal matchers / fills / writers: Name__r_*, Name__m_*, Name__b_*,
     * Name__fill, Name__s__m_* / __s__b_* (skip-tier), Name__w* (schema
     * write helpers with raw `.u`).  Any `__` was too broad — user helpers
     * like `lev__prep` became opaque tape and lost UFCS.  Leading `__`
     * (libc/CC) stays excluded.
     * `__w*` used to fall into opaque tape only because
     * `static __attribute__((unused))` broke parse_static_fn; once attrs
     * are skipped they must stay opaque or safety rejects schema `.u`. */
    {
        ...
        if (buf[0] != '_' && dd) {
            ...
            if ((after[0] == 'r' || after[0] == 'm' || after[0] == 'b') &&
                after[1] == '_')
                return 1;
            if (after[0] == 'w') return 1; /* __wput / __wchk / __wmeasure */
...
/* Advance index past one `__attribute__((…))` (nested parens). Unchanged if
 * toks[i] is not `__attribute__`. */
static int shadow_skip_attribute_at(Parser* p, int i) {
    int depth;
    if (!p || i < 0 || i >= p->n) return i;
    if (p->toks[i].kind != TK_IDENT ||
        !spell_eq(p->toks[i].spell, "__attribute__"))
        return i;
    i++;
    if (i >= p->n || !tok_eq(p->toks[i], TK_PUNCT, "(")) return i;
    depth = 0;
    while (i < p->n) {
...
static int shadow_skip_attributes_at(Parser* p, int i) {
    int next;
    for (;;) {
        next = shadow_skip_attribute_at(p, i);
        if (next == i) return i;
        i = next;
    }
}

/* Consume `__attribute__((…))*` at the current parse cursor. */
static void shadow_skip_attributes(Parser* p) {
    int next;
    if (!p) return;
    for (;;) {
        next = shadow_skip_attribute_at(p, p->i);
        if (next == p->i) return;
        p->i = next;
    }
}

/* Storage / type-specifier words that can precede a function declarator. */
static int looks_like_decl_spec(Token t) {
    ShadowKwKind k = shadow_kw(t);
    if (k == SHADOW_KW_STATIC || k == SHADOW_KW_INLINE ||
        k == SHADOW_KW_CONST || k == SHADOW_KW_STRUCT ||
        k == SHADOW_KW_ENUM || k == SHADOW_KW_VOID ||
        k == SHADOW_KW_INT || k == SHADOW_KW_CHAR ||
        k == SHADOW_KW_BOOL || k == SHADOW_KW_SIZE_T ||
        k == SHADOW_KW_TYPEDEF)
        return 1;
    if (t.kind == TK_IDENT &&
        (spell_eq(t.spell, "unsigned") || spell_eq(t.spell, "signed") ||
         spell_eq(t.spell, "long") || spell_eq(t.spell, "short") ||
         spell_eq(t.spell, "volatile") || spell_eq(t.spell, "extern") ||
         spell_eq(t.spell, "union") || spell_eq(t.spell, "restrict")))
        return 1;
    return 0;
}

/* Soft body miss at another function (`static T name(` / `T name(`) or EOF
 * — not a local `static T x;` / `static T *p` / `static T (*fp)(`. */
static int looks_like_next_fn_decl(Parser* p) {
    int i;
    if (!p) return 0;
    if (p->i >= p->n || p->toks[p->i].kind == TK_EOF) return 1;
    i = p->i;
    for (;;) {
        int next = shadow_skip_attributes_at(p, i);
        if (next != i) {
            i = next;
            continue;
        }
        if (i < p->n && looks_like_decl_spec(p->toks[i])) {
            i++;
            continue;
        }
        break;
    ...
    if (i < p->n && p->toks[i].kind == TK_IDENT) {
        if (i + 1 < p->n && tok_eq(p->toks[i + 1], TK_PUNCT, "("))
            return 1;
        i++;
    }
    while (i < p->n && tok_eq(p->toks[i], TK_PUNCT, "*")) i++;
    if (i + 2 < p->n && tok_eq(p->toks[i], TK_PUNCT, "[") &&
        tok_eq(p->toks[i + 1], TK_PUNCT, ":")) {
        if (tok_eq(p->toks[i + 2], TK_PUNCT, "]"))
            i += 3;
        else if (i + 3 < p->n && tok_eq(p->toks[i + 2], TK_PUNCT, "!") &&
                 tok_eq(p->toks[i + 3], TK_PUNCT, "]"))
            i += 4;
    }
    if (i < p->n && tok_eq(p->toks[i], TK_PUNCT, "(")) return 0;
    if (i >= p->n || p->toks[i].kind != TK_IDENT) return 0;
    if (i + 1 >= p->n || !tok_eq(p->toks[i + 1], TK_PUNCT, "(")) return 0;
    return 1;
}

/* static [attr] [inline] [attr] [const] [attr] Ret[*] name ( params ) { body }
 * — stmts or raw (d). Attributes after `static` (grammar emit's
 * `__attribute__((unused))` placement) must be skipped so Node_* helpers
 * stay AST_STATIC_FN and register for UFCS; otherwise they fall through to
 * opaque tape and `nd->count()` dies as "not declared". */
static AstNode* parse_static_fn(Parser* p) {
    ...
    if (p->i + 4 >= p->n) return NULL;
    int ri0 = p->i + 1;
    int ri = shadow_skip_attributes_at(p, ri0);
    int has_inline = 0;
    int has_const = 0;
    int has_unused = 0;
    int has_always_inline = 0;
    if (ri < p->n && shadow_kw(p->toks[ri]) == SHADOW_KW_INLINE) {
        has_inline = 1;
        ri = shadow_skip_attributes_at(p, ri + 1);
    }
    ...
    }
    {
        int j;
        for (j = ri0; j < ri; j++) {
            if (p->toks[j].kind != TK_IDENT) continue;
            if (spell_eq(p->toks[j].spell, "unused")) has_unused = 1;
            if (spell_eq(p->toks[j].spell, "always_inline"))
                has_always_inline = 1;
        }
    }
    ...
    {
        int ty_end = ri + 1;
        if (ast_try_generic_ret(p, ri, &ty_end, &ni, gret, sizeof(gret))) {
            /* Name::[args] return — spelled into gret. */
        }
    }
    int is_long_long = 0;
    int is_unsigned = 0;
    int is_unsigned_long = 0;
    int is_slice = 0;
    int is_slice_unique = 0;
    /* `unsigned` / `unsigned long` / `unsigned long long` / `unsigned int`. */
    if (rty.kind == TK_IDENT && spell_eq(rty.spell, "unsigned")) {
        is_unsigned = 1;
        if (ni < p->n && p->toks[ni].kind == TK_IDENT &&
            spell_eq(p->toks[ni].spell, "long")) {
            is_unsigned_long = 1;
            ni++;
            if (ni < p->n && p->toks[ni].kind == TK_IDENT &&
                spell_eq(p->toks[ni].spell, "long")) {
                ...
                ni++;
                   ...
                   (shadow_kw(p->toks[ni]) == SHADOW_KW_INT ||
                    (p->toks[ni].kind == TK_IDENT &&
                     spell_eq(p->toks[ni].spell, "int")))) {
            ni++; /* unsigned int */
        }
    } else if (rty.kind == TK_IDENT && spell_eq(rty.spell, "long") &&
               ni < p->n && p->toks[ni].kind == TK_IDENT &&
               spell_eq(p->toks[ni].spell, "long")) {
        is_long_long = 1;
        ni++;
    }
    ...
    /* Mid-declarator GNU attr: `static void __attribute__((unused)) name`. */
    {
        int ni1 = shadow_skip_attributes_at(p, ni);
        int j;
        for (j = ni; j < ni1; j++) {
            if (p->toks[j].kind != TK_IDENT) continue;
            if (spell_eq(p->toks[j].spell, "unused")) has_unused = 1;
    ...
    p_next(p); /* ret (or unsigned / long) */
    if (gret[0]) {
        while (p->i < ni) p_next(p);
    } else if (is_unsigned) {
        /* Consumed `unsigned`; optional int/long/long long already in ni. */
        while (p->i < ni) p_next(p);
    } else if (is_long_long) {
        p_next(p); /* long (second) */
    }
    if (is_slice) {
        p_next(p); /* [ */
        p_next(p); /* : */
    ...
    /* Spell from tokens, not FileTape bytes: tape rewrite turns
     * `@typeview(Mode) Base` into ident `Base_Restrict_Mode` whose
     * spell no longer matches the source span at `@`. */
    char params[2048];
    params[0] = 0;
    if (p0 < p->i &&
        !ast_spell_token_range(p, p0, p->i, params, sizeof(params))) {
    ...
    /* Forward decl: `static T name(params);`. Slice sugar (`char[:]`)
     * must be AST_FN_PROTO so emit rewrites `[:]`. Other protos stay tape
     * (attributes / odd C spelling). */
    if (tok_eq(p_peek(p), TK_PUNCT, ";")) {
        int slice_proto = is_slice ||
                          (strstr(params, "[") && strstr(params, ":"));
        if (gret[0] || slice_proto) {
    ...
    }
    {
        ...
        n->c = ast_arena_cstr(p, params);
        {
            ...
            fl[0] = 0;
            if (has_inline)
                fo += (size_t)snprintf(fl + fo, sizeof(fl) - fo, "%sinline",
                                       fo ? " " : "");
            if (has_unused)
                fo += (size_t)snprintf(fl + fo, sizeof(fl) - fo, "%sunused",
                                       fo ? " " : "");
        ...
        /* Soft miss: do not tape a body that still has Concurrent-C — that
         * leaves !>/@/=>/[:] / :: unlowered. A following function or EOF
         * is an unclosed body — say so, do not blame later `@errhandler`. */
        if (!ok_stmts || !tok_eq(p_peek(p), TK_PUNCT, "}")) {
            char fname[64];
            char msg[192];
            if (p->err && p->err != saved_err) {
                p->nn = saved_nn;
                p->nkstore = saved_nk;
                return NULL;
            }
            if (ok_stmts) miss_at = p_peek(p);
            snprintf(fname, sizeof(fname), "%s", n->b);
            if (looks_like_next_fn_decl(p) ||
                p_peek(p).kind == TK_EOF) {
                snprintf(msg, sizeof(msg),
                         "expected '}' to close '%s'", fname);
                p->nn = saved_nn;
                p->nkstore = saved_nk;
                p->err = saved_err;
                parser_fail(p, miss_at, msg);
                parser_note(p, open_brace, "to match this '{'");
                return NULL;
            }
            {
                int j = body0;
                ...
                if (has_cc && !shadow_cparse_fn_in_attach) {
                    snprintf(msg, sizeof(msg),
                             "could not parse statement in '%s'", fname);
                    p->nn = saved_nn;
                    p->nkstore = saved_nk;
                    p->err = saved_err;
                    parser_fail(p, miss_at, msg);
                ...
                /* cparse envelope: per-stmt overlay attach (never whole-fn
                 * FileTape). Soft miss tapes one stmt only. */
                if (shadow_cparse_fn_in_attach) {
                    p->err = saved_err;
                    p->nkstore = (int)(n->kids - p->kids_storage);
                    n->nkids = 0;
                    if (!shadow_cparse_attach_fn_body(p, n, fn_start, body0)) {
                        p->nn = saved_nn;
                        p->nkstore = saved_nk;
        ...
        /* Hard diagnostic during soft body (e.g. empty RHS `int x = ;`) must
         * not be RAW_LINE-swallowed — that would re-emit the bad C and exit 0. */
...
static AstNode* parse_expr_stmt(Parser* p) {
    ...
    AstNode* n;
    AstNode* ue;
    if (t.kind != TK_IDENT && t.kind != TK_NUM && t.kind != TK_STR &&
        !tok_eq(t, TK_PUNCT, "(") && !tok_eq(t, TK_PUNCT, "*") &&
        !tok_eq(t, TK_PUNCT, "&") && !tok_eq(t, TK_PUNCT, "-") &&
        !tok_eq(t, TK_PUNCT, "+") && !tok_eq(t, TK_PUNCT, "!") &&
        !tok_eq(t, TK_PUNCT, "~") && !tok_eq(t, TK_PUNCT, "++") &&
...
/* `@auto(src) name(arena) @destroy` — grower of src's char/slice face,
 * capacity `src.len`. Same ownership as `Vec::[char] name@(arena) @destroy`. */
static AstNode* parse_auto_grower(Parser* p) {
    Token name;
    int src0, src1, ar0, ar1;
    char src[256];
    char arena[128];
    char init[512];
    AstNode* n;
    if (!tok_eq(p_peek(p), TK_PUNCT, "@")) return NULL;
    if (p->i + 1 >= p->n || p->toks[p->i + 1].kind != TK_IDENT ||
        !spell_eq(p->toks[p->i + 1].spell, "auto"))
        return NULL;
    p_next(p); /* @ */
    p_next(p); /* auto */
    if (!tok_eq(p_peek(p), TK_PUNCT, "(")) {
        parser_fail(p, p_peek(p), "expected '(src)' after @auto");
        return NULL;
    }
    src0 = p->i + 1;
    if (!skip_parens(p)) {
        parser_fail(p, p_peek(p), "unterminated @auto(src)");
        return NULL;
    }
    src1 = p->i - 1;
    if (src1 <= src0) {
        parser_fail(p, p_peek(p), "@auto(src) needs a slice source");
        return NULL;
    }
    src[0] = 0;
    if (!ast_spell_token_range(p, src0, src1, src, sizeof(src)) &&
        !span_text(p, src0, src1, src, sizeof(src))) {
        parser_fail(p, p_peek(p), "@auto(src) too long");
        return NULL;
    }
    if (p_peek(p).kind != TK_IDENT) {
        parser_fail(p, p_peek(p), "expected binder after @auto(src)");
        return NULL;
    }
    name = p_next(p);
    if (!tok_eq(p_peek(p), TK_PUNCT, "(")) {
        parser_fail(p, p_peek(p), "expected '(arena)' after @auto binder");
        return NULL;
    }
    ar0 = p->i + 1;
    if (!skip_parens(p)) {
        parser_fail(p, p_peek(p), "unterminated @auto arena");
        return NULL;
    }
    ar1 = p->i - 1;
    if (ar1 <= ar0) {
        parser_fail(p, name, "@auto grower needs an arena");
        return NULL;
    }
    arena[0] = 0;
    if (!ast_spell_token_range(p, ar0, ar1, arena, sizeof(arena)) &&
        !span_text(p, ar0, ar1, arena, sizeof(arena))) {
        parser_fail(p, name, "@auto arena too long");
        return NULL;
    }
    if (!tok_eq(p_peek(p), TK_PUNCT, "@") || p->i + 1 >= p->n ||
        shadow_kw(p->toks[p->i + 1]) != SHADOW_KW_DESTROY) {
        parser_fail(p, name, "@auto grower requires '@destroy'");
        return NULL;
    }
    p_next(p); /* @ */
    p_next(p); /* destroy */
    if (!p_accept(p, TK_PUNCT, ";")) {
        parser_fail(p, p_peek(p), "expected ';' after @auto … @destroy");
        return NULL;
    }
...
/* Leftover `@cache` — scratch is `cache (name)` on the wait-for head. */
static AstNode* parse_cache_decl(Parser* p) {
    Token at;
    if (!tok_eq(p_peek(p), TK_PUNCT, "@")) return NULL;
    if (p->i + 1 >= p->n || shadow_kw(p->toks[p->i + 1]) != SHADOW_KW_CACHE)
        return NULL;
    p_next(p); /* @ */
    at = p_next(p); /* cache */
    parser_fail(p, at,
                "write cache (name) on the wait-for, not @cache on a "
                "declaration");
    return NULL;
}

static int parser_is_typeish(Token t) {
    ShadowKwKind k = shadow_kw(t);
    return t.kind == TK_IDENT || k == SHADOW_KW_INT || k == SHADOW_KW_VOID ||
           k == SHADOW_KW_CHAR || k == SHADOW_KW_BOOL || k == SHADOW_KW_SIZE_T ||
           k == SHADOW_KW_STRUCT || k == SHADOW_KW_CONST;
}

static int parser_stmt_storage_end(Parser* p) {
    int i;
    if (!p) return 0;
    i = p->i;
    while (i < p->n) {
        if (shadow_kw(p->toks[i]) == SHADOW_KW_STATIC)
            i++;
        else if (tok_is_cv(p->toks[i]))
            i++;
        else
            break;
    }
...
/* TYPE IDENT leftover that is not a declarator punct — space in a name, etc.
 * Same shape as TU leftover; do not fall through to an expression. */
static int parser_fail_typeish_leftover(Parser* p) {
    int i;
    Token a, b, c;
    if (!p || p->err) return 0;
    i = parser_stmt_storage_end(p);
    if (i + 2 >= p->n) return 0;
    a = p->toks[i];
    b = p->toks[i + 1];
    c = p->toks[i + 2];
    if (!parser_is_typeish(a) || b.kind != TK_IDENT) return 0;
    if (tok_eq(c, TK_PUNCT, "(") || tok_eq(c, TK_PUNCT, ";") ||
        tok_eq(c, TK_PUNCT, "=") || tok_eq(c, TK_PUNCT, "[") ||
        tok_eq(c, TK_PUNCT, ",") || tok_eq(c, TK_PUNCT, "*"))
        return 0;
    {
        char msg[192];
        snprintf(msg, sizeof(msg), "expected '(' or ';' after '%.*s'",
                 (int)b.spell.len, b.spell.ptr);
        parser_fail(p, c, msg);
        return 1;
    }
}
...
static AstNode* parse_stmt_inner(Parser* p) {
     ...
     * (stage-2 root tape). File-scope parse_external_inner handles these;
     * stmt bodies must too or `#endif` is stolen as an expr/assign
     * spanning to the next `;`. */
    ...
    if (tok_eq(t, TK_PUNCT, "@")) {
        ...
        n = parse_stage(p);
        if (n || p->err) return n;
        n = parse_cache_decl(p);
        if (n || p->err) return n;
        n = parse_auto_grower(p);
        if (n || p->err) return n;
    ...
    if (shadow_kw(t) == SHADOW_KW_STATIC) {
        ...
        if (n || p->err) return n;
        /* Local `static T name[N];` / `static T name = …;` */
        n = parse_typed_init(p);
        if (n || p->err) return n;
    }
    if (shadow_kw(t) == SHADOW_KW_INT) {
        n = parse_parallel_result_init(p);
        if (n || p->err) return n;
    ...
    if (n || p->err) return n;
    if (parser_fail_typeish_leftover(p)) return NULL;
...
/* Stage2 marker for `#pragma(@parallel) off|on` — positional, so it is
 * consumed exactly where the pragma sat in the source. */
static int parse_parallel_pragma_marker(Parser* p) {
    Token t = p_peek(p);
    if (t.kind != TK_IDENT) return 0;
    if (tok_eq(t, TK_IDENT, "__cc_pragma_parallel_off"))
        p->parallel_off = 1;
    else if (tok_eq(t, TK_IDENT, "__cc_pragma_parallel_on"))
        p->parallel_off = 0;
    else if (tok_eq(t, TK_IDENT, "__cc_pragma_prelude_off"))
        p->prelude_off = 1;
    else if (tok_eq(t, TK_IDENT, "__cc_pragma_linenumbers_off"))
        p->linenumbers_off = 1;
    else
        return 0;
    p_next(p);
