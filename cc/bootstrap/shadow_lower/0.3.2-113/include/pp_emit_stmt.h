...
static void shadow_collect_binds_node(AstNode* st) {
     ...
     * poisons outer same-named UFCS receivers (Acc vs Bcc `acc`). */
    ...
    for (k = 0; k < st->nbody; k++) {
         ...
         * collecting them here would poison outer same-named UFCS receivers. */
        if (g_shadow_collect_fn_locals && st->body[k] &&
            st->body[k]->kind == AST_BLOCK)
            continue;
