...
typedef enum {
    ...
    AST_SERIAL,
    /* @stage (gate, args…) { stmts } — gate.wait(args); stmts; gate.pass(args);
     * with pass guaranteed on the wait-for error path. a=gate receiver text,
     * b=trailing args text; body=stmts. Top-level of a wait-for body only. */
    AST_STAGE,
    /* name++; — a=name */
    AST_INC,
