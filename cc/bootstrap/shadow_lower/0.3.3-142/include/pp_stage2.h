 ...
 * angle + ordinary quote-.h passthrough. On the root tape, `#if`/`#elif`/
 * `#else`/`#endif` are cpp-transparent (Phase 3): every arm is lowered and
 * conditional lines are emitted verbatim for the host cpp. Spliced include
 * frames keep classic operand eval/skip. Exhaustive directive policy via
 * pp_dir static_map. Requires pp_tape.cch + <ccc/std/static_map.h>. */
#pragma once
