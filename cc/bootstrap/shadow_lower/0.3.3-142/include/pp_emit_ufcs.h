...
static int shadow_ufcs_lower_parts(const char* recv, const char* meth_name,
     ...
     * rewrite `.` into a pointer call. */
    {
        ...
        if (!walked_fields && type_is_ptr &&
            ...
            type_is_ptr = 0;
        /* Channel / Result handles are pointer-sized but use `.` UFCS
         * (`ch.free()`, `ch.send()`). */
        if (type_is_ptr && vty[0] &&
            (strncmp(vty, "CCChan", 6) == 0 ||
             strncmp(vty, "CCResult_", 9) == 0))
            type_is_ptr = 0;
        ufcs_ptr_recv = is_arrow || recv_addr;
        if (!is_arrow && type_is_ptr && !recv_addr) {
