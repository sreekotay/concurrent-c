...
typedef struct {
    ...
    int npending_moves;
    /* Named `@defer name:` live in the current block / function, like locals. */
    char ndefer_name[SHADOW_SAFE_NDEFER_CAP][64];
    int ndefer_scope[SHADOW_SAFE_NDEFER_CAP];
    int nndefer;
    int ndefer_block;
} ShadowSafeCtx;
...
static void shadow_safe_err_at_col(ShadowSafeCtx* ctx, AstNode* st, int force_col,
    ...
    if (ctx->err) return;
    ctx->err = 1;
    /* force_col overrides tape column for a few oracles; still counts. */
    if (force_col > 0 && st && ctx->cache && st->file_id) {
        int line = 1, col = 1;
        ft = tape_by_id(ctx->cache, st->file_id);
        if (ft && ft->bytes) {
            char lfile[1024];
            offset_to_linecol(ft, st->tok_off, &line, &col);
            tape_logical_at(ft, st->tok_off, lfile, sizeof(lfile), &line);
            col = force_col;
            diag_err_loc(tape_diag_file(ft, st->tok_off, lfile, sizeof(lfile)),
                         line, col, msg);
            return;
        }
    }
...
static int shadow_safe_amp_ident(const char* text, char* out, size_t cap) {
    ...
    /* Box peel: `(recv).p` / `(recv)->p` is the same identity as recv. */
    if (text[i] == '(') {
        int depth = 1;
        size_t j = i + 1;
        char inner[160];
        ...
        while (text[j] && depth) {
            ...
            if (depth == 0) break;
            if (inn + 1 < sizeof(inner)) inner[inn++] = text[j];
            j++;
        }
        inner[inn] = 0;
        if (text[j] == ')') {
            ...
            if (text[k] == '.' && text[k + 1] == 'p' &&
                !shadow_safe_is_word(text[k + 2]))
                return shadow_safe_amp_ident(inner, out, cap);
            if (text[k] == '-' && text[k + 1] == '>' && text[k + 2] == 'p' &&
                !shadow_safe_is_word(text[k + 3]))
                return shadow_safe_amp_ident(inner, out, cap);
        }
    }
...
static void shadow_safe_commit_moves_in_text(ShadowSafeCtx* ctx, AstNode* st,
    ...
    while ((p = strstr(p, "cc_move(")) != NULL) {
        ...
        if (srcv->moved) {
            ...
            shadow_safe_err_at(ctx, st, msg);
            diag_note("after cc_move(x), the source variable is no longer "
                      "valid");
            return;
        }
        srcv->moved = 1;
...
static void shadow_safe_add_type_name(ShadowSafeCtx* ctx, const char* name) {
    ...
    if (ctx->ntypes >= SHADOW_SAFE_TYPE_CAP) {
        shadow_safe_err_at(ctx, NULL,
                           "safety type-name table capacity exceeded (512)");
        return;
    }
    ...
    while ((p = strstr(p, "type_of(")) != NULL) {
        ...
        if (!shadow_safe_type_name_known(ctx, q, n)) {
            snprintf(msg, sizeof(msg),
                     ...
                     (int)n, q, (int)n, q);
            shadow_safe_err_at(ctx, st, msg);
            diag_plainf("  hint: register with CC_TYPE_INFO_BEGIN(%.*s) / "
...
static void shadow_safe_check_map_key_text(ShadowSafeCtx* ctx, AstNode* st,
    ...
    while (*p) {
        ...
        if (strncmp(p, "ArrayMap_", 9) == 0) {
            hit = p + 9;
            skip = 9;
        } else if (strncmp(p, "Table_", 6) == 0) {
            hit = p + 6;
            skip = 6;
        } else if (strncmp(p, "table_new_count_", 16) == 0) {
            hit = p + 16;
            skip = 16;
        } else if (strncmp(p, "table_new_", 10) == 0) {
            hit = p + 10;
            skip = 10;
        ...
        if (!shadow_safe_map_key_installed(ctx, kbuf)) {
            ...
            shadow_safe_err_at(ctx, st, msg);
            diag_plainf("declare cc_map_key_hash_%s and cc_map_key_eq_%s to "
                        "install it",
                        kbuf, kbuf);
            return;
        }
...
static int shadow_safe_fn_has_refcount_dec(AstNode* fn) {
    int k;
    if (!fn) return 0;
    if (fn->kind == AST_STATIC_FN && fn->d && fn->d[0] && fn->nkids == 0 &&
...
static int shadow_safe_node_releases_py_obj(AstNode* st, const char* name) {
    ...
    if ((st->kind == AST_CALL_ARGS || st->kind == AST_CALL_NUM) &&
        strcmp(ast_slot(st->a), "cc_py_obj_release") == 0 &&
...
static int shadow_safe_node_unique_slice(AstNode* st) {
    if (!st) return 0;
    if (st->e && st->e[0] == '!' && st->e[1] == 0) return 1;
    if (strstr(ast_slot(st->a), "[:!]") != NULL) return 1;
    return 0;
}
        ...
        while ((p = strstr(p, v->name)) != NULL) {
            ...
            if (after[0] == '-' && after[1] == '>' && after[2] == 'u' &&
                ...
                if (!vt->is_schema && !vt->is_packed)
                    diag_note("access inactive or packed union payload via "
                              "a dominating check, '!>' handler, or '?>' "
                              "fallback");
                return;
            }
                                ...
                                }
                                {
                                    if (!ctx->err) {
                                        ...
                                        diag_at(ctx->cache, t, msg);
                                        {
                                            char __note[384];
                                            snprintf(
                                                __note, sizeof(__note),
                                                "protect it: 'if (%s.kind == "
                                                "%s_%s) { ... }' or 'case .%s:' "
                                                "in a switch on '%s', or handle "
                                                "the other arms: '%s.%s !> { "
                                                "...diverge... }' / '%s.%s ?> "
                                                "fallback'",
...
/* RAW_LINE label `case .arm:` / `case .arm(bind):` / `case Name_arm:`. */
static int shadow_safe_case_label_arm(ShadowSafeCtx* ctx, const char* root,
                                     const char* lab, char* arm, size_t acap) {
    const char* c;
    const char* after;
    const char* colon;
    size_t tlen;
    size_t i;
    if (!ctx || !root || !lab || !arm || !acap) return 0;
    if (shadow_safe_find_kw(lab, "default")) return 0;
    c = shadow_safe_find_kw(lab, "case");
    if (!c) return 0;
    after = c + 4;
    colon = strchr(after, ':');
    tlen = colon ? (size_t)(colon - after) : strlen(after);
    for (i = 0; i < tlen; i++) {
        if (after[i] == '(') {
            tlen = i;
            break;
    ...
    return shadow_safe_arm_from_tag(ctx, root, after, tlen, arm, acap);
}
...
static void shadow_safe_on_typed_init(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    stars = ((st->d && st->d[0] == '*')) || (st->kind == AST_PTR_INIT) ||
            (ty && strchr(ty, '*') != NULL);
    ...
    if (st->e && st->e[0] == '[') v->is_stack_array = 1;
        ...
        if (shadow_safe_plain_ident(expr, src, sizeof(src))) {
            ...
            if (srcv && srcv->is_slice) {
                ...
                if (srcv->unique) {
                    ...
                    diag_plain("  hint: unique slices have move-only semantics; "
...
static void shadow_safe_on_assign(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (strstr(ast_slot(st->b), "cc_move(") != NULL) {
    ...
    if (shadow_safe_plain_ident(st->b, rname, sizeof(rname))) {
        ...
        if (rhs && rhs->is_slice) {
            ...
            if (rhs->unique || lhs->unique) {
                shadow_safe_err_at(ctx, st, "cannot copy unique slice");
                diag_plain("  hint: unique slices have move-only semantics; use "
...
static void shadow_safe_on_return(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (shadow_safe_plain_ident(st->a, name, sizeof(name))) {
        ...
        if (v && v->is_slice && v->unique) {
            ...
            diag_plain("  hint: unique slices (T[:!]) require explicit ownership "
...
static void shadow_safe_on_deleter(ShadowSafeCtx* ctx, AstNode* st,
    ...
    if ((strcmp(callee, "free") == 0 || strcmp(callee, "cfree") == 0) &&
        ...
        diag_note("non-atomic int ref / last-drop races under concurrency "
                  "(CVE-2026-10653 class)");
        diag_plain("  hint: cc_arc_from_ptr / cc_arc_clone / cc_arc_drop, or don't "
    ...
    if (v->unproven_ptr &&
        ...
        shadow_safe_err_at(ctx, st, msg);
        diag_plain("  hint: free only heap owners (malloc/calloc) or let the "
...
static void shadow_safe_on_arena_epoch(ShadowSafeCtx* ctx, AstNode* st,
    ...
    if (shadow_safe_arena_pinned(ctx, arena)) {
        snprintf(msg, sizeof(msg),
                 ...
                 op, arena);
        shadow_safe_err_at(ctx, st, msg);
        diag_note("capturing an arena borrow into a spawn pins that "
    ...
    if (strcmp(op, "cc_arena_reset") == 0 &&
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 arena);
        shadow_safe_err_at(ctx, st, msg);
        diag_note("arena provenance epoch would invalidate live borrows "
...
static int shadow_typeview_body_as_names(const char* body, char names[][64],
    ...
    while (*p) {
        ...
        /* `^pat` deny prefix (and any other punct) must not stall the walk. */
        if (!shadow_safe_is_word(*p) && *p != '*') {
            p++;
            continue;
        }
...
static void shadow_mark_struct_as_fields(AstNode* st, char names[][64], int nn) {
    ...
    for (i = 0; i < st->nkids; i++) {
        AstNode* f = st->kids[i];
        if (!f || f->kind != AST_FIELD_SIMPLE || !(f->b && f->b[0])) continue;
        {
            const char* np = f->b;
            char one[64];
            int hit = 0;
            while (!hit && shadow_field_next_name(&np, one, sizeof(one))) {
                char id[64];
                shadow_decl_ident(one, id, sizeof(id));
                if (!id[0]) continue;
                for (k = 0; k < nn; k++) {
                    if (strcmp(id, names[k]) == 0) {
                        f->e = (char*)"as";
                        hit = 1;
                        break;
                    }
...
static void shadow_apply_typeview_as_faces(AstNode** items, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (it->kind != AST_AT_STMT || strcmp(ast_slot(it->a), "typeview") != 0 || !(it->d && it->d[0]))
            continue;
        ...
        for (j = 0; j < n; j++) {
            ...
            if (st->kind == AST_TYPEDEF_STRUCT &&
                ((st->b && st->b[0] && strcmp(ast_slot(st->b), it->d) == 0) ||
                 (st->a && st->a[0] && strcmp(ast_slot(st->a), it->d) == 0)))
                shadow_mark_struct_as_fields(st, names, nn);
            else if (st->kind == AST_STRUCT && st->a && st->a[0] &&
                     strcmp(ast_slot(st->a), it->d) == 0)
                shadow_mark_struct_as_fields(st, names, nn);
        }
    }
}
...
static int shadow_safe_register_struct(ShadowSafeCtx* ctx, AstNode* st) {
    ShadowSafeStruct* s;
    int i;
    if (!ctx || !st || st->kind != AST_TYPEDEF_STRUCT || !(st->b && st->b[0])) return 0;
    if (ctx->nstructs >= SHADOW_SAFE_STRUCT_CAP) {
        shadow_safe_err_at(ctx, st,
                           "safety struct table capacity exceeded");
        return 0;
    }
    ...
    for (i = 0; i < st->nkids && s->nf < SHADOW_SAFE_FIELD_CAP; i++) {
        AstNode* f = st->kids[i];
        if (!f || f->kind != AST_FIELD_SIMPLE || !(f->a && f->a[0])) continue;
        /* Anonymous / raw @as cannot be proven as a named embed. */
        if (strcmp(ast_slot(f->e), "raw") == 0 &&
            (strstr(ast_slot(f->a), "@as") != NULL || strstr(ast_slot(f->a), "/*@as*/") != NULL)) {
            shadow_safe_err_at(ctx, f, "type: anonymous as: field is ill-formed");
            return 0;
        }
        if (strcmp(ast_slot(f->e), "as") == 0 && !(f->b && f->b[0])) {
            shadow_safe_err_at(ctx, f, "type: anonymous as: field is ill-formed");
            return 0;
        }
        if (!(f->b && f->b[0])) continue;
        {
            const char* np = f->b;
            char one[64];
            while (s->nf < SHADOW_SAFE_FIELD_CAP &&
                ...
                shadow_decl_ident(one, id, sizeof(id));
                if (!id[0]) continue;
                snprintf(s->fname[s->nf], sizeof(s->fname[0]), "%s", id);
                snprintf(fty, sizeof(fty), "%s", f->a);
                if (strchr(one, '*') && !strchr(fty, '*')) {
                    size_t L = strlen(fty);
                    if (L + 1 < sizeof(fty)) {
                        fty[L] = '*';
                        fty[L + 1] = 0;
                ...
                snprintf(s->fty[s->nf], sizeof(s->fty[0]), "%s", fty);
                s->is_as[s->nf] = (strcmp(ast_slot(f->e), "as") == 0);
                s->nf++;
            }
        }
    }
...
static int shadow_safe_register_variant(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (strcmp(ast_slot(st->a), "variant") != 0 || !(st->b && st->b[0])) return 0;
    ...
    if (st->d && st->d[0] && strcmp(ast_slot(st->d), "packed") == 0)
        v->is_packed = 1;
    p = st->c;
...
/* Harvested `@variant` from included `.cch` (extract leaves enum+union C
 * in the `.h`). Safety and emit share this table — register-only, no
 * second typedef. */
enum { SHADOW_HARVEST_VARIANT_CAP = 32, SHADOW_HARVEST_ARM_CAP = 256 };
typedef struct {
    char name[64];
    char arms[SHADOW_HARVEST_ARM_CAP][64];
    char tys[SHADOW_HARVEST_ARM_CAP][96];
    int is_void[SHADOW_HARVEST_ARM_CAP];
    int narm;
    int is_packed;
} ShadowHarvestedVariant;

static ShadowHarvestedVariant g_harvested_variants[SHADOW_HARVEST_VARIANT_CAP];
static int g_nharvested_variants;
static char g_harvest_paths[64][512];
static int g_nharvest_paths;
static int g_harvest_done;
...
static int shadow_harvest_is_word(char c) {
    return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_' ||
           (c >= '0' && c <= '9');
}

static int shadow_harvest_seen_path(const char* path) {
    int i;
    if (!path || !path[0]) return 1;
    for (i = 0; i < g_nharvest_paths; i++)
        if (strcmp(g_harvest_paths[i], path) == 0) return 1;
    return 0;
}

static void shadow_harvest_mark_path(const char* path) {
    if (!path || !path[0] || g_nharvest_paths >= 64) return;
    if (shadow_harvest_seen_path(path)) return;
    snprintf(g_harvest_paths[g_nharvest_paths++],
             sizeof(g_harvest_paths[0]), "%s", path);
}

static void shadow_harvest_dir_of(const char* path, char* out, size_t cap) {
    char* sl;
    if (!out || cap == 0) return;
    snprintf(out, cap, "%s", path && path[0] ? path : ".");
    sl = strrchr(out, '/');
    if (sl && sl != out)
        *sl = 0;
    else
...
static ShadowHarvestedVariant* shadow_harvest_find(const char* name) {
    int i;
    if (!name || !name[0]) return NULL;
    for (i = 0; i < g_nharvested_variants; i++)
        if (strcmp(g_harvested_variants[i].name, name) == 0)
            return &g_harvested_variants[i];
    return NULL;
}

static void shadow_variant_harvest_add(const char* name, int packed,
                                      char arms[][64], char tys[][96],
                                      const int* is_void, int narm) {
    ShadowHarvestedVariant* v;
    int a;
    if (!name || !name[0] || narm <= 0) return;
    if (shadow_harvest_find(name)) return;
    if (g_nharvested_variants >= SHADOW_HARVEST_VARIANT_CAP) return;
    v = &g_harvested_variants[g_nharvested_variants++];
    memset(v, 0, sizeof(*v));
    snprintf(v->name, sizeof(v->name), "%s", name);
    v->is_packed = packed ? 1 : 0;
    v->narm = narm > SHADOW_HARVEST_ARM_CAP ? SHADOW_HARVEST_ARM_CAP : narm;
    for (a = 0; a < v->narm; a++) {
        snprintf(v->arms[a], sizeof(v->arms[0]), "%s", arms[a]);
        if (tys && tys[a][0])
            snprintf(v->tys[a], sizeof(v->tys[0]), "%s", tys[a]);
        v->is_void[a] = is_void && is_void[a] ? 1 : 0;
    }
}

static int shadow_variant_harvest_parse_arms(const char* body, char arms[][64],
                                            char tys[][96], int* is_void,
                                            int* narm) {
    const char* p = body;
    if (!body || !narm) return 0;
    *narm = 0;
    while (*p && *narm < SHADOW_HARVEST_ARM_CAP) {
        const char* an;
        const char* tn;
        size_t al, tl;
        while (*p == ' ' || *p == '\t' || *p == '\n' || *p == '\r' ||
        ...
        if (is_void) is_void[*narm] = (strcmp(tys[*narm], "void") == 0);
        (*narm)++;
        if (*p == ';') p++;
    }
    return *narm > 0;
...
static void shadow_variant_harvest_text(const char* text) {
    const char* p;
    int depth = 0;
    int line_cmt = 0, block_cmt = 0, str = 0, chr = 0;
    if (!text) return;
    for (p = text; *p; p++) {
        if (line_cmt) {
            if (*p == '\n') line_cmt = 0;
        ...
        if (depth != 0) continue;
        if (p[0] != '@' || strncmp(p, "@variant", 8) != 0 ||
            shadow_harvest_is_word(p[8]))
            continue;
        {
            const char* q = p + 8;
            int packed = 0;
            ...
            if (*q == '(') {
                q++;
                while (*q == ' ' || *q == '\t') q++;
                if (strncmp(q, "packed", 6) == 0 && !shadow_harvest_is_word(q[6])) {
                    packed = 1;
                    q += 6;
                }
                while (*q && *q != ')') q++;
            ...
            if (!shadow_harvest_is_word(*q) || (*q >= '0' && *q <= '9')) {
                p = q;
                continue;
            }
            nl = 0;
            while (shadow_harvest_is_word(*q) && nl + 1 < sizeof(name))
                name[nl++] = *q++;
            name[nl] = 0;
            while (*q == ' ' || *q == '\t' || *q == '\n') q++;
            if (*q != '{') {
            ...
            {
                size_t bl = (size_t)(be - bs);
                if (bl >= sizeof(body)) bl = sizeof(body) - 1;
                memcpy(body, bs, bl);
                body[bl] = 0;
            }
            if (shadow_variant_harvest_parse_arms(body, arms, tys, is_void,
                                                 &narm))
                shadow_variant_harvest_add(name, packed, arms, tys, is_void,
                                          narm);
            p = be;
            if (p > text) p--;
        }
...
static void shadow_variant_harvest_path(const char* path, int nest);

static void shadow_variant_harvest_incs(const char* text, const char* from_dir,
                                       int nest) {
    const char* p;
    int line_cmt = 0, block_cmt = 0, str = 0;
    if (!text || nest <= 0) return;
    for (p = text; *p; p++) {
        if (line_cmt) {
            if (*p == '\n') line_cmt = 0;
            continue;
        ...
        if (*p != '#') continue;
        {
            ...
            size_t n;
            while (*q == ' ' || *q == '\t') q++;
            if (strncmp(q, "include", 7) != 0 || shadow_harvest_is_word(q[7]))
                continue;
            q += 7;
            while (*q == ' ' || *q == '\t') q++;
            if (*q != '"' && *q != '<') continue;
            ...
            shadow_variant_harvest_path(cand, nest - 1);
            if (rel[0] != '/') shadow_variant_harvest_path(rel, nest - 1);
        }
    }
}
...
static void shadow_variant_harvest_path(const char* path, int nest) {
    ...
    size_t tlen = 0;
    char dir[512];
    if (!path || !path[0] || nest < 0) return;
    if (shadow_harvest_seen_path(path)) return;
    if (!read_file(path, &text, &tlen) || !text) return;
    shadow_harvest_mark_path(path);
    shadow_variant_harvest_text(text);
    shadow_harvest_dir_of(path, dir, sizeof(dir));
    shadow_variant_harvest_incs(text, dir, nest);
    free(text);
}

static void shadow_variant_harvest_scan(TapeCache* cache) {
    size_t i, n;
    if (g_harvest_done) return;
    g_harvest_done = 1;
    n = cc_lowered_local_header_count();
    for (i = 0; i < n; i++)
        shadow_variant_harvest_path(cc_lowered_local_header_source_path(i), 2);
    n = cc_included_cch_source_count();
    for (i = 0; i < n; i++)
        shadow_variant_harvest_path(cc_included_cch_source_path(i), 2);
    if (cache) {
        int fi;
        for (fi = 0; fi < cache->n; fi++) {
            ...
            if (ft->bytes && ft->len > 0)
                shadow_variant_harvest_incs(ft->bytes, dir, 2);
        }
    }
}

static void shadow_safe_install_harvested(ShadowSafeCtx* ctx) {
    int i, a;
    if (!ctx) return;
    for (i = 0; i < g_nharvested_variants; i++) {
        ShadowHarvestedVariant* h = &g_harvested_variants[i];
        ShadowSafeVariant* v;
        if (shadow_safe_find_variant(ctx, h->name)) continue;
        if (ctx->nvariants >= SHADOW_SAFE_VARIANT_CAP) return;
        v = &ctx->variants[ctx->nvariants++];
        memset(v, 0, sizeof(*v));
        snprintf(v->name, sizeof(v->name), "%s", h->name);
        v->is_packed = h->is_packed;
        for (a = 0; a < h->narm && v->narm < SHADOW_SAFE_VARM_CAP; a++)
            snprintf(v->arms[v->narm++], sizeof(v->arms[0]), "%s", h->arms[a]);
        if (h->name[0]) shadow_safe_add_type_name(ctx, h->name);
    }
}
...
static void shadow_safe_check_as_embeds(ShadowSafeCtx* ctx, AstNode** items,
    ...
    for (i = 0; i < n && !ctx->err; i++) {
        ...
        int nseen = 0;
        if (!it || it->kind != AST_TYPEDEF_STRUCT || !(it->b && it->b[0])) continue;
...
static void shadow_safe_emit_channel_stable_hints(const char* payload) {
    diag_note("non-unique slices (arena / stack / untracked "
              "from_buffer) may dangle after send (channel-stable-borrow)");
    diag_plain("  hint: include a CCArena in the same message, use unique "
               "T[:!] / cc_slice_from_static, or use send_into / "
               "try_send_into");
    /* Match production checker: channel macros copy by value into __cc_tmp. */
    (void)payload;
    diag_note("'__cc_tmp' is the channel send macro's by-value copy of "
              "the payload");
}
...
static void shadow_safe_check_channel_pair_text(ShadowSafeCtx* ctx, AstNode* st,
    ...
    if (!ctx->cache || !st->file_id) {
        diag_ft(ctx->cache, NULL, 0, "channel: cc_channel_pair");
        return;
    }
    ft = tape_by_id(ctx->cache, st->file_id);
    if (!ft || !ft->bytes) {
    ...
    }
    {
        ...
        t.file_id = st->file_id;
        t.offset = off;
        diag_at(ctx->cache, t, "channel: cc_channel_pair");
    }
}
...
static void shadow_safe_on_ufcs(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (strcmp(ast_slot(st->b), "recv") == 0 && ctx->in_async &&
        !(st->d && st->d[0] && strncmp(ast_slot(st->d), "bang", 4) == 0)) {
        shadow_safe_err_at(
            ctx, st,
    ...
    if (strcmp(ast_slot(st->b), "destroy") != 0 && strcmp(ast_slot(st->b), "free") != 0 &&
        strcmp(ast_slot(st->b), "reset") != 0)
        return;
    ...
    if (v->is_arena &&
        (strcmp(ast_slot(st->b), "reset") == 0 || strcmp(ast_slot(st->b), "free") == 0 ||
         strcmp(ast_slot(st->b), "destroy") == 0)) {
        snprintf(args, sizeof(args), "&%s", st->a);
        if (strcmp(ast_slot(st->b), "reset") == 0)
            shadow_safe_on_arena_epoch(ctx, st, "cc_arena_reset", args);
        else if (strcmp(ast_slot(st->b), "free") == 0)
...
static int shadow_safe_stmt_diverges(AstNode* st) {
    if (!st) return 0;
    if (st->kind == AST_EXPR_STMT && st->a && st->a[0]) {
        const char* p = st->a;
        while (*p == ' ' || *p == '\t') p++;
    ...
    if (st->kind == AST_RETURN_EXPR || st->kind == AST_RETURN_INT ||
        ...
        return 1;
    if (st->kind == AST_CALL_ARGS || st->kind == AST_CALL_NUM) {
        if (strcmp(ast_slot(st->a), "exit") == 0 || strcmp(ast_slot(st->a), "abort") == 0 ||
            strcmp(ast_slot(st->a), "_exit") == 0 || strcmp(ast_slot(st->a), "longjmp") == 0 ||
            strcmp(ast_slot(st->a), "cc_error_exit") == 0 || strcmp(ast_slot(st->a), "goto") == 0)
            return 1;
    }
...
/* Site `!>(e){…}` shares AST_ERRHANDLER with `@errhandler` but is not
 * a scope handler. Bare `!>;` in its body dispatches to the enclosing
 * `@errhandler` — that is composition, not re-entry. */
static int shadow_safe_eh_is_site(AstNode* eh) {
    return eh && eh->kind == AST_ERRHANDLER && eh->e && eh->e[0] == 's';
}
...
static int shadow_safe_diag_eh_e(ShadowSafeCtx* ctx, AstNode* st,
    ...
    for (i = ctx->neh - 1; i >= 0; i--) {
        ...
        if (eh && eh->a && eh->a[0] && strcmp(ast_slot(eh->a), ety) == 0) exact++;
    }
    if (exact) return 0;
    for (i = ctx->neh - 1; i >= 0; i--) {
        AstNode* eh = ctx->eh_stack[i];
        if (!eh || !(eh->a && eh->a[0]) || strcmp(ast_slot(eh->a), ety) == 0) continue;
...
static void shadow_safe_check_err_fwds(ShadowSafeCtx* ctx, AstNode* site,
    ...
    for (i = 0; i < nbody && !ctx->err; i++) {
        ...
        if (st->c && st->c[0] && strcmp(ast_slot(st->c), "delegate") == 0) continue;
        if (!binder || !binder[0]) {
            char msg[160];
            snprintf(msg, sizeof(msg),
        ...
        if (st->a && st->a[0] && strcmp(ast_slot(st->a), binder) != 0) {
            char msg[192];
            snprintf(msg, sizeof(msg),
        ...
        if (!shadow_safe_body_diverges(eh->body, eh->nbody)) {
            ...
            if (!(eh->c && eh->c[0] &&
                  (strcmp(ast_slot(eh->c), "exit") == 0 || strcmp(ast_slot(eh->c), "abort") == 0 ||
                   strcmp(ast_slot(eh->c), "_exit") == 0 ||
                   strcmp(ast_slot(eh->c), "cc_error_exit") == 0 ||
                   strcmp(ast_slot(eh->c), "longjmp") == 0))) {
                shadow_safe_err_at(ctx, eh,
...
static void shadow_safe_diag_bare_bang_reenter(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    for (i = 0; i < ctx->neh; i++) {
        AstNode* eh = ctx->eh_stack[i];
        if (!eh || shadow_safe_eh_is_site(eh)) continue;
...
static void shadow_safe_on_unwrap(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (st->kind == AST_VAR_UNWRAP) {
        ...
        if (strcmp(mode, "bang") == 0) {
            ...
            if (st->nbody == 0 || (st->e && st->e[0] && strstr(ast_slot(st->e), "!>")) ) {
                if (shadow_safe_diag_eh_e(ctx, st, st->a)) return;
                shadow_safe_require_eh(ctx, st, NULL);
            }
    ...
    if (st->kind == AST_PTR_UNWRAP) {
        ...
        if (strncmp(mode, "bang_eh", 7) == 0) {
            if (shadow_safe_diag_eh_e(ctx, st, st->c && st->c[0] ? st->c : st->a))
                return;
            shadow_safe_require_eh(ctx, st, NULL);
        }
    ...
    if (st->kind == AST_PRINTLN_BANG || st->kind == AST_PRINTLN_TPL) {
        if (st->nbody > 0)
            ...
            if (shadow_safe_diag_eh_e(ctx, st, (st->d && st->d[0] == 'e')
                                                ? "cc_eprintln("
                                                : "cc_println("))
                return;
            shadow_safe_require_eh(ctx, st, NULL);
...
static int shadow_safe_fn_returns_result(const AstNode* fn) {
    ...
    if (fn->kind == AST_ASYNC_FN && strcmp(ast_slot(fn->e), "result") == 0) return 1;
    return 0;
}
...
static void shadow_safe_on_unhandled_call(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (st->kind == AST_UFCS_STMT) {
        ...
        if (cc_result_fn_registry_is_discard_ok(st->b, strlen(ast_slot(st->b)))) return;
        callee = st->b;
        ...
        if (cc_result_fn_registry_is_discard_ok(st->a, strlen(ast_slot(st->a)))) return;
        callee = st->a;
    } else {
        return;
    }
...
static int shadow_safe_body_mutates_ident(AstNode** body, int n,
    ...
    for (i = 0; i < n; i++) {
        ...
        char lname[64];
        if (!st) continue;
        if (st->kind == AST_INC && strcmp(ast_slot(st->a), name) == 0) return 1;
    ...
    while (p && *p) {
        ...
        if (v && ((v->is_slice && v->is_stack_slice_view) || v->is_scratch_string)) {
            shadow_safe_err_at(
                ...
                "closure");
            diag_note("the closure outlives the stack / @scratch frame");
            diag_plain("  hint: pass as parameter, or allocate with an explicit "
                       "heap/arena");
            return;
        }
    }
...
static void shadow_safe_on_spawn(ShadowSafeCtx* ctx, AstNode* st, int escapes) {
    ...
    int is_unsafe;
    if (!ctx || !st || ctx->err) return;
    is_unsafe = (strcmp(ast_slot(st->b), "spawn_unsafe") == 0 ||
...
static void shadow_safe_walk_attached_closures(ShadowSafeCtx* ctx, AstNode* st,
    ...
    for (k = 0; k < st->ndbody && !ctx->err; k++) {
        ...
        if (cl->b && cl->b[0] && (strcmp(ast_slot(cl->b), "send_task") == 0 ||
                         strcmp(ast_slot(cl->b), "send_task_hybrid") == 0))
            ctx->in_send_task = 1;
        saved_npend = ctx->npending_moves;
        {
            /* Nursery spawn is scoped for stack slices (`escapes` 0) but
             * value-captured pointer alias mutation still task-escapes. */
            int mut_esc = escapes;
            if (st->kind == AST_UFCS_STMT &&
                (strcmp(ast_slot(st->b), "spawn") == 0 ||
                 strcmp(ast_slot(st->b), "spawnhybrid") == 0))
                mut_esc = 1;
            shadow_safe_on_spawn(ctx, cl, mut_esc);
        }
        {
            int saved_nndefer = ctx->nndefer;
            int saved_ndefer_block = ctx->ndefer_block;
            ctx->ndefer_block++;
            if (!ctx->err) shadow_safe_walk_list(ctx, cl->body, cl->nbody);
            ctx->nndefer = saved_nndefer;
            ctx->ndefer_block = saved_ndefer_block;
        }
...
static void shadow_safe_walk_stmt(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    case AST_TYPEDEF_CHAN: {
        int role = 0;
        if ((st->c && st->c[0] == '>')) role = 1;
        else if ((st->c && st->c[0] == '<')) role = 2;
        if (st->a && st->a[0] && role) shadow_safe_add_chan_td(ctx, st->a, role);
        break;
    }
    case AST_CHAN_VAR: {
        const char* p = st->a;
        ...
        size_t ni;
        int role = 0;
        if (strcmp(ast_slot(st->c), ">") == 0) role = 1;
        else if (strcmp(ast_slot(st->c), "<") == 0) role = 2;
        while (*p) {
            ShadowSafeVar* cv;
            while (*p == ' ' || *p == '\t' || *p == ',') p++;
            if (!*p) break;
    ...
    case AST_VAR_DECL: {
        ...
        ShadowSafeVar* cv;
        if (st->a && st->a[0]) shadow_safe_check_map_key_text(ctx, st, st->a);
        if (st->b && st->b[0] && !ctx->err) {
            const char* bp = st->b;
            while (*bp && !ctx->err) {
                char name[64];
                size_t ni = 0;
                ...
                if (*bp == ',') bp++;
                if (!name[0]) continue;
                cv = shadow_safe_add(ctx, name);
                if (cv) {
                    snprintf(cv->ty, sizeof(cv->ty), "%s", st->a);
                    if (role) cv->is_chan = role;
                    if (strchr(ast_slot(st->a), '*') || strchr(ast_slot(st->b), '*'))
                        cv->is_ptr = 1;
                    if ((st->c && st->c[0] == '[') || this_arr) cv->is_stack_array = 1;
                }
            }
        }
        break;
    ...
    case AST_PTR_INIT:
        ...
        if (!ctx->err && st->c && st->c[0] &&
            (strstr(ast_slot(st->c), "@create") || strstr(ast_slot(st->c), "__cc_at_create"))) {
            char msg[192];
    ...
    case AST_VAL_DESTROY:
         ...
         * `T x@(args) !>(e){…} @destroy` parks the site handler on dbody. */
        {
            int k;
            int site = 0;
            for (k = 0; k < st->ndbody && !site; k++)
                if (st->dbody[k] && st->dbody[k]->kind == AST_ERRHANDLER)
                    site = 1;
            if (!ctx->err && !site &&
                ((st->c && st->c[0] && strstr(ast_slot(st->c), "!>") != NULL) ||
                 (st->f && st->f[0] && strstr(st->f, "bang") != NULL)))
                shadow_safe_require_eh(ctx, st, NULL);
            for (k = 0; k < st->ndbody && !ctx->err; k++) {
                if (st->dbody[k] && st->dbody[k]->kind == AST_ERRHANDLER)
                    shadow_safe_walk_stmt(ctx, st->dbody[k]);
            }
        }
        if (!ctx->err) shadow_safe_walk_attached_closures(ctx, st, 0);
        break;
    case AST_ASSIGN:
        ...
        if (!ctx->err) {
            ...
            if (shadow_safe_plain_ident(st->a, lhs, sizeof(lhs)) &&
                ...
                escapes = 1;
            else if (strstr(ast_slot(st->a), ".") != NULL || strstr(ast_slot(st->a), "->") != NULL)
                escapes = 1;
    ...
    case AST_UFCS_EXPR:
        ...
        {
            /* Nursery spawn is scoped — same as the old AST_SPAWN_CLOSURE path. */
            int escapes = 1;
            if (strcmp(ast_slot(st->b), "spawn") == 0 ||
                strcmp(ast_slot(st->b), "spawnhybrid") == 0)
                escapes = 0;
            if (!ctx->err)
                shadow_safe_walk_attached_closures(ctx, st, escapes);
    ...
    case AST_SPAWN_CLOSURE: {
        ...
        int is_nursery =
            strcmp(ast_slot(st->b), "spawn") == 0 || strcmp(ast_slot(st->b), "spawn_unsafe") == 0 ||
            strcmp(ast_slot(st->b), "spawnhybrid") == 0 ||
            strcmp(ast_slot(st->b), "spawnhybrid_unsafe") == 0;
    ...
    case AST_ERRHANDLER:
        if (!shadow_safe_eh_is_site(st))
            shadow_safe_push_eh(ctx, st);
        shadow_safe_walk_list(ctx, st->body, st->nbody);
        break;
    case AST_ERR_SYNTAX:
        if (st->nbody == 0 && !(st->d && st->d[0]) && !shadow_safe_eh(ctx)) {
            shadow_safe_err_at(ctx, st,
                               "@err with no local handler requires @errhandler");
            return;
        }
        if (st->nbody > 0) {
            int bi;
            const char* bind = st->d && st->d[0] ? strrchr(st->d, ' ') : NULL;
            if (bind) bind++;
            else bind = st->d && st->d[0] ? st->d : NULL;
            for (bi = 0; bi < st->nbody && !ctx->err; bi++) {
                AstNode* fwd = st->body[bi];
                if (!fwd || fwd->kind != AST_ERR_FWD ||
                    strcmp(ast_slot(fwd->c), "delegate") != 0)
                    continue;
                if (!bind || !bind[0] || strcmp(ast_slot(fwd->a), bind) != 0 ||
                    !shadow_safe_eh(ctx)) {
                    shadow_safe_err_at(
                        ctx, fwd,
    ...
    case AST_NURSERY_DESTROY: {
        ...
        if (st->kind == AST_NURSERY_DESTROY && st->b && st->b[0])
        ...
        for (k = 0; k < st->nbody; k++) {
            ...
            if (s->kind == AST_NURSERY_DESTROY) {
                ...
                nty = s->a;
                       ...
                       s->d && s->d[0] && strstr(ast_slot(s->d), "_D") != NULL) {
                nname = s->b;
                nty = s->a;
            }
    ...
    case AST_SWITCH: {
        ...
        shadow_safe_check_type_of_text(ctx, st, st->d);
        if (ctx->err) return;
        /* Structured switches keep case/stmt nodes on body[]. Opaque
         * domination lives on st->d; body[] walk must re-apply the
         * current `case .arm:` or `val->str` is an unprotected proj. */
        if (st->nbody > 0 && shadow_safe_switch_root(st->a, root, sizeof(root))) {
            int i;
            int pushed = 0;
            for (i = 0; i < st->nbody && !ctx->err; i++) {
                AstNode* kid = st->body[i];
                char arm[32];
                if (kid && kid->kind == AST_RAW_LINE && kid->a && kid->a[0]) {
                    if (pushed) {
                        shadow_safe_dom_pop(ctx);
                        pushed = 0;
                    }
                    if (shadow_safe_case_label_arm(ctx, root, kid->a, arm,
                                                   sizeof(arm))) {
                        shadow_safe_dom_push(ctx, root, arm);
                        pushed = 1;
                    }
                ...
                if (kid) shadow_safe_walk_stmt(ctx, kid);
            }
            if (pushed) shadow_safe_dom_pop(ctx);
        } else {
            shadow_safe_walk_list(ctx, st->body, st->nbody);
    ...
    case AST_CANCEL_DEFER: {
        int i, found = 0;
        if (!(st->a && st->a[0])) {
            shadow_safe_err_at(ctx, st, "@cancel_defer requires a name");
            return;
        }
        for (i = ctx->nndefer - 1; i >= 0; i--) {
            if (strcmp(ctx->ndefer_name[i], st->a) == 0) {
                found = 1;
                break;
            }
        }
    ...
    case AST_STAGE:
        if (st->kind == AST_DEFER) {
            if (st->a && st->a[0]) {
                int i;
                for (i = 0; i < ctx->nndefer; i++) {
                    if (ctx->ndefer_scope[i] == ctx->ndefer_block &&
                        strcmp(ctx->ndefer_name[i], st->a) == 0) {
                        char msg[160];
                        snprintf(msg, sizeof(msg),
                                 "redeclaration of named @defer '%s'", st->a);
                        shadow_safe_err_at(ctx, st, msg);
                        return;
                    }
                }
                if (ctx->nndefer >= SHADOW_SAFE_NDEFER_CAP) {
                    shadow_safe_err_at(ctx, st,
                                       "too many named @defer in function");
                    return;
                }
                snprintf(ctx->ndefer_name[ctx->nndefer],
                         sizeof(ctx->ndefer_name[0]), "%s", st->a);
                ctx->ndefer_scope[ctx->nndefer] = ctx->ndefer_block;
                ctx->nndefer++;
            }
            if ((strcmp(ast_slot(st->c), "ok") == 0 || strcmp(ast_slot(st->c), "err") == 0) &&
         ...
         * no re-raise edge. */
        if (st->kind == AST_PARALLEL_FOR && st->e && st->e[0] && !st->forced_seq &&
    ...
    case AST_VAR_UNWRAP: {
        ...
        int prot = st->c && st->c[0] && (strncmp(ast_slot(st->c), "bang", 4) == 0 ||
                                strncmp(ast_slot(st->c), "qmark", 5) == 0);
    ...
    case AST_PTR_UNWRAP: {
        int prot = st->d && st->d[0] && (strncmp(ast_slot(st->d), "bang", 4) == 0 ||
                                strncmp(ast_slot(st->d), "qmark", 5) == 0);
        shadow_safe_commit_moves_in_text(ctx, st, st->c);
        if (ctx->err) return;
        shadow_safe_check_moved_use(ctx, st, st->c);
        if (ctx->err) return;
...
static void shadow_safe_walk_fn(ShadowSafeCtx* ctx, AstNode* fn) {
    ...
    saved_nndefer = ctx->nndefer;
    saved_ndefer_block = ctx->ndefer_block;
    saved_fn = ctx->cur_fn;
    ctx->cur_fn = fn;
    ctx->ndefer_block++;
...
/* Missing field: unique `as:` hop is emit's job; else suggest peel. */
...
static void shadow_safe_check_field_suggest_text(ShadowSafeCtx* ctx, AstNode* st,
    ...
    while (*p && !ctx->err) {
        ...
        /* Unique `as:` face that has the field: emit hops. Two faces:
         * ambiguous. A nested member without `as:` still suggests peel. */
        {
            int as_hits = 0;
            int nest_i = -1;
            ...
            if (as_hits > 1) {
                char msg[192];
                snprintf(msg, sizeof(msg),
                         "as: ambiguous field '%s' on '%s'", field,
                         outer->name);
                shadow_safe_err_at(ctx, st, msg);
                return;
            }
            if (as_hits == 1) continue;
            if (nest_i >= 0) {
                char msg[192];
                snprintf(msg, sizeof(msg),
                         "field not found: %s — member '%s' has it; "
                         "spell '.%s.%s'",
                         field, outer->fname[nest_i], outer->fname[nest_i],
                         field);
                shadow_safe_err_at(ctx, st, msg);
                return;
            }
...
static int shadow_safety_check(AstNode** items, int n, TapeCache* cache) {
    ShadowSafeCtx ctx;
    /* The struct table is the arena's, not the frame's: it is one row per
     * typedef that names a struct, and a translation unit the size of a
     * lowerer has hundreds. On the stack the cap and the frame grow
     * together, which is what kept the cap at a number real units reach. */
    ShadowSafeStruct* structs;
    ShadowSafeVariant variants[SHADOW_SAFE_VARIANT_CAP];
    CCArena safe_ar;
    size_t structs_bytes = sizeof(ShadowSafeStruct) * (size_t)SHADOW_SAFE_STRUCT_CAP;
    int i;
    int ok;
    const char* strict;
    ...
    structs = (ShadowSafeStruct*)cc_arena_alloc(safe_ar, structs_bytes,
                                               _Alignof(ShadowSafeStruct));
    if (!structs) {
        fprintf(stderr, "error: safety check: out of memory for the struct table\n");
        if (cc_arena_is_live(safe_ar)) cc_arena_free(&safe_ar);
        return 0;
    }
    memset(structs, 0, structs_bytes);
    ctx.safe_ar = safe_ar;
    ctx.cache = cache;
    ctx.structs = structs;
    ctx.nstructs = 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!fname || strncmp(fname, pre, pl) != 0) {
            if (it->kind == AST_RAW_LINE && it->a && it->a[0]) {
                const char* hit = strstr(ast_slot(it->a), pre);
    ...
    for (i = 0; i < n; i++) {
        ...
        if (it->kind == AST_VAR_DECL || it->kind == AST_TYPED_INIT ||
            ...
            if (it->b && it->b[0]) shadow_safe_add_global(&ctx, it->b);
        } else if (it->kind == AST_CHAN_VAR || it->kind == AST_VAR_INT) {
            if (it->a && it->a[0]) shadow_safe_add_global(&ctx, it->a);
        }
        /* Container instances used in the TU are codegen-registered. */
        {
            ...
            if (it->kind == AST_FN || it->kind == AST_STATIC_FN ||
                ...
                for (k = 0; k < it->nkids; k++) {
                    ...
                    char base[96];
                    size_t bn = 0;
                    if (!kid || !(kid->a && kid->a[0])) continue;
                    if (strncmp(ast_slot(kid->a), "CCVec_", 6) != 0 &&
                        strncmp(ast_slot(kid->a), "Map_", 4) != 0 &&
                        strncmp(ast_slot(kid->a), "ArrayMap_", 9) != 0 &&
                        strncmp(ast_slot(kid->a), "Table_", 6) != 0)
                        continue;
        ...
        if (it->kind == AST_TYPEDEF_STRUCT) {
            ...
            if (it->b && it->b[0]) shadow_safe_add_type_name(&ctx, it->b);
            if (it->a && it->a[0]) shadow_safe_add_type_name(&ctx, it->a);
        } else if (it->kind == AST_TYPEDEF_INT || it->kind == AST_TYPEDEF_ENUM ||
                   it->kind == AST_TYPEDEF_FN_PTR ||
                   it->kind == AST_TYPEDEF_CHAN) {
            if (it->kind == AST_TYPEDEF_CHAN && it->a && it->a[0]) {
                int role = 0;
                if ((it->c && it->c[0] == '>')) role = 1;
                else if ((it->c && it->c[0] == '<')) role = 2;
                shadow_safe_add_type_name(&ctx, it->a);
                if (role) shadow_safe_add_chan_td(&ctx, it->a, role);
            } else if (it->kind == AST_TYPEDEF_INT && it->a && it->a[0]) {
                shadow_safe_add_type_name(&ctx, it->a);
                if (it->b && it->b[0]) shadow_safe_add_td_alias(&ctx, it->a, it->b);
            } else if (it->b && it->b[0])
                shadow_safe_add_type_name(&ctx, it->b);
            else if (it->a && it->a[0])
                shadow_safe_add_type_name(&ctx, it->a);
            if (it->kind == AST_TYPEDEF_INT) {
                if (it->a && it->a[0]) shadow_safe_check_map_key_text(&ctx, it, it->a);
                if (it->b && it->b[0]) shadow_safe_check_map_key_text(&ctx, it, it->b);
            }
        } else if (it->kind == AST_STRUCT && it->a && it->a[0]) {
            shadow_safe_add_type_name(&ctx, it->a);
        }
        if (it->kind == AST_AT_STMT && strcmp(ast_slot(it->a), "variant") == 0) {
        ...
        if (it->kind == AST_RESULT_FN && it->a && it->a[0])
            shadow_safe_add_result_fn_err(&ctx, it->a,
                                          it->b && it->b[0] ? it->b : NULL,
                                          it->forced_seq);
        fname = NULL;
        params = NULL;
        if (it->kind == AST_FN) {
    ...
    shadow_safe_install_harvested(&ctx);
