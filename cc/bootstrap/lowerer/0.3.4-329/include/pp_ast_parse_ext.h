...
static AstNode* parse_fn(Parser* p) {
    if (!shadow_cparse_fn_in_attach) {
        ...
        if (cp < 0) return NULL;
        if (cp == 1) {
            shadow_cparse_fn_in_attach = 1;
            n = parse_fn(p);
            shadow_cparse_fn_in_attach = 0;
            (void)shadow_cparse_fn_take_owned();
            return n;
        }
    }
    int ti = p->i;
    ...
    {
        int ty_end = ti + 1;
        int iend = has_struct ? -1 : peek_c_int_type_end(p, ti);
        int is_slice = 0;
        int is_slice_unique = 0;
        /* Owner splice keeps `static long long f` as written. */
        if (!has_struct &&
            ast_try_generic_ret(p, ti, &ty_end, &name_i, gret, sizeof(gret))) {
            /* Name::[args] return — spelled into gret. */
        } else if (iend > ti) {
            ty_end = iend;
            name_i = iend;
        }
        if (!gret[0] && !has_struct && name_i + 2 < p->n &&
        ...
        while (p->i < ty_end) p_next(p); /* ret / tag / long long */
        if (is_slice) {
            p_next(p); /* [ */
            p_next(p); /* : */
        ...
        n->b = ast_arena_cstr(p, params);
        {
            ...
            if (has_const && has_struct)
                ...
                if (has_const)
                    snprintf(rbuf, sizeof(rbuf), "const %s", gret);
                else
                    snprintf(rbuf, sizeof(rbuf), "%s", gret);
            } else if (ty_end > ti + 1) {
                if (!ast_spell_token_range(p, ti, ty_end, rbuf, sizeof(rbuf)) &&
                    !span_text(p, ti, ty_end, rbuf, sizeof(rbuf)))
                    return NULL;
                if (has_const) {
                    char prefixed[256];
                    snprintf(prefixed, sizeof(prefixed), "const %s", rbuf);
                    snprintf(rbuf, sizeof(rbuf), "%s", prefixed);
                }
            } else if (has_const)
            ...
            n->e = ast_arena_cstr(p, rbuf);
        }
        {
            int body0 = p->i;
            if (shadow_cparse_fn_in_attach) {
                if (!shadow_cparse_attach_fn_body(p, n, ti, body0))
                    return NULL;
            } else {
...
static AstNode* parse_with_deadline(Parser* p) {
    ...
    if (!n) return NULL;
    if (e0 < e1 && (((n->a = ast_arena_span(p, e0, e1))), p->err)) {
    ...
    while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF && !p->err) {
        AstNode* s = parse_stmt(p);
        if (!s) return NULL;
        if (!ast_body_push(p, n, s)) return NULL;
    }
    if (!p_accept(p, TK_PUNCT, "}")) {
...
/* @parallel { name = expr; | expr; | @serial { … }; … }
 * @parallel spawn { … }
 * @parallel (pred) { … }
 * @parallel for (name in lo..hi) { … } */
static int parallel_is_decl_arm(AstNode* s) {
    if (!s) return 0;
    return s->kind == AST_VAR_DECL || s->kind == AST_VAR_INT ||
           s->kind == AST_PTR_DECL || s->kind == AST_TYPED_INIT ||
           s->kind == AST_RESULT_LOCAL || s->kind == AST_SLICE_VAR ||
           s->kind == AST_VAR_UNWRAP || s->kind == AST_PTR_UNWRAP ||
           s->kind == AST_PTR_INIT || s->kind == AST_SLICE_INIT ||
           s->kind == AST_VAL_DESTROY || s->kind == AST_STATIC_VAR;
}

/* An expression (or other executable) statement: no dest, just run. */
static int parallel_expr_arm(AstNode* s) {
    if (!s) return 0;
    if (s->kind == AST_FOR || s->kind == AST_BLOCK) return 0;
    if (s->kind == AST_DEFER || s->kind == AST_ERRHANDLER) return 0;
    if (s->kind == AST_PARALLEL || s->kind == AST_PARALLEL_FOR ||
        s->kind == AST_SERIAL)
        return 0;
    if (parallel_is_decl_arm(s)) return 0;
    return 1;
}
...
static void serial_walk_dests(AstNode* s, char locals[][64], int nloc,
    ...
    if (s->kind == AST_ASSIGN)
        lhs = s->a;
    else if (s->kind == AST_STMT_UNWRAP && s->e && s->e[0])
        lhs = s->e; /* `x = call !>…;` — assign-unwrap keeps its lhs in e */
    if (lhs && parallel_lhs_is_ident(lhs)) {
        char id[128];
        serial_trim_ident(lhs, id, sizeof(id));
...
static AstNode* parse_serial_arm(Parser* p) {
    ...
    }
    {
        ...
        if (nd > 1) {
            parser_fail(p, at,
                        "@serial arm must assign at most one simple outer name");
            return NULL;
        }
        if (nd == 1)
            n->a = ast_arena_cstr(p, dests[0]);
    }
    return n;
}
...
static AstNode* parse_parallel_block(Parser* p) {
    ...
    while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF &&
        ...
        if (tok_eq(at, TK_PUNCT, "@") && p->i + 1 < p->n &&
            ...
            s = parse_stmt(p);
            if (!s) return NULL;
            if (s->kind == AST_ASSIGN && (!(s->c && s->c[0]) || strcmp(ast_slot(s->c), "=") == 0) &&
                parallel_lhs_is_ident(s->a)) {
                /* assignment arm */
            } else if (s->kind == AST_ASSIGN) {
                parser_fail(p, at,
                            "@parallel left-hand side must be a simple name");
                return NULL;
            } else if (parallel_expr_arm(s)) {
                /* Expression arm, including `x = f() !>…;`. Same one-outer-
                 * name rule as @serial, so the thunk writes the outer name
                 * through its out pointer instead of an unbound `x`. */
                AstNode* ser = ast_new(p, AST_SERIAL);
                char locals[32][64];
                char dests[8][128];
                int nloc = 0, nd = 0;
                if (!ser) return NULL;
                if (!ast_body_push(p, ser, s)) return NULL;
                serial_walk_locals(s, locals, &nloc, 32);
                serial_walk_dests(s, locals, nloc, dests, &nd, 8);
                if (nd > 1) {
                    parser_fail(p, at,
                                "@parallel arm must assign at most one simple outer name");
                    return NULL;
                }
                if (nd == 1)
                    ser->a = ast_arena_cstr(p, dests[0]);
                s = ser;
            } else if (s->kind == AST_FOR) {
                parser_fail(p, at,
                            "for is not an @parallel { } arm; use @parallel for "
                            "or @serial { … }");
                return NULL;
            } else {
                parser_fail(p, at,
                            "@parallel arm must be an assignment, an "
                            "expression, or `@serial { … }`");
                return NULL;
            }
        }
        if (!ast_body_push(p, n, s)) return NULL;
    ...
    if (n->nbody < 1) {
        parser_fail(p, p_peek(p), "@parallel needs at least one arm");
        return NULL;
    }
    return n;
}
...
static int peek_parallel_at_for(Parser* p) {
    return p->i + 1 < p->n && tok_eq(p->toks[p->i], TK_PUNCT, "@") &&
           shadow_kw(p->toks[p->i + 1]) == SHADOW_KW_FOR;
}

static int peek_parallel_for(Parser* p) {
    return shadow_kw(p_peek(p)) == SHADOW_KW_FOR;
}
...
static AstNode* parse_parallel_for(Parser* p, const char* gate,
    ...
    int depth;
    AstNode* n;
    if (peek_parallel_at_for(p)) {
        parser_fail(p, p_peek(p),
                    "`@for` is standalone only; use `@parallel for (...)`");
        return NULL;
    }
    if (!peek_parallel_for(p)) {
        parser_fail(p, p_peek(p), "expected 'for' after @parallel");
        return NULL;
    }
    p_next(p); /* for */
    if (!tok_eq(p_peek(p), TK_PUNCT, "(")) {
    ...
    n->forced_seq = p->parallel_off;
    n->a = ast_arena_slice(p, name.spell);
    if (gate && gate[0]) n->e = ast_arena_cstr(p, gate);
    if (seq_cond && seq_cond[0]) n->f = ast_arena_cstr(p, seq_cond);
    {
        char lo[2048], hi[2048];
        if (!parallel_split_range(p, rp, lo, sizeof(lo), hi, sizeof(hi))) {
            parser_fail(p, p_peek(p),
                        "expected `lo..hi` after @parallel for (name in");
            return NULL;
        }
        n->b = ast_arena_cstr(p, lo);
        n->c = ast_arena_cstr(p, hi);
    }
    p->i = rp;
    if (!p_accept(p, TK_PUNCT, ")")) {
...
static const char* shadow_pw_result_dest(AstNode* n) {
    ...
    for (k = 0; k < n->ndbody; k++) {
        AstNode* d = n->dbody[k];
        if (d && d->kind == AST_VAR_DECL && d->b && d->b[0] &&
            (!(d->a && d->a[0]) || strcmp(ast_slot(d->a), "CCParallel") != 0))
            return d->b;
    }
    return NULL;
}

static AstNode* shadow_par_dest_var(AstNode* n) {
    int k;
    if (!n) return NULL;
    if (n->kind != AST_PARALLEL && n->kind != AST_PARALLEL_FOR)
        return NULL;
    for (k = 0; k < n->ndbody; k++) {
        AstNode* d = n->dbody[k];
        if (!d || d->kind != AST_VAR_DECL || !(d->b && d->b[0]))
            continue;
        /* Brace dest may be an untyped assign (`h = @parallel { }`).
         * Wait-for handle dest is the typed `CCParallel` bind only —
         * an untyped assign stays the bool dest. */
        if (n->kind == AST_PARALLEL_FOR &&
            (!(d->a && d->a[0]) || strcmp(ast_slot(d->a), "CCParallel") != 0))
            continue;
        return d;
    }
...
static const char* shadow_par_handle_dest(AstNode* n) {
    AstNode* d = shadow_par_dest_var(n);
    return d ? d->b : NULL;
}

static int shadow_par_dest_has_destroy(AstNode* n) {
    AstNode* d = shadow_par_dest_var(n);
    const char* mode;
    if (!d || !(d->e && d->e[0])) return 0;
    mode = ast_slot(d->e);
    return strcmp(mode, "_Dbare") == 0 || strcmp(mode, "_D") == 0;
}
...
static int parallel_attach_dest(Parser* p, AstNode* n, const char* name,
    ...
    if (!d) return 0;
    if (ty && ty[0]) d->a = ast_arena_cstr(p, ty);
    d->b = ast_arena_cstr(p, name);
    if (!ast_dbody_push(p, n, d)) return 0;
    return 1;
}

/* Dest on assignment join: `!>` unwraps CCParallel !>(CCError) into the
 * promise. Wait-for bool dests live on AST_PARALLEL_FOR; a typed
 * `CCParallel` dest on wait-for is the handle (live during enter). */
static int parallel_dest_is_handle(AstNode* n) {
    return shadow_par_handle_dest(n) != NULL;
}

/* Dest-live one-arm `@serial`: spawned worker, no kick. An outer
 * assign (`saw = …`) does not make this the caller — the dest is
 * planted and the caller proceeds. */
static int parallel_dest_expr_worker(AstNode* n) {
    return n && n->kind == AST_PARALLEL && n->nbody == 1 && n->body[0] &&
           n->body[0]->kind == AST_SERIAL &&
           !n->par_wake && parallel_dest_is_handle(n);
}

static int parse_parallel_bang_tail(Parser* p, AstNode* n) {
    Token at;
    int have_paren = 0;
    /* `@parallel {…}` is CCParallel !>(CCError). `!>` is the usual unwrap;
     * the promise is that Ok. `.wait()` / `.cancel()` are UFCS on it. */
    if (!tok_eq(p_peek(p), TK_PUNCT, "!>")) {
        parser_fail(p, p_peek(p),
                    "@parallel {…} is CCParallel !>(CCError); unconsumed Result");
        return 0;
    }
    at = p_next(p); /* unwrap create */
    if (tok_eq(p_peek(p), TK_PUNCT, ".")) {
        AstNode* ch = ast_new(p, AST_UFCS_EXPR);
        if (!ch) return 0;
        if (!parse_unwrap_ufcs_chain_tail(p, ch, at)) return 0;
        if (!ast_dbody_push(p, n, ch)) return 0;
        if (ch->e && ch->e[0] && strncmp(ast_slot(ch->e), ".wait(", 6) == 0)
            n->par_wake = 1;
        return 1;
    }
    if (p_accept(p, TK_PUNCT, ";")) {
        if (parallel_dest_is_handle(n) || shadow_pw_result_dest(n))
            return 1;
        parser_fail(p, at,
                    "@parallel {…} !>; discards the handle; bind CCParallel h = … !>; or !>.wait()!>");
        return 0;
    }
    /* `!> @destroy` — dest-live ownership: invalidate at scope exit. */
    if (tok_eq(p_peek(p), TK_PUNCT, "@") && p->i + 1 < p->n &&
        shadow_kw(p->toks[p->i + 1]) == SHADOW_KW_DESTROY) {
        AstNode* dv = shadow_par_dest_var(n);
        if (!dv) {
            parser_fail(p, p_peek(p), "@destroy needs a dest bind");
            return 0;
        }
        p_next(p); /* @ */
        p_next(p); /* destroy */
        if (p_accept(p, TK_PUNCT, ";")) {
            dv->e = ast_arena_cstr(p, "_Dbare");
            return 1;
        }
        if (!p_accept(p, TK_PUNCT, "{")) {
        ...
        while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF &&
            ...
            AstNode* s = parse_stmt(p);
            if (!s) return 0;
            if (!ast_body_push(p, dv, s)) return 0;
        }
        if (!p_accept(p, TK_PUNCT, "}") || !p_accept(p, TK_PUNCT, ";")) {
            parser_fail(p, p_peek(p), "expected '};' after @destroy { ... }");
            return 0;
        }
        dv->e = ast_arena_cstr(p, "_D");
        return 1;
    }
    if (!p_accept(p, TK_PUNCT, "(")) {
        parser_fail(p, at, "expected '!>;', !>.method(), or !> @destroy");
        return 0;
    }
    have_paren = 1;
    if (have_paren) {
        ...
        if (tok_eq(p_peek(p), TK_PUNCT, "{")) {
            ...
            while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF &&
                ...
                AstNode* s = parse_stmt(p);
                if (!s) return 0;
                if (!ast_body_push(p, eh, s)) return 0;
            }
            if (!p_accept(p, TK_PUNCT, "}")) {
                parser_fail(p, p_peek(p),
        ...
        if (!ast_dbody_push(p, n, eh)) return 0;
        return 1;
    }
...
static int parallel_require_result_bind(Parser* p, AstNode* n) {
    if (!n || n->kind != AST_PARALLEL_FOR || !(n->e && n->e[0])) return 1;
...
/* Dest bound, one assignment arm: kick is that arm, so the dest is never
 * live on the caller after the construct returns. `!>.wait()!>` joins
 * before the statement ends. One `@serial` is a sequential block arm
 * (contents, not “caller”). An expression arm is the worker — spawned,
 * dest lives. */
static int parallel_reject_dead_dest(Parser* p, AstNode* n) {
    if (!n || n->kind != AST_PARALLEL) return 1;
    if (n->par_wake) return 1;
    if (!parallel_dest_is_handle(n)) return 1;
    if (n->nbody != 1) return 1;
    if (n->body[0] && n->body[0]->kind == AST_SERIAL) return 1;
    parser_fail(p, p_peek(p), "this dest is never live on the caller");
    return 0;
}
...
static AstNode* parse_parallel_finish(Parser* p, AstNode* n) {
    ...
    if (!parallel_reject_dead_dest(p, n)) return NULL;
    return n;
}

/* `Type name = @parallel … !>` / `name = @parallel … !>` — dest is the
 * ordinary unwrap lhs. The construct is the Result producer. */
static AstNode* parse_parallel_unwrap_rhs(Parser* p, const char* dest,
                                         const char* ty) {
    AstNode* n = parse_parallel_head(p);
    if (!n) return NULL;
    if (dest && dest[0] && !parallel_attach_dest(p, n, dest, ty))
        return NULL;
    return parse_parallel_finish(p, n);
}

/* `{ … }` already tokenized — skip a balanced brace for lookahead. */
static int parallel_skip_braced(Parser* p) {
    int depth = 0;
...
/* Dest cell: `h` or one field hop (`zip.app`, `srv->app`). Not a call. */
static int parallel_take_dest_cell(Parser* p, char* dest, size_t cap) {
    Token t;
    size_t n = 0;
    if (!dest || cap < 2) return 0;
    dest[0] = 0;
    if (p_peek(p).kind != TK_IDENT) return 0;
    t = p_next(p);
    if (t.spell.len + 1 > cap) return 0;
    memcpy(dest, t.spell.ptr, t.spell.len);
    n = t.spell.len;
    dest[n] = 0;
    if (tok_eq(p_peek(p), TK_PUNCT, ".") || tok_eq(p_peek(p), TK_PUNCT, "->")) {
        Token op = p_next(p);
        Token f;
        if (p_peek(p).kind != TK_IDENT) return 0;
        f = p_next(p);
        if (n + op.spell.len + f.spell.len + 1 > cap) return 0;
        memcpy(dest + n, op.spell.ptr, op.spell.len);
        n += op.spell.len;
        memcpy(dest + n, f.spell.ptr, f.spell.len);
        n += f.spell.len;
        dest[n] = 0;
    }
    return 1;
...
/* `@parallel (dest) { … }` without `!>` is dest admit. dest is a name or
 * one field hop. The same tokens with `!>` stay the predicate join. */
static int peek_parallel_onto(Parser* p) {
    int save = p->i;
    int onto;
    char dest[256];
    if (!tok_eq(p_peek(p), TK_PUNCT, "(")) return 0;
    p_next(p);
    if (!parallel_take_dest_cell(p, dest, sizeof(dest))) {
        p->i = save;
        return 0;
    }
    if (!tok_eq(p_peek(p), TK_PUNCT, ")")) {
    ...
    if (!parallel_skip_braced(p)) {
        p->i = save;
        return 0;
    }
    onto = !tok_eq(p_peek(p), TK_PUNCT, "!>");
    p->i = save;
    return onto;
}

static AstNode* parse_parallel_onto(Parser* p) {
    char dest[256];
    AstNode* n;
    AstNode* ser;
    if (!p_accept(p, TK_PUNCT, "(")) {
    ...
    if (!parallel_take_dest_cell(p, dest, sizeof(dest))) {
        parser_fail(p, p_peek(p),
                    "@parallel(h) dest must be a name or field (h, zip.app, srv->app)");
        return NULL;
    }
    if (!p_accept(p, TK_PUNCT, ")")) {
        parser_fail(p, p_peek(p), "expected ')' after @parallel dest");
        return NULL;
    }
    if (!tok_eq(p_peek(p), TK_PUNCT, "{")) {
        parser_fail(p, p_peek(p), "expected '{' after @parallel(h)");
        return NULL;
    }
    p_next(p);
    n = ast_new(p, AST_PARALLEL);
    if (!n) return NULL;
    n->par_onto = 1;
    n->forced_seq = p->parallel_off;
    n->a = ast_arena_cstr(p, dest);
    ser = ast_new(p, AST_SERIAL);
    if (!ser) return NULL;
    while (!tok_eq(p_peek(p), TK_PUNCT, "}") && p_peek(p).kind != TK_EOF &&
           !p->err) {
        AstNode* s = parse_stmt(p);
    ...
    p_accept(p, TK_PUNCT, ";");
    if (ser->nbody < 1) {
        parser_fail(p, p_peek(p), "@parallel(h) body is empty");
        return NULL;
    }
    if (!ast_body_push(p, n, ser)) return NULL;
    return n;
}
...
static AstNode* parse_parallel_head(Parser* p) {
    ...
    p_next(p); /* @ */
    p_next(p); /* parallel */
    if (tok_eq(p_peek(p), TK_PUNCT, "(") && peek_parallel_onto(p))
        return parse_parallel_onto(p);
    if (tok_eq(p_peek(p), TK_IDENT, "spawn") ||
        shadow_kw(p_peek(p)) == SHADOW_KW_SPAWN) {
        Token sp = p_peek(p);
        p_next(p); /* spawn */
        want_spawn = 1;
        if (peek_parallel_at_for(p) || peek_parallel_for(p) ||
            tok_eq(p_peek(p), TK_IDENT, "wait")) {
            parser_fail(p, sp,
                        "@parallel spawn is a brace join; do not combine "
                        "with for or wait");
            return NULL;
        }
    }
...
static AstNode* parse_parallel_result_init(Parser* p) {
    ...
    if (shadow_kw(p_peek(p)) == SHADOW_KW_BOOL)
        ...
        snprintf(ty, sizeof(ty), "_Bool");
             ...
             spell_eq(p_peek(p).spell, "CCParallel"))
        snprintf(ty, sizeof(ty), "CCParallel");
    else
        return NULL;
    p_next(p);
    ...
    return parse_parallel_unwrap_rhs(p, dest, ty);
}
...
static AstNode* parse_parallel_result_assign(Parser* p) {
    ...
    p_next(p); /* = */
    slice_to(dest, sizeof(dest), name.spell);
    n = parse_parallel_unwrap_rhs(p, dest, NULL);
    if (!n) {
        p->i = save;
        return NULL;
    }
...
static AstNode* parse_stage(Parser* p) {
    ...
    if (!n) return NULL;
    if (!({ char __spell_tmp[4096]; int __spell_ok = ast_spell_token_range(p, p0, split >= 0 ? split : rp, __spell_tmp, sizeof(__spell_tmp)); if (__spell_ok) n->a = ast_arena_cstr(p, __spell_tmp); __spell_ok; }) ||
        !(n->a && n->a[0])) {
        parser_fail(p, at, "expected gate expression in @stage (...)");
        return NULL;
    }
    if (split >= 0 &&
        !({ char __spell_tmp[4096]; int __spell_ok = ast_spell_token_range(p, split + 1, rp, __spell_tmp, sizeof(__spell_tmp)); if (__spell_ok) n->b = ast_arena_cstr(p, __spell_tmp); __spell_ok; })) {
...
static AstNode* parse_async_fn(Parser* p) {
    ...
    if (!n) return NULL;
    n->a = ast_arena_cstr(p, rbuf);
    n->b = ast_arena_slice(p, name.spell);
    n->c = ast_arena_cstr(p, params);
    if (is_result) n->e = ast_arena_cstr(p, "result");
    ...
    if ((((n->d = ast_arena_span(p, body0, p->i))), p->err)) {
...
static AstNode* parse_static_var(Parser* p) {
    ...
    int ni;
    gent[0] = 0;
    /* `static Vec::[T] g` — same walker as fields / typedef, not tape. */
    if (ty_end > ti) {
        if (!ast_spell_type_tokens(p, ti, ty_end, gent, sizeof(gent)))
            return NULL;
        shadow_seed_spelled_type_name(p, gent);
        ni = ty_end;
    } else {
        ni = ti + 1;
        ...
        if (ty.kind == TK_IDENT && spell_eq(ty.spell, "long") && ni < p->n &&
            ...
            return NULL;
        /* Multi-token C types (`unsigned char`, `signed int`, …) — tape. */
        if (ty.kind == TK_IDENT &&
            (spell_eq(ty.spell, "unsigned") || spell_eq(ty.spell, "signed") ||
             spell_eq(ty.spell, "short")) &&
            ni < p->n &&
            (p->toks[ni].kind == TK_IDENT ||
             shadow_kw(p->toks[ni]) == SHADOW_KW_INT ||
    ...
    if (!n) return NULL;
    {
        char tbuf[160];
        if (gent[0])
            snprintf(tbuf, sizeof(tbuf), "%s", gent);
        else
            slice_to(tbuf, sizeof(tbuf), ty.spell);
        if (has_atomic && has_volatile && has_const)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "_Atomic volatile const %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else if (has_atomic && has_volatile)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "_Atomic volatile %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else if (has_atomic && has_const)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "_Atomic const %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else if (has_atomic)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "_Atomic %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else if (has_volatile && has_const)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "volatile const %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else if (has_volatile)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "volatile %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else if (has_const)
            do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "const %s", tbuf); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
        else
            n->a = ast_arena_cstr(p, tbuf);
    }
    n->b = ast_arena_slice(p, name.spell);
    spawn_note_global(n->b);
    if (is_ptr) {
        char atmp[512];
        snprintf(atmp, sizeof(atmp), "%s*", n->a ? n->a : "");
    ...
    if (tok_eq(p_peek(p), TK_PUNCT, "=")) {
         ...
         * Probe size first: ast_spell/span_text diagnose into p->err. */
        {
            ...
            (void)need; /* unbounded: arena path below */
        }
        if (!({ char __spell_tmp[4096]; int __spell_ok = ast_spell_token_range(p, e0, p->i, __spell_tmp, sizeof(__spell_tmp)); if (__spell_ok) n->d = ast_arena_cstr(p, __spell_tmp); __spell_ok; }) &&
            (((n->d = ast_arena_span(p, e0, p->i))), p->err)) {
            p->i = start;
            p->err = 0;
            p->err_msg[0] = 0;
            return NULL;
...
static AstNode* parse_global_var(Parser* p) {
    ...
    {
        int decl0 = (ty_end > ti) ? ty_end : ti + 1;
    ...
    spawn_note_global(n->b);
    n->e = ast_arena_cstr(p, "global");
    if (is_ptr) n->c = ast_arena_cstr(p, "*");
    if (tok_eq(p_peek(p), TK_PUNCT, "[")) {
        int a0 = p->i;
        int depth = 1;
        p_next(p); /* [ */
    ...
    while (tok_eq(p_peek(p), TK_PUNCT, ",")) {
        ...
        {
            char btmp[4096];
            bl = n->b ? strlen(ast_slot(n->b)) : 0;
            if (bl + 1 + nxt.spell.len + 1 >= sizeof(btmp)) {
            ...
            btmp[bl++] = ',';
            btmp[bl++] = ' ';
            memcpy(btmp + bl, nxt.spell.ptr, nxt.spell.len);
            btmp[bl + nxt.spell.len] = 0;
...
static AstNode* parse_at_stmt(Parser* p) {
    ...
    if (p->i + 1 < p->n && shadow_kw(p->toks[p->i + 1]) == SHADOW_KW_FOR)
        return parse_cc_for(p);
    if (p->i + 1 < p->n && shadow_kw(p->toks[p->i + 1]) == SHADOW_KW_SWITCH)
        return parse_cc_switch(p);
    if (p->i + 1 < p->n && shadow_kw(p->toks[p->i + 1]) == SHADOW_KW_STAGE)
    ...
    if (spell_eq(name.spell, "typeview")) {
        ...
        }
        {
            ...
            if (!n) return NULL;
            n->a = ast_arena_cstr(p, "typeview");
            if (unnamed) n->b = g_ast_empty;
            else n->b = ast_arena_slice(p, mode.spell);
            n->c = ast_arena_cstr(p, body);
            n->d = ast_arena_cstr(p, subject);
            if (unnamed) {
                /* Keyed on Base itself — no parallel typedef. */
                n->e = ast_arena_cstr(p, subject);
            } else if (strchr(subject, '*')) {
                /* Family glob: seed Concrete_Restrict_Mode per match. */
                char mode_s[64];
                slice_to(mode_s, sizeof(mode_s), mode.spell);
                shadow_tv_named_glob_register(p, subject, mode_s);
                n->e = ast_arena_cstr(p, subject);
            } else {
                snprintf(mangled, sizeof(mangled), "%s_Restrict_%.*s", subject,
                         (int)mode.spell.len, mode.spell.ptr);
                n->e = ast_arena_cstr(p, mangled);
                ms.ptr = mangled;
    ...
    if (spell_eq(name.spell, "typehooks")) {
        ...
        n = ast_new(p, AST_AT_STMT);
        if (!n) return NULL;
        n->a = ast_arena_cstr(p, "typehooks");
        n->c = ast_arena_cstr(p, body);
        n->d = ast_arena_cstr(p, subject);
        return n;
    }
    ...
    if (spell_eq(name.spell, "await")) {
        ...
        n = ast_new(p, AST_AT_STMT);
        if (!n) return NULL;
        n->a = ast_arena_cstr(p, "await");
        n->c = ast_arena_cstr(p, expr);
        return n;
    }
    ...
    if (spell_eq(name.spell, "comptime") &&
        ...
        if (!n) return NULL;
        n->a = ast_arena_cstr(p, "comptime");
        n->b = ast_arena_cstr(p, "if");
        n->c = ast_arena_cstr(p, pred);
        return n;
    }
    {
            ...
            typ = p_next(p);
        {
            ...
            if (!n) return NULL;
            n->a = ast_arena_slice(p, name.spell);
            if (typ.kind == TK_IDENT) n->b = ast_arena_slice(p, typ.spell);
            if (variant_packed) n->d = ast_arena_cstr(p, "packed");
            ...
            if (spell_eq(name.spell, "variant") && body0 >= 0 && body1 > body0) {
                ...
                if (body) n->c = ast_arena_cstr(p, body);
            }
...
static AstNode* parse_field_simple(Parser* p) {
    ...
    n->b = ast_arena_cstr(p, names);
    if (has_as) n->e = ast_arena_cstr(p, "as");
    return n;
}
...
static AstNode* parse_typedef_enum(Parser* p) {
    ...
    if (p_peek(p).kind == TK_IDENT) {
        Token tag = p_next(p);
        n->a = ast_arena_slice(p, tag.spell);
    }
    if (!p_accept(p, TK_PUNCT, "{")) {
    ...
    if ((((n->d = ast_arena_span(p, b0, p->i))), p->err)) {
    ...
    n->b = ast_arena_slice(p, alias.spell);
    if (!p_accept(p, TK_PUNCT, ";")) {
        parser_fail(p, p_peek(p), "expected ';' after typedef enum");
        return NULL;
    }
...
/* Variant designator (`case .arm`) or string-literal cases — require @switch. */
static int switch_body_is_checked(Parser* p, size_t body_off) {
    FileTape* ft = (p->cache && p_peek(p).file_id)
                       ? tape_by_id(p->cache, p_peek(p).file_id)
                       : NULL;
    const char* bytes;
    size_t blen, off;
    int d, in_bt, in_dq, in_sq;
    if (!ft || !ft->bytes) return 0;
    bytes = ft->bytes;
    blen = ft->len;
    off = body_off;
    if (off >= blen || bytes[off] != '{') return 0;
    d = 1;
    in_bt = in_dq = in_sq = 0;
    off++;
    while (off < blen && d > 0) {
        ...
        if (c == 'c' && off + 4 < blen && memcmp(bytes + off, "case", 4) == 0 &&
            (off == 0 ||
            ...
            while (j < blen &&
                   (bytes[j] == ' ' || bytes[j] == '\t' ||
                    bytes[j] == '\n' || bytes[j] == '\r'))
                j++;
            if (j < blen && (bytes[j] == '.' || bytes[j] == '"')) return 1;
        }
        if (c == '{') d++;
        else if (c == '}') {
            d--;
 ...
 * text fallback for `@string` / tick-heavy bodies. cc_switch: `@switch`. */
static AstNode* parse_switch_loop(Parser* p, int cc_switch) {
    if (cc_switch) {
        if (!tok_eq(p_peek(p), TK_PUNCT, "@")) return NULL;
        p_next(p); /* @ */
    }
    ...
    {
        Token body_brace = p_peek(p);
        if (!tok_eq(body_brace, TK_PUNCT, "{")) {
            parser_fail(p, p_peek(p), "expected '{ after switch");
            return NULL;
        }
        if (!cc_switch &&
            switch_body_is_checked(p, (size_t)body_brace.offset)) {
            parser_fail(p, body_brace,
                        "checked switch requires `@switch`; use "
                        "`@switch (v) { case .arm: ... }` or "
                        "`@switch (name) { case \"GET\": ... }`");
            return NULL;
        }
    }
    if (!p_accept(p, TK_PUNCT, "{")) {
     ...
     * so preprocessor lines and `case X: stmt; break;` survive intact. */
    {
        ...
        /* body is a per-node growable table — not kids_storage (that aliases
         * the enclosing fn list and once emitted a bare `case 0:`). Soft
         * parse still falls back to opaque tape on structure miss; OOM from
         * ast_body_push fails loud (no silent opaque fallback). */
        p->soft_stmt = 1;
        p->err = 0;
        while (!tok_eq(p_peek(p), TK_PUNCT, "}") &&
            ...
            if (t.kind == TK_IDENT &&
                ...
                p_next(p); /* : */
                {
                    ...
                    if (!lab ||
                        (!({ char __spell_tmp[4096]; int __spell_ok = ast_spell_token_range(p, l0, p->i, __spell_tmp, sizeof(__spell_tmp)); if (__spell_ok) lab->a = ast_arena_cstr(p, __spell_tmp); __spell_ok; }) &&
                         (((lab->a = ast_arena_span(p, l0, p->i))), p->err))) {
                        ok = 0;
                        break;
                    }
                    /* `case .arm` stays a RAW_LINE label; following stmts
                     * (including `!>` / `@defer`) parse as AST. Emit rewrites
                     * the label and kinds the subject. Do not fall back to
                     * opaque tape — that dropped `!> { }` and `@cancel_defer`. */
                    if (!ast_body_push(p, n, lab)) {
                        ok = 0;
                        break;
                    }
                }
        ...
        /* Hard fail (OOM etc.) during soft structured parse — keep sticky err. */
        if (p->err && !saved_err) return NULL;
...
switch_opaque_body:
     ...
     * the switch (token scan sees `{`/`}` as punct even inside ticks). */
    {
        ...
        }
        {
            ...
            {
                /* Prefer arena copy; fall back to tape bind for huge bodies. */
                char* dcopy = (char*)cc_arena_alloc(p->parse_ar, nlen + 1, 1);
                if (dcopy) {
                    memcpy(dcopy, bytes + start, nlen);
                    dcopy[nlen] = 0;
                    n->d = dcopy;
                    n->span_off = 0;
                    n->span_len = 0;
...
static AstNode* parse_switch_stmt(Parser* p) { return parse_switch_loop(p, 0); }

static AstNode* parse_cc_switch(Parser* p) { return parse_switch_loop(p, 1); }
...
static AstNode* parse_typedef_struct(Parser* p) {
    ...
    if (p_peek(p).kind == TK_IDENT) {
        Token tag = p_next(p);
        st->a = ast_arena_slice(p, tag.spell);
    }
    if (p_accept(p, TK_PUNCT, "{")) {
    ...
    st->b = ast_arena_slice(p, alias.spell);
    if (!p_accept(p, TK_PUNCT, ";")) {
        parser_fail(p, p_peek(p), "expected ';' after typedef struct");
        return NULL;
    }
...
static AstNode* parse_typedef_chan(Parser* p) {
    ...
    if (!n) return NULL;
    n->a = ast_arena_slice(p, alias.spell);
    n->b = ast_arena_cstr(p, cap_txt);
    n->c = ast_arena_slice(p, dir.spell);
    ...
    if (ordered && topo_txt[0])
        do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "o:%s", topo_txt); n->e = ast_arena_cstr(p, __ast_tmp); } while (0);
    else if (ordered)
        n->e = ast_arena_cstr(p, "o");
    else if (topo_txt[0])
        do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "t:%s", topo_txt); n->e = ast_arena_cstr(p, __ast_tmp); } while (0);
    return n;
}
...
static AstNode* parse_fn_proto(Parser* p) {
    ...
    gret[0] = 0;
    if (!has_struct &&
        ast_try_generic_ret(p, ri, &ty_end, &ni, gret, sizeof(gret))) {
        /* Name::[args] return — spelled into gret. */
    } else if (!has_struct) {
        int iend = peek_c_int_type_end(p, ri);
        if (iend > ri) {
            ty_end = iend;
            ni = iend;
        }
    }
    if (!gret[0] && !has_struct && ni + 2 < p->n &&
    ...
    if (!n) return NULL;
    {
        ...
        if (has_const && has_struct)
            ...
            if (has_const)
                snprintf(ty, sizeof(ty), "const %s", gret);
            else
                snprintf(ty, sizeof(ty), "%s", gret);
        } else if (ty_end > ri + 1) {
            if (!ast_spell_token_range(p, ri, ty_end, ty, sizeof(ty)) &&
                !span_text(p, ri, ty_end, ty, sizeof(ty)))
                return NULL;
            if (has_const) {
                char prefixed[256];
                snprintf(prefixed, sizeof(prefixed), "const %s", ty);
                snprintf(ty, sizeof(ty), "%s", prefixed);
            }
        } else if (has_const)
...
static AstNode* parse_typedef_result(Parser* p) {
    ...
    n->d = ast_arena_slice(p, err.spell);
    return n;
}
...
static AstNode* parse_typedef_slice(Parser* p) {
    ...
    if (!n) return NULL;
    n->a = ast_arena_slice(p, alias.spell);
    do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "%sCCSlice", has_const ? "const " : ""); n->b = ast_arena_cstr(p, __ast_tmp); } while (0);
    return n;
}
...
static AstNode* parse_typedef_restricted(Parser* p) {
    ...
    /* A single typedef alias cannot name a type-family glob. Use
     * `@typeview Mode on Pat* { … }` and `@typeview(Mode) Concrete*`. */
    if (tok_eq(p_peek(p), TK_PUNCT, "*")) {
        parser_fail(p, p_peek(p),
                    "typedef @typeview alias cannot use a glob subject");
        return NULL;
    }
    if (!tok_eq(p_peek(p), TK_PUNCT, "{")) {
    ...
    if (!n) return NULL;
    n->a = ast_arena_cstr(p, "typeview");
    n->b = ast_arena_slice(p, mode.spell);
    n->c = ast_arena_cstr(p, body);
    n->d = ast_arena_slice(p, base.spell);
    /* e = mangled#alias or mangled#*alias (typedef+define form). */
    do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "%s#%s%.*s", mangled, is_ptr ? "*" : "",
             (int)alias.spell.len, alias.spell.ptr); n->e = ast_arena_cstr(p, __ast_tmp); } while (0);
    return n;
}
...
static AstNode* parse_typedef_int(Parser* p) {
    ...
    if (p->i + 2 < p->n && p->toks[p->i + 1].kind == TK_IDENT) {
        ...
        if (j < p->n && p->toks[j].kind == TK_IDENT && j + 1 < p->n &&
            ...
            if (!n) return NULL;
            n->a = ast_arena_slice(p, alias.spell);
            n->b = ast_arena_slice(p, base.spell);
            if (nstars > 0) {
                char btmp[512];
                size_t L;
                snprintf(btmp, sizeof(btmp), "%s", n->b ? n->b : "");
                L = strlen(btmp);
                while (nstars-- > 0 && L + 1 < sizeof(btmp)) {
                    btmp[L++] = '*';
                    btmp[L] = 0;
                }
                n->b = ast_arena_cstr(p, btmp);
...
static AstNode* parse_star_stmt(Parser* p) {
    ...
    if (!n) return NULL;
    n->a = ast_arena_slice(p, a.spell);
    n->b = ast_arena_slice(p, b.spell);
    return n;
}
...
static AstNode* parse_struct(Parser* p) {
    ...
    }
    {
        ...
        if (tok_eq(p_peek(p), TK_PUNCT, ";")) {
            ...
            if (!st) return NULL;
            st->a = ast_arena_slice(p, name.spell);
            st->e = ast_arena_cstr(p, "fwd");
            {
                char ctmp[2048];
                shadow_same_line_c_comment(p, semi, ctmp, sizeof(ctmp));
                st->d = ast_arena_cstr(p, ctmp);
            }
            return st;
        }
        if (!p_accept(p, TK_PUNCT, "{")) {
...
static int parse_struct_fields(Parser* p, AstNode* st) {
    ...
    /* Always cparse: #if / comma / nested C, then overlay-attach CC fields
     * (!> / [:] / [~ / generics). No beachhead fallback. */
    if (close < p->i) {
        parser_fail(p, p_peek(p), "unclosed struct");
        return 0;
    }
...
static AstNode* parse_global_arr(Parser* p) {
    ...
    if (!n) return NULL;
    if (has_const)
        do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "const %.*s%s", (int)ty.spell.len, ty.spell.ptr,
                 is_ptr ? "*" : ""); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
    else
        do { char __ast_tmp[4096]; snprintf(__ast_tmp, sizeof(__ast_tmp), "%.*s%s", (int)ty.spell.len, ty.spell.ptr,
                 is_ptr ? "*" : ""); n->a = ast_arena_cstr(p, __ast_tmp); } while (0);
    n->b = ast_arena_slice(p, name.spell);
...
static AstNode* parse_external_inner(Parser* p) {
    ...
    if (t.kind == TK_IDENT &&
        (spell_eq(t.spell, "CC_DECL_RESULT_SPEC") ||
         ...
         spell_eq(t.spell, "CC_TABLE_DECL") ||
         spell_eq(t.spell, "CC_TABLE_DECL_UFCS")) &&
        p->i + 1 < p->n && tok_eq(p->toks[p->i + 1], TK_PUNCT, "(")) {
        int a0 = p->i;
        ...
        n = ast_new(p, AST_RAW_LINE);
        if (!n) return NULL;
        if ((((n->a = ast_arena_span(p, a0, p->i))), p->err)) {
    ...
    if (t.kind == TK_IDENT &&
        ...
        if (!n) return NULL;
        n->a = ast_arena_slice(p, fam.spell);
        shadow_parse_gfac_note(n->a);
        n->b = ast_arena_cstr(p, arity);
        n->c = ast_arena_cstr(p, is_extend ? "extend" : "base");
        {
            CCSlice tpl = bt.spell;
            if (tpl.len >= 2) {
                tpl.ptr += 1;
                tpl.len -= 2;
            } else {
...
static AstNode* parse_external(Parser* p) {
    ...
    if (n) {
        ...
        if (p->ntu > 0) {
            ...
            if (prev->kind == AST_STRUCT && strcmp(ast_slot(prev->e), "fwd") == 0 && prev->d && prev->d[0])
...
static int parse_tu(Parser* p, AstNode*** out_items, int* out_n) {
    ...
    while (p_peek(p).kind != TK_EOF && !p->err) {
        ...
        if (p->ntu >= p->tu_cap) {
            int need = p->tu_cap > 0 ? p->tu_cap * 2 : 64;
            if (!parser_ptrtab_grow(p, &p->tu_items, &p->tu_cap, need)) {
                parser_fail(p, p_peek(p), "out of memory (TU item table)");
                break;
            }
        }
        p->tu_items[p->ntu++] = e;
...
static const char* ast_kind_name(AstKind k) {
    ...
    case AST_CANCEL_DEFER: return "cancel_defer";
