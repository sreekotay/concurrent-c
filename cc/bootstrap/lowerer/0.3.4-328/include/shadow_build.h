...
static uint64_t shadow_toolchain_content_fp(const char* self_path) {
    ...
    /* The libraries live beside this binary's tree (out/cc/bin/shadow_lower
     * → repository root three levels up), not beside the caller's cwd:
     * from an application directory a relative spelling folds as absent. */
    {
        char root[1024];
        char lib[1200];
        const char* p = self_path ? self_path : "";
        size_t n = strlen(p);
        int ups = 0;
        while (n > 0 && ups < 3) {
            n--;
        ...
        if (ups == 3 && n < sizeof(root)) {
            memcpy(root, p, n);
            root[n] = 0;
        } else {
            snprintf(root, sizeof(root), ".");
        }
        snprintf(lib, sizeof(lib), "%s/out/cc/obj/libshadow_comptime.a", root);
        h = shadow_fold_file_content(h, lib);
        snprintf(lib, sizeof(lib), "%s/third_party/tcc/libtcc.a", root);
        h = shadow_fold_file_content(h, lib);
    }
    {
        const char* ver = getenv("CCC_VERSION");
        h = shadow_fnv1a64_str(h, "\x03" "ccc_version:");
        h = shadow_fnv1a64_str(h, ver && ver[0] ? ver : "<unknown>");
    }
...
/* Resolve a bare host compiler name through PATH so its bytes can be
 * folded; an absolute name is returned as is. Empty when not found. */
static void shadow_host_cc_resolve(const char* host_cc, char* out, size_t cap) {
    const char* path = getenv("PATH");
    out[0] = 0;
    if (!host_cc || !host_cc[0]) return;
    if (strchr(host_cc, '/')) {
        snprintf(out, cap, "%s", host_cc);
        return;
    }
    while (path && *path) {
        ...
        char cand[1024];
        if (n > 0 && n < sizeof(cand) - 2) {
            snprintf(cand, sizeof(cand), "%.*s/%s", (int)n, path, host_cc);
...
static uint64_t shadow_obj_cache_key(uint64_t emit_key, const char* host_cc) {
    ...
    /* The host compiler's bytes, whether named absolutely or found on PATH:
     * a compiler upgrade under the same name must miss. */
    shadow_host_cc_resolve(host_cc ? host_cc : "cc", resolved, sizeof(resolved));
    if (resolved[0])
        h = shadow_fold_file_content(h, resolved);
    else
        h = shadow_fnv1a64_str(h, "\x01" "<hostcc-unresolved>");
...
static int shadow_diag_quoted_ident_col(const char* err, const char* orig) {
    ...
    while (*p) {
        ...
        n = (size_t)(e - (s + 1));
        p = e + 1;
        if (n == 0 || n >= sizeof(ident)) continue;
        ...
        if (shadow_diag_ident_boundary(orig, n, hit))
            return (int)(hit - orig) + 1;
    }
    if (saw_quote) {
        const char* slot = strstr(orig, "${");
        if (slot) return (int)(slot - orig) + 1;
    }
    return 0;
}

/* Host-cc "unknown type name 'CCVec_T'; did you mean 'CCSlice_T'?" is a
 * lie — Vec::[T] and T[:] are different types. Say which family it is. */
static int shadow_host_diag_surface_generic(const char* ident, char* fam,
                                            size_t fcap, char* args,
                                            size_t acap) {
    if (!ident || !ident[0] || !fam || !fcap || !args || !acap) return 0;
    if (strncmp(ident, "CCVec_", 6) == 0 && ident[6]) {
        snprintf(fam, fcap, "Vec");
        snprintf(args, acap, "%s", ident + 6);
        return 1;
    }
    if (strncmp(ident, "ArrayMap_", 9) == 0 && ident[9]) {
        snprintf(fam, fcap, "ArrayMap");
        snprintf(args, acap, "%s", ident + 9);
        return 1;
    }
    if (strncmp(ident, "Table_", 6) == 0 && ident[6]) {
        snprintf(fam, fcap, "Table");
        snprintf(args, acap, "%s", ident + 6);
        return 1;
    }
    if (strncmp(ident, "Map_", 4) == 0 && ident[4]) {
        snprintf(fam, fcap, "Map");
        snprintf(args, acap, "%s", ident + 4);
        return 1;
    }
    return 0;
}

static int shadow_host_diag_rewrite_unknown_generic(const char* line,
                                                    char* out, size_t cap) {
    const char* err;
    const char* q1;
    const char* q2;
    ...
    size_t ilen;
    size_t locn;
    if (!line || !out || !cap) return 0;
    err = strstr(line, "error: unknown type name '");
    if (!err) return 0;
    q1 = err + strlen("error: unknown type name '");
    q2 = strchr(q1, '\'');
    if (!q2) return 0;
    ilen = (size_t)(q2 - q1);
    if (!ilen || ilen >= sizeof(ident)) return 0;
    memcpy(ident, q1, ilen);
    ident[ilen] = 0;
    if (!shadow_host_diag_surface_generic(ident, fam, sizeof(fam), args,
                                          sizeof(args)))
        return 0;
    locn = (size_t)(err - line);
    if (locn >= sizeof(loc)) locn = sizeof(loc) - 1;
    memcpy(loc, line, locn);
    loc[locn] = 0;
    mean = strstr(q2, "did you mean 'CCSlice");
    if (mean)
        snprintf(out, cap,
                 "%serror: unknown type name '%s' — that is %s::[%s], "
                 "not a slice",
                 loc, ident, fam, args);
    else
        snprintf(out, cap,
...
/* Wrap path from the driver → stamped `#line` file. */
static int shadow_wrap_user_file(const char* wrap, char* dst, size_t cap) {
    char head[2048];
    FILE* f;
    size_t n;
    if (!wrap || !dst || !cap) return 0;
    if (!tape_path_is_cache_wrap(wrap)) return 0;
    f = fopen(wrap, "r");
    if (!f) return 0;
    ...
    if (!n) return 0;
    head[n] = 0;
    return tape_lead_line_file(head, n, dst, cap);
}
    ...
    while (p < end) {
        ...
        if (expect_ln && shadow_diag_is_snippet(line, expect_ln, NULL) &&
            ...
            if (shadow_diag_parse_loc(line, path, sizeof(path), &ln, &col)) {
                ...
                if (shadow_wrap_user_file(path, user, sizeof(user))) {
                    const char* rest = strstr(line, ": error:");
                    if (!rest) rest = strstr(line, ": warning:");
                    if (rest) {
                        if (rest[0] == ':' && rest[1] == ' ') rest += 2;
                        snprintf(mapped_line, sizeof(mapped_line), "%s:%d:%d: %s",
                                 user, ln, col, rest);
                        print = mapped_line;
                        snprintf(path, sizeof(path), "%s", user);
                    }
                }
                if (!mark) mark = strstr(path, "out/include/");
                if (!msg) msg = strstr(line, "warning:");
                if (mark && msg) {
                    const char* rel = strstr(mark, "include/");
                    size_t rlen;
                    char src[1024];
                    if (rel) rel += sizeof("include/") - 1;
            ...
            if (shadow_host_diag_rewrite_unknown_generic(print, rewritten,
                                                        sizeof(rewritten)))
                print = rewritten;
            fprintf(stderr, "%s\n", print);
            if (remapped
...
/* Path-shaped @link operand: slash, or archive / DSO suffix.
 * Same rules as host cc__is_lib_path — pass through, do not prefix -l. */
static int shadow_is_lib_path(const char* lib) {
    size_t len;
    if (!lib || !lib[0]) return 0;
...
/* Append a @link operand: `-lname`, or the path unchanged (legacy
 * cc__add_lib_to_flags). pthread / m are already on the host link line. */
...
/* A host-cc reject must not stay a warm emit hit. Fail tests extract
 * on the cold path; replaying emit.c skips that diagnostic. */
static void shadow_emit_cache_invalidate(const char* in_path,
                                         const char* self_path) {
    uint64_t ekey;
    char dir[512], ekey_path[600], c_path[600], okey_path[600];
    if (!in_path) return;
    ekey = shadow_emit_cache_key(in_path, self_path);
    shadow_cache_dir(ekey, dir, sizeof(dir));
    ...
    {
        char link_path[600];
        snprintf(link_path, sizeof(link_path), "%s/link.key", dir);
        unlink(link_path);
    }
}

/* Link identity: the object key, the runtime object's bytes and the link
 * flags. Stored as bin.key beside tu.o, with bin.sum = the linked binary's
 * bytes, so a binary another build wrote to the same path, or a same-second
 * relink, is never mistaken for this one. mtimes play no part. */
static uint64_t shadow_bin_cache_key(uint64_t okey, const char* o_path,
                                     const char* runtime_o) {
    uint64_t h = okey;
    h = shadow_fnv1a64_str(h, "\x05" "link:");
    /* The object's bytes, not its key: a header edit recompiles tu.o under
     * the same object key (the emitted C only names the header). */
    h = shadow_fold_file_content(h, o_path);
    if (runtime_o && runtime_o[0])
        h = shadow_fold_file_content(h, runtime_o);
    else
        h = shadow_fnv1a64_str(h, "<no-runtime>");
    h = shadow_fnv1a64_str(h, g_shadow_host_opts.ld_flags);
    h = shadow_fnv1a64_i64(h, g_shadow_host_opts.no_runtime ? 1 : 0);
    return h;
}

static int shadow_bin_fresh(const char* dir, const char* bin_path, uint64_t bkey) {
    ShadowFileSig bin;
    char key_path[600], sum_path[600];
    uint64_t prev = 0, sum = 0;
    if (shadow_stat_sig(bin_path, &bin) != 0 || bin.size <= 0) return 0;
    snprintf(key_path, sizeof(key_path), "%s/bin.key", dir);
    snprintf(sum_path, sizeof(sum_path), "%s/bin.sum", dir);
    if (shadow_read_u64_file(key_path, &prev) != 0 || prev != bkey) return 0;
    if (shadow_read_u64_file(sum_path, &sum) != 0) return 0;
    return shadow_fold_file_content(1469598103934665603ULL, bin_path) == sum;
}

static void shadow_bin_cache_note(const char* dir, const char* bin_path, uint64_t bkey) {
    char key_path[600], sum_path[600];
    snprintf(key_path, sizeof(key_path), "%s/bin.key", dir);
    snprintf(sum_path, sizeof(sum_path), "%s/bin.sum", dir);
    (void)shadow_write_u64_file(key_path, bkey);
    (void)shadow_write_u64_file(sum_path,
                                shadow_fold_file_content(1469598103934665603ULL, bin_path));
}
...
static int shadow_build_host(const char* in_path, const char* bin_path,
    ...
    /* The object and link keys fold the emitted C itself: the emit key
     * names the source, but a header edit re-emits under the same key. */
    okey = shadow_obj_cache_key(ekey, shadow_host_cc());
    okey = shadow_fnv1a64_str(okey, "\x04" "emit.c:");
    okey = shadow_fold_file_content(okey, c_path);
    if (!(cache_ok && reuse_emit && shadow_file_exists(o_path) &&
        ...
        {
            long long t_cc = shadow_now_ms();
            if (shadow_host_compile(c_path, o_path, in_path) != 0) {
                unlink(ekey_path);
                unlink(okey_path);
                return -1;
            }
            shadow_prof_span("host_cc", t_cc);
        }
        (void)shadow_write_u64_file(okey_path, okey);
    }

    /* Emit/obj reuse still used to relink every process. Skip only when
     * the binary on disk is the one this object and runtime produced. */
    {
        uint64_t bkey = shadow_bin_cache_key(okey, o_path, runtime_o);
        long long t_ld;
        if (cache_ok && shadow_bin_fresh(dir, bin_path, bkey))
            return 0;
        t_ld = shadow_now_ms();
        if (shadow_host_link(o_path, bin_path, runtime_o, c_path, in_path) != 0) {
            unlink(ekey_path);
            unlink(okey_path);
            return -1;
        }
        shadow_prof_span("host_link", t_ld);
        if (cache_ok) shadow_bin_cache_note(dir, bin_path, bkey);
    }
    return 0;
}
