...
typedef struct {
    ...
    char result_fn_errs[SHADOW_SAFE_RFN_CAP][64];
    int nrfn;
...
static void shadow_safe_err_at_col(ShadowSafeCtx* ctx, AstNode* st, int force_col,
    ...
    tape_logical_at(ft->bytes, ft->len, st->tok_off, ft->path, NULL, 0, &line);
...
static ShadowSafeVar* shadow_safe_add(ShadowSafeCtx* ctx, const char* name) {
    ShadowSafeVar* v;
    if (!ctx || !name || !name[0]) return NULL;
    if (ctx->nvars >= SHADOW_SAFE_VAR_CAP) {
        shadow_safe_err_at(ctx, NULL,
                           "safety var table capacity exceeded (96)");
        return NULL;
    }
...
try_reversed:
    /* Reversed: `Tag == root.kind` / `.arm == root->kind` */
    {
        ...
        memcpy(root, kp, rlen);
        root[rlen] = 0;
        /* Check `->kind` or `.kind` follows root in the original */
        rarrow = (strstr(cond + (kind - cond), "->kind") == kind) ? 1 : 0;
                                ...
                                }
                                {
                                    ...
                                    if (!ctx->err) {
                                        ...
                                        offset_to_linecol(ft->bytes, ft->len, off,
                                                          &line, &col);
                                        tape_logical_at(ft->bytes, ft->len, off,
                                                        ft->path, NULL, 0, &line);
                                        fprintf(stderr, "%s:%d:%d: error: %s\n",
                                                ft->path, line, col, msg);
                                        fprintf(stderr,
...
static void shadow_safe_add_result_fn_err(ShadowSafeCtx* ctx, const char* name,
    ...
    if (!ctx || !name || !name[0]) return;
    for (i = 0; i < ctx->nrfn; i++) {
        if (strcmp(ctx->result_fns[i], name) == 0) {
            if (err_ty && err_ty[0] && !ctx->result_fn_errs[i][0])
                snprintf(ctx->result_fn_errs[i], sizeof(ctx->result_fn_errs[0]),
                         "%s", err_ty);
            return;
        }
    }
    if (ctx->nrfn >= SHADOW_SAFE_RFN_CAP) {
        shadow_safe_err_at(ctx, NULL,
                           "safety result-fn table capacity exceeded (64)");
        return;
    }
    snprintf(ctx->result_fns[ctx->nrfn], sizeof(ctx->result_fns[0]), "%s",
             name);
    ctx->result_fn_errs[ctx->nrfn][0] = 0;
    if (err_ty && err_ty[0])
        snprintf(ctx->result_fn_errs[ctx->nrfn],
                 sizeof(ctx->result_fn_errs[0]), "%s", err_ty);
    ctx->nrfn++;
}
...
static const char* shadow_safe_result_fn_err(ShadowSafeCtx* ctx,
    ...
    for (i = 0; i < ctx->nrfn; i++)
        if (strcmp(ctx->result_fns[i], name) == 0)
            return ctx->result_fn_errs[i][0] ? ctx->result_fn_errs[i] : NULL;
    return NULL;
}
static int shadow_safe_as_face_of(ShadowSafeCtx* ctx, const char* outer,
    ...
    for (si = 0; si < ctx->nstructs; si++) {
        ...
        for (fi = 0; fi < s->nf; fi++) {
            ...
            if (strcmp(s->fty[fi], face) == 0) return 1;
        }
    }
    return 0;
}
...
static void shadow_safe_on_unwrap(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    if (st->kind == AST_STMT_UNWRAP) {
        ...
        binder = st->d;
        if (!mode[0]) {
            /* `call() !>;` — ambiguous @as faces across in-scope handlers. */
            {
                char callee[64];
                const char* p = st->a;
                size_t ni = 0;
                const char* ety;
                int exact = 0, as_hits = 0, i;
                while (*p == ' ' || *p == '\t') p++;
                ...
                if (ety && ety[0]) {
                    for (i = 0; i < ctx->neh; i++) {
                        AstNode* eh = ctx->eh_stack[i];
                        if (!eh || !eh->a[0]) continue;
                        if (strcmp(eh->a, ety) == 0)
                            exact++;
                        else if (shadow_safe_as_face_of(ctx, ety, eh->a))
                            as_hits++;
                    }
                    if (!exact && as_hits > 1) {
                        char msg[192];
...
static void shadow_safe_check_escape_stack_slice(ShadowSafeCtx* ctx,
    ...
    while (p && *p) {
        ...
        if (!*p) break;
        n = 0;
        if (*p == '&') p++; /* ref capture of stack slice still escapes */
...
static void shadow_safe_walk_stmt(ShadowSafeCtx* ctx, AstNode* st) {
    ...
    case AST_PTR_INIT:
        ...
        if (!ctx->err && st->c[0] &&
            ...
            int is_ptr = (st->kind == AST_PTR_INIT) ||
                         (st->d[0] == '*');
            snprintf(msg, sizeof(msg),
                     "type: `%s%s` created with '@create' requires explicit "
                            ...
                            if (ft && ft->bytes) {
                                ...
                                for (i = st->tok_off; i + wl < ft->len; i++) {
                                    if (memcmp(ft->bytes + i, want, wl) == 0) {
                                        int fl = 1, fc = 1;
                                        offset_to_linecol(ft->bytes, ft->len, i,
                                                          &fl, &fc);
                                        force_col = fc;
                                        break;
                                    }
    ...
    case AST_NURSERY_DESTROY: {
        ...
        for (vi = saved_nvars; vi < ctx->nvars; vi++) {
            if (shadow_safe_type_is_nursery(ctx->vars[vi].ty))
...
static void shadow_safe_check_field_suggest_text(ShadowSafeCtx* ctx, AstNode* st,
    ...
    while (*p && !ctx->err) {
        ...
        v = NULL;
        for (i = 0; i < ctx->nvars; i++) {
            if (strcmp(ctx->vars[i].name, recv) == 0) {
                v = &ctx->vars[i];
                break;
            }
        }
