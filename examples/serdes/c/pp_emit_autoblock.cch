...
enum { SHADOW_AB_BATCH_CAP = 64 };
enum { SHADOW_AB_TAG_CAP = 128 };
...
static void shadow_ab_tag_set(AstNode* st, const char* tag) {
    ...
    if (g_shadow_nab_tags >= SHADOW_AB_TAG_CAP) return;
...
static const char* shadow_ab_arg_cast_fn(const char* callee, int argi,
    ...
    if (arg[0] == '&') return "(intptr_t)";
    p = arg;
    while (*p == ' ' || *p == '\t') p++;
    if (*p >= '0' && *p <= '9') return "(intptr_t)";
    if (*p == '-' && p[1] >= '0' && p[1] <= '9') return "(intptr_t)";
    fp = callee ? shadow_fnparam_lookup(callee, argi) : NULL;
    if (fp && fp->stars > 0) return "(intptr_t*)";
    {
        const ShadowBind* lb = shadow_bind_lookup(arg);
        if (lb && lb->ty[0] && strchr(lb->ty, '*')) return "(intptr_t*)";
    }
    return "(intptr_t)";
}
...
static int shadow_callee_is_known_cc(const char* name) {
    return name && name[0] && shadow_ufn_exists(name);
}
...
static void shadow_ab_batch_save(const ShadowAbBatch* b) {
    if (!b || b->id < 0 || b->id >= SHADOW_AB_BATCH_CAP) return;
...
/* True when every arg can travel through intptr_t / pointer casts. */
static int shadow_ab_args_intptr_ok(const char* callee, const char* args) {
    char argv[8][128];
    int argc;
    int i;
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (*p == '"' || *p == '\'') continue;
        fp = callee ? shadow_fnparam_lookup(callee, i) : NULL;
        if (fp) {
            if (fp->stars > 0) continue;
            if (fp->base[0] && shadow_ab_ty_by_value_agg(fp->base)) return 0;
        }
        lb = shadow_bind_lookup(p);
        if (lb && lb->ty[0] && shadow_ab_ty_by_value_agg(lb->ty)) return 0;
    }
    return 1;
}
...
static int shadow_call_should_autoblock_args(AstNode* st, ShadowCtx* ctx,
    ...
    return shadow_ab_args_intptr_ok(fn, args);
}
...
static int shadow_ab_fill_item(AstNode* st, ShadowCtx* ctx, ShadowAbItem* it) {
    if (!st || !it) return 0;
    it->st = st;
    snprintf(it->callee, sizeof(it->callee), "%s", st->a);
    shadow_emit_expr_text(st, st->b, it->args, sizeof(it->args), NULL);
    return shadow_call_should_autoblock_args(st, ctx, st->a, it->args);
}

static int shadow_ab_try_tail(AstNode* st, ShadowCtx* ctx, ShadowAbBatch* b) {
    char fn[64], args[288];
    if (!st || !b) return 0;
    if (st->kind == AST_RETURN_CC && st->b[0]) {
        if (!shadow_parse_call_expr(st->b, fn, sizeof(fn), args, sizeof(args)))
            return 0;
        if (!shadow_call_should_autoblock_args(st, ctx, fn, args)) return 0;
        b->tail_kind = 1;
    ...
    while (i < nbody) {
        ...
        if (st->kind == AST_CALL_ARGS && !st->d[0]) {
            ...
            while (j < nbody && batch.n < SHADOW_AB_BATCH_MAX) {
                ShadowAbItem it = {0};
                if (!shadow_ab_fill_item(body[j], ctx, &it)) break;
                batch.items[batch.n++] = it;
                j++;
            }
            if (batch.n >= 2 ||
                (batch.n == 1 && j < nbody &&
                 shadow_ab_try_tail(body[j], ctx, &batch))) {
        ...
        if (st->kind == AST_TYPED_INIT && !shadow_ab_tag_get(st)) {
            ...
            if (shadow_parse_call_expr(st->c, fn, sizeof(fn), args,
                                       sizeof(args)) &&
                shadow_call_should_autoblock_args(st, ctx, fn, args)) {
...
static void shadow_prescan_autoblock_stmt(AstNode* st, ShadowCtx* ctx,
                                          CEmit* out) {
    if (!st) return;
    if (st->kind == AST_DEFER) return;
    if (st->kind == AST_BLOCK) {
...
static int shadow_emit_assign_maybe_autoblock(AstNode* st, CEmit* out,
    ...
    if (lb && shadow_ty_is_structy(lb->ty)) {
        ...
        if (!cemit_fmt(out, "));\n")) return 0;
        return cemit_fmt(out, "%s%s = __cc_ab_ret_%d;\n", indent, st->a, id);
    }
