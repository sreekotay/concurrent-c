...
| cctext external review, 2026-09, checked against head `7b53945`: dirty is `hist.head != saved_head` (`document.ccs:411`); `rtx_hist_drop_tail` sets `len = head` without touching `saved_head` (`:6572`); `RtxWs_save_buf` returns ok when not dirty (`workspace.ccs:1871`). A divergent edit after save and undo reuses the position and the save is skipped | cctext | defect (read; identity compared by position) |
| cctext: `rx_search_bt` counts a budget exhaustion and moves to the next start, returning `NONE` or a later match (`rx.ccs:4826-4832`); the replace scanner handles `NONE` and `PARTIAL` only (`find.ccs:2053-2057`) | cctext | defect (read; a three-way outcome collapsed into two) |
| cctext: workbook rename restores the text with `RtxDoc_undo` after rejecting (`wb.ccs:4251`), which leaves the rejected edit in redo and cannot restore a dropped redo branch | cctext | defect (read; rollback of one fact without its co-facts) |
| cctext: CI workflows pin concurrent-c `e59f6b9` (3 sites) while both Dockerfiles pin `0a63396` (2 sites) | `.github/workflows/`, `docker/` | defect (duplicated constant, drifted) |
| cctext: backup save through the original inode leaves ORIGINAL piece references meaning different bytes (`document.ccs:10224-10327`) | external review | defect (reviewer's reduced reproduction; not checked here) |
