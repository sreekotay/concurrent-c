...
static int test_bump(void) {
    ...
    off = cc_arena_slab_offset(a.a);

    /* Tip pop. */
    if (!cc_arena_release_sized(a, r, 32)) return fail(1, "tip release");
    if (cc_arena_slab_offset(a.a) != off - 32) return fail(1, "tip popped");
    if (cc_arena_slab_live(a.a) != 2) return fail(1, "live after pop");
    ...
    if (cc_arena_slab_live(a.a) != 2) return fail(1, "refusal leaves the count");

    /* Hole: not at the tip. */
    if (!cc_arena_release_sized(a, p, 32)) return fail(1, "hole release");
    if (cc_arena_slab_offset(a.a) != off - 32) return fail(1, "hole keeps the tip");
    if (cc_arena_slab_live(a.a) != 1) return fail(1, "live after hole");
    ...
    if (cc_arena_slab_offset(a.a) != 0) return fail(1, "last-live rewind");
    ...
    if (cc_arena_slab_offset(a.a) != off) return fail(1, "unsized release cannot pop");
    cc_arena_free(&a);
...
static int test_reuse(void) {
    ...
    if (cc_arena_slab_live(a.a) != 1) return fail(2, "class table counted live");
    ...
    if (cc_arena_slab_live(a.a) != 3) return fail(2, "listed block stays counted live");
    r = cc_arena_alloc(a, 50, 8);   /* class 64: re-served from the list */
    if (r != p) return fail(2, "same class reuses the block");
    if (cc_arena_slab_offset(a.a) != off) return fail(2, "reuse did not bump");
    ...
    if (cc_arena_slab_offset(a.a) != off) return fail(2, "tip pop preferred");
