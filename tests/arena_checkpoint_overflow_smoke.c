...
static int test_malloc_ctor_epoch_drain(void) {
    ...
    }
    {
        int n = 0;
        CCArenaOvfHeader *h;
        for (h = a.ovf_head; h; h = h->next) n++;
        if (n != 1) {
            cc_arena_free(&a);
            return fail(1, "post-checkpoint overflow should be drained");
        }
        if (a.ovf_head != cc__arena_ovf_header(keep)) {
            cc_arena_free(&a);
...
static int test_stack_chunk_epoch_drain(void) {
    ...
    void *drop;
    CCArenaCheckpoint cp;
    uint8_t *stack_buf = s.base;
    int n;

    s.block_max = 2; /* root + one grow, then chunk overflow */
    ...
    if (!keep || cc__arena_find_block(&s, keep) || !s.ovf_chunks) {
        cc_arena_free(&s);
    ...
    {
        CCArenaOvfChunk *c;
        for (c = s.ovf_chunks; c; c = c->next) n++;
    }
    if (n != 1) {
        cc_arena_free(&s);
...
static int test_heap_chunk_epoch_drain(void) {
    ...
    if (!keep || cc__arena_find_block(&a, keep) || !a.ovf_chunks) {
        cc_arena_free(&a);
    ...
    {
        CCArenaOvfChunk *c;
        for (c = a.ovf_chunks; c; c = c->next) n++;
    }
    if (n != 1) {
        cc_arena_free(&a);
