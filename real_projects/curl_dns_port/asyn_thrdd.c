 ...
 * Does nothing here.
 */
/* CcResolvQ lives in multi->resolv_thrdq (same pointer slot). */
#define CC_RESOLV_Q(m) ((CcResolvQ *)(m)->resolv_thrdq)
                              ...
                              struct Curl_resolv_async *async)
{
  ...
  if(async->queries_ongoing && !async->done &&
    ...
    CcResolvQ_clear(CC_RESOLV_Q(data->multi),
...
#ifdef ENABLE_WAKEUP
static void async_thrdd_event(void *user_data)
{
...
#endif
    CcResolvQ_destroy(CC_RESOLV_Q(multi), join);
...
void Curl_async_thrdd_multi_process(struct Curl_multi *multi)
{
  ...
  while(!CcResolvQ_recv(CC_RESOLV_Q(multi), &qitem)) {
...
CURLcode Curl_async_thrdd_multi_set_props(struct Curl_multi *multi,
                                          ...
                                          uint32_t idle_time_ms)
{
  return (CURLcode)CcResolvQ_set_props(CC_RESOLV_Q(multi), 0,
                              min_threads, max_threads, idle_time_ms);
}
                                  ...
                                  uint8_t dns_queries)
{
  ...
  result = (CURLcode)CcResolvQ_send(CC_RESOLV_Q(data->multi), item,
                           async_item_description(item), (int64_t)async->timeout_ms);
  if(result)
    goto out;
  item = NULL;
  async->queries_ongoing++;
