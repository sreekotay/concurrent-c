...
# next server's benchmark hits "connection reset".
...
echo "Redis benchmark: upstream vs redis_idiomatic"
...
if ! kill -0 "$idiomatic_pid" 2>/dev/null; then
    echo "redis_idiomatic exited during its suite; log tail:" >&2
    tail -80 "$idiomatic_log" >&2
    exit 1
fi
stop_server "$idiomatic_pid"
idiomatic_pid=""
...
print("command\tupstream_rps\tidiomatic_rps\tidiomatic/upstream\tupstream_p50_ms\tidiomatic_p50_ms\tupstream_rps_range\tidiomatic_rps_range")
for cmd in cmds:
